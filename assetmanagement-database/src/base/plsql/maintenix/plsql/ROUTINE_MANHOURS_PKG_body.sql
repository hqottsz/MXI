...
--changeSet ROUTINE_MANHOURS_PKG_body:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
OPEN lcur_JICListCursor FOR
 ...
 SELECT
  ...
  WHERE
     ...
     AND
     ref_task_class.task_class_cd NOT IN ('CLOSEPANEL', 'OPENPANEL', 'MPC')
     AND
     jic_task.rstat_cd 		  = 0
     AND
     req_task.rstat_cd = 0
...
SELECT
   SUM( (task_labour_list.man_pwr_ct) * (NVL(task_labour_list.work_perf_hr,0) + NVL(task_labour_list.cert_hr,0) + NVL(task_labour_list.insp_hr,0)) )
INTO ln_ManHours
FROM
   task_labour_list
WHERE
...
PROCEDURE calculateRoutineManhrs(
    ...
    an_STDPlanningTypeId       IN task_task.planning_type_id%TYPE,
    as_STDPlanningTypeCd       IN eqp_planning_type.planning_type_cd%TYPE,
    an_ltab_JICList            IN OUT mxkeytable
    )
IS
    ln_LabourHrs                NUMBER;
    on_Return                   NUMBER;
   ...
   lb_IsJICDuplicate        := FALSE;
        ...
        IF( ab_ReqWorkscopeBool= 0) THEN
           ...
           lcur_JICTaskList := getJICForReq(
            ...
            lb_IsJICDuplicate        := FALSE;
            ln_LabourHrs             := 0;
            an_ltab_JICList.EXTEND;
            -- Iterate over the JIC list to find if this JIC is duplicate or not
            FOR j IN 1..an_ltab_JICList.COUNT
              LOOP
                  IF (ln_JICTaskDbId = an_ltab_JICList(j).db_id AND
                      ln_JICTaskId = an_ltab_JICList(j).id) THEN
              ...
              IF (lb_IsJICDuplicate = FALSE) THEN
                 ...
                 an_ltab_JICList(an_ltab_JICList.LAST) := mxkey( ln_JICTaskDbId, ln_JICTaskId);
            		 -- Get Labour Row
