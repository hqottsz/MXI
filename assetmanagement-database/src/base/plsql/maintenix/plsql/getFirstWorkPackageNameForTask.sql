...
--changeSet getFirstWorkPackageNameForTask:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/********************************************************************************
*
* Function:      getFirstWorkPackageNameForTask
...
* Description:   This function determines if there is a work package available in the
		 ...
		 returns the task name of that opportunity
*
* Orig.Coder:    CDALEY
* Recent Date:   May 22, 2007
*
*********************************************************************************/
CREATE OR REPLACE FUNCTION getFirstWorkPackageNameForTask
(
    aLpaDbId number,
    aSchedDbId number,
    aSchedId number,
...
IS
   ls_WP_Task_Name String(2000);

BEGIN
   SELECT
      wp_task_name
   INTO ls_WP_Task_Name
   FROM
   (
      SELECT
         FIRST_VALUE(wp_event.event_sdesc) OVER ( ORDER BY wp_event.event_status_cd ) AS wp_task_name,
         RANK() OVER (order by wp_event.event_status_cd) as rank_number
      FROM
         lpa_stask_maint_op,
         -- task tables
         sched_stask,
   ...
   RETURN ls_WP_Task_Name;
END getFirstWorkPackageNameForTask;
/