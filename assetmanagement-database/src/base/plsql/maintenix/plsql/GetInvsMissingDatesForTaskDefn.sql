...
--changeSet GetInvsMissingDatesForTaskDefn:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/********************************************************************************
*
...
IS
   ...
   ls_ScheduleFromCd  task_task.task_sched_from_cd%TYPE;
   ...
   IS
	  ...
	  *
      ...
      WHERE
         ...
         AND
         task_task.task_sched_from_cd IN ( 'MANUFACT_DT', 'RECEIVED_DT' )
         -- only BLOCK and REQ
         AND
   ...
   type cursor_rec_t is table of lcur_taskdefn_invs%ROWTYPE index by binary_integer;

   lt_cursor_result       cursor_rec_t  ;
   lb_found               boolean;
   ln_recnum              integer;
...
BEGIN
   ...
   BEGIN
      ...
      SELECT
         task_task.assmbl_bom_id, task_task.task_sched_from_cd, eqp_assmbl_bom.bom_class_cd
         INTO
         ln_AssmblBomId, ls_ScheduleFromCd, ls_BomClassCd
      FROM
         task_task
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
            ...
            BEGIN  
			   SELECT
					task_sched_from_cd INTO ls_ScheduleFromCd
			   FROM
					task_task
			   WHERE
   ...
   LOOP
      ...
      IF (lt_cursor_result.count () = 0)
      THEN
               lt_cursor_result (1).inv_no_db_id := lrec_taskdefn_invs.inv_no_db_id;
               lt_cursor_result (1).inv_no_id    := lrec_taskdefn_invs.inv_no_id;
      ELSE
               lb_found := false;
               FOR ii IN 1.. lt_cursor_result.count ()
               LOOP

                      IF (lt_cursor_result (ii).inv_no_db_id = lrec_taskdefn_invs.inv_no_db_id)
                          AND
                         (lt_cursor_result (ii).inv_no_id    = lrec_taskdefn_invs.inv_no_id)               
                      THEN
                              lb_found := true;
                              EXIT;
                      END IF;
               ...
               THEN 
                   CONTINUE;
               END IF;
               ln_recnum := lt_cursor_result.count () + 1;
               lt_cursor_result (ln_recnum).inv_no_db_id := lrec_taskdefn_invs.inv_no_db_id;
               lt_cursor_result (ln_recnum).inv_no_id    := lrec_taskdefn_invs.inv_no_id;
      ...
      FROM
         (
           ...
           UNION ALL
            ...
            -- if config slot is SYS then get inventory of parent assembly
            SELECT
               inv_assmbl.inv_no_db_id, inv_assmbl.inv_no_id, inv_assmbl.inv_no_sdesc, inv_assmbl.barcode_sdesc, inv_assmbl.inv_cond_cd ,inv_assmbl.received_dt,inv_assmbl.manufact_dt,inv_assmbl.rstat_cd
            FROM
               inv_inv,
               inv_inv inv_assmbl
            WHERE
               ...
               inv_inv.inv_no_id    =  lrec_taskdefn_invs.inv_no_id
               AND
               inv_assmbl.inv_no_db_id (+)= inv_inv.assmbl_inv_no_db_id AND
               inv_assmbl.inv_no_id    (+)= inv_inv.assmbl_inv_no_id
         ) inv_tree
      WHERE
         -- conditions for Manufacture Date or Received Date
         (
          (
           ls_ScheduleFromCd = 'RECEIVED_DT' AND
           inv_tree.received_dt IS NULL
          )
          OR
          (
           ls_ScheduleFromCd = 'MANUFACT_DT' AND
         ...
         inv_tree.rstat_cd = 0
         AND
         isTaskApplicableForInventory(inv_tree.inv_no_db_id, inv_tree.inv_no_id,
                                      an_TaskDbId, an_TaskId) = 1;
       EXCEPTION
          WHEN OTHERS THEN
            lt_tab_inv_tree.delete();
      END;
