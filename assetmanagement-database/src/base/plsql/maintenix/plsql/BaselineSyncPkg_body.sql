...
--changeSet BaselineSyncPkg_body:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   LOOP
      ...
      MERGE INTO zip_task
         USING (
            SELECT /*+ cardinality (inv_list 3000) */ DISTINCT
               aZipId AS zip_id,
               sched_stask.sched_db_id,
               sched_stask.sched_id,
               ref_task_class.class_mode_cd,
      ...
      MERGE INTO zip_task
         USING (
            SELECT /*+ cardinality (inv_list 3000)  cardinality (blocks 3000) */  DISTINCT
               aZipId AS zip_id,
               sched_stask.sched_db_id,
               sched_stask.sched_id,
               ref_task_class.class_mode_cd,
...
PROCEDURE RequestZipForNewBlock(
   ...
   -- Limit of how many inventory items to fetch at one time
   lInvFetchLimit PLS_INTEGER := 3000;

   -- CURSORS

   -- All blocks and block chains to which the provided block task belongs.
   CURSOR block_cur IS
      -- get the single blocks (no chain)
      SELECT
         mxkey(task_task.task_db_id, task_task.task_id)
   ...
   -- All inventory under the assembly of the provided inventory.
   CURSOR inv_cur IS
      -- If the provided inventory is an ACFT or ASSY then return it.
      SELECT
         mxkey(inv_inv.inv_no_db_id, inv_inv.inv_no_id)
      FROM inv_inv
      WHERE
         ...
         inv_inv.locked_bool = 0
      UNION ALL
      -- If the provided inventory is an ACFT or ASSY then return all inventory installed under it.
      -- (note, installed ASSY inv have their assmbl_inv key set to the ACFT/ASSY to which
      --  they are installed, only when loose does their assmbl_inv key equal themselves)
      SELECT
         mxkey(assmbl_inv.inv_no_db_id, assmbl_inv.inv_no_id)
      FROM
         inv_inv assmbl_inv
      ...
      WHERE
         ...
         inv_inv.locked_bool = 0
      UNION ALL
      -- If the provided inventory is NOT an ACFT or ASSY then determine the assembly
      -- on which it is installed and return all the inventory installed on that assembly.
      SELECT
         mxkey(assmbl_inv.inv_no_db_id, assmbl_inv.inv_no_id)
      FROM
         inv_inv assmbl_inv
         -- All inventory that share the same assembly inventory.
         INNER JOIN inv_inv ON
            assmbl_inv.assmbl_inv_no_db_id = inv_inv.assmbl_inv_no_db_id AND
            assmbl_inv.assmbl_inv_no_id    = inv_inv.assmbl_inv_no_id
      WHERE
         ...
         inv_inv.inv_no_id    = an_InvNoId
         AND
         -- Limit to non ASSY or ACFT cases.
         ( inv_inv.inv_class_db_id, inv_inv.inv_class_cd) NOT IN ((0, 'ACFT'), (0, 'ASSY'))
         AND
         inv_inv.prevent_synch_bool = 0 AND
         inv_inv.locked_bool = 0
      UNION ALL
      -- If the provided inventory is NOT an ACFT or ASSY then return its assembly inventory.
      SELECT
         mxkey(inv_inv.assmbl_inv_no_db_id, inv_inv.assmbl_inv_no_id)
      FROM
         inv_inv assmbl_inv
...
BEGIN
   ...
   LOOP
      ...
      -- Loop through all inventories in chuncks of lInvFetchLimit.
      FETCH inv_cur
         BULK COLLECT INTO lInventoryList LIMIT lInvFetchLimit;
      ...
      -- Exit out of the loop once we're provessed all batches of the fetch limit
      EXIT WHEN lInventoryList.COUNT < lInvFetchLimit;
   ...
   -- Remove duplicate queue entries
   removeDuplicateQueues(ln_ZipId, on_Return);

   -- Activate this zip queue.
   UPDATE
      zip_queue
   SET
      actv_bool = 1
   ...
   -- Create a zip queue work item against the highest inventory .
   SELECT
...
EXCEPTION
   WHEN OTHERS THEN
      -- Unexpected error
      on_Return := icn_Error;
      APPLICATION_OBJECT_PKG.SetMxiError('DEV-99999','baselinesyncpkg@@@RequestZipForNewBlock@@@'||SQLERRM);
      RETURN;
