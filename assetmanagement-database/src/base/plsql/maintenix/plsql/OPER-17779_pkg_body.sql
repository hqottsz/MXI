...
*
*********************************************************************************/
CREATE OR REPLACE PROCEDURE FindForecastedDeadDtFcModel(
      aAircraftDbId IN inv_ac_reg.inv_no_db_id%type,
      aAircraftId IN inv_ac_reg.inv_no_id%type,
      aDataTypeDbId IN mim_data_type.data_type_db_id%type,
      aDataTypeId IN mim_data_type.data_type_id%type,
...
AS
   ...
   -- Variables
   ln_YearTotal      NUMBER := 0; -- Stores the total number of years jumped
   ln_YearSkip       NUMBER := 0; -- Stores the current number of years being skipped
   ln_YearUsage      fc_rate.forecast_rate_qt%TYPE := 0; -- Stores the annual usage
   ln_LeapDayRate    fc_rate.forecast_rate_qt%TYPE := 0; -- Stores the usage rate for the leap day
   ln_LeapYearCount  NUMBER := 0; -- Stores the number of leap years that were skipped
...
BEGIN
   ...
   -- make sure we have remaining usage is actualy specified
   ...
   -- Get the annual usage and leap year rate.  Using the collection is faster than going back to the database
   FOR indx IN 1..ltab_Ranges.COUNT LOOP

      ln_YearUsage := ln_YearUsage + (ltab_Ranges(indx).end_date - ltab_Ranges(indx).start_date) * ltab_Ranges(indx).forecast_rate;


      -- Get the leap day rate
      IF ltab_Ranges(indx).start_month < 3 AND NVL(ltab_Ranges(indx).end_month,3) > 2 THEN
         ln_LeapDayRate := ltab_Ranges(indx).forecast_rate;
         -- Adjust annual total if it was calculated using leap year data
         IF MOD(lCurrentYear,4) = 0 THEN
            ln_YearUsage := ln_YearUsage - ln_LeapDayRate;
         END IF;
      END IF;
   ...
   FOR blk IN 1..ltab_Blackouts.COUNT LOOP
      ...
      ELSIF lCurrentDate >= ltab_Blackouts(blk).end_date THEN
         ...
         -- Blackout is before the current date - get the next blackout - this should not be possible but just in case
         CONTINUE;

      ELSIF MONTHS_BETWEEN(ltab_Blackouts(blk).start_date,lCurrentDate) >= 12 AND
            lRemainingUsage/ln_YearUsage > 1 THEN

         -- Blackout is more than 1 year away - skip years
         ln_YearSkip := LEAST(TRUNC(MONTHS_BETWEEN(ltab_Blackouts(blk).start_date,lCurrentDate)/12)
                             ,TRUNC(lRemainingUsage/ln_YearUsage)
                             );

         -- Determine the number of leap years that were skipped
         ln_LeapYearCount := ADD_MONTHS(lCurrentDate,ln_YearSkip * 12) - lCurrentDate - 365 * ln_YearSkip;

         -- Adjust the current date and usage remaining
         lCurrentDate := ADD_MONTHS(lCurrentDate,ln_YearSkip * 12);
         lRemainingUsage := lRemainingUsage - ln_YearSkip * ln_YearUsage - ln_LeapYearCount * ln_LeapDayRate;

      END IF;

      -- Adjust year total
      ln_YearTotal := TRUNC(MONTHS_BETWEEN(lCurrentDate,aStartDate)/12);

      -- Three iterations are needed to cover all possible forecast ranges
      FOR rep IN 1..3 LOOP

         -- Loop until usage remaining is used up or the blackout starts
         FOR indx IN 1.. ltab_Ranges.COUNT LOOP

            -- Build index range
            ld_CurrStartDate    := ADD_MONTHS(ltab_Ranges(indx).start_date, ln_YearTotal * 12);
            ld_CurrEndDate      := ADD_MONTHS(ltab_Ranges(indx).end_date  , ln_YearTotal * 12);
   ...
   -- Determine if years can be skiped
   IF lRemainingUsage/ln_YearUsage > 1 THEN

      ln_YearSkip := TRUNC(lRemainingUsage/ln_YearUsage);
   ...
   FOR rep IN 1..3 LOOP
      ...
      -- Increment year for second pass
      ln_YearTotal := ln_YearTotal + 1;
