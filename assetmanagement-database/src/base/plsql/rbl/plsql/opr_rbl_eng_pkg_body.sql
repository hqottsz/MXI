...
BEGIN
   ...
   SELECT
      ...
      flight.sched_departure_dt as data_date,
      data_value_code,
      data_quantity,
      data_flag
   FROM
      acor_rbl_eng_v1
      INNER JOIN acor_rbl_eng_measure_v1 measurement ON
         acor_rbl_eng_v1.inventory_id = measurement.inventory_id
      INNER JOIN
         fl_leg flight ON
           flight.leg_id = measurement.leg_id
   WHERE
      -- year and month input param
      TRUNC(flight.sched_departure_dt) BETWEEN aidt_start_date AND aidt_end_date AND
      data_quantity = 1;
         ...
         (
             SELECT
                operator_id,
                     ...
                     COUNT(DISTINCT engine_type) AS qty
                  FROM
                 opr_rbl_eng_indicator_raw
                  WHERE
                  -- year and month input param
