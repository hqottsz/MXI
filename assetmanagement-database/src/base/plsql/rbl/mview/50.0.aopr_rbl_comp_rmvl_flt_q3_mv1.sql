...
--changeSet 50.0.aopr_rbl_comp_rmvl_flt_q3_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_RBL_COMP_RMVL_Q3_MV1');
END;
/

--changeSet 50.0.aopr_rbl_comp_rmvl_flt_q3_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW aopr_rbl_comp_rmvl_q3_mv1
BUILD DEFERRED
...
AS
  (
      SELECT
         ...
         QPA,
         year_month,
         TO_NUMBER(TO_CHAR(ADD_MONTHS(to_date(year_month,'YYYYMM'),-6),'YYYYMM')) ym_7mon,
         TO_NUMBER(TO_CHAR(ADD_MONTHS(to_date(year_month,'YYYYMM'),-8),'YYYYMM')) ym_9mon
      FROM
         rvw_rmvl_mon
  ),
rvw_rmvl_q3
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN rvw_rmvl_mon ON
              ...
              rvw_rmvl_mon.year_month BETWEEN rvw_rmvl_unq.ym_9mon AND rvw_rmvl_unq.ym_7mon
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN aopr_monthly_operator_usage_v1 monthly_usage ON
              ...
              AND -- previous remonths
              TO_NUMBER(monthly_usage.year_code || monthly_usage.month_code) BETWEEN  rvw_rmvl_unq.ym_9mon AND rvw_rmvl_unq.ym_7mon
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
...
AS
   (
      SELECT
         rvw_rmvl_q3.operator_id,
         rvw_rmvl_q3.fleet_type,
         rvw_rmvl_q3.config_slot,
         rvw_rmvl_q3.part_number,
         rvw_rmvl_q3.year_month,
         removal_count,
         unsched_removal,
         justified_failure,
         cycles,
     ...
     FROM
        rvw_rmvl_q3
        INNER JOIN rvw_usage_q3 ON
           rvw_rmvl_q3.operator_id  = rvw_usage_q3.operator_id AND
           rvw_rmvl_q3.fleet_type   = rvw_usage_q3.fleet_type AND
           rvw_rmvl_q3.config_slot  = rvw_usage_q3.config_slot AND
           rvw_rmvl_q3.part_number  = rvw_usage_q3.part_number AND
           rvw_rmvl_q3.year_month   = rvw_usage_q3.year_month
   )
SELECT
   operator_id,
   fleet_type,
   ...
   part_number,
   year_month,
   -- q3 - between 1st and 3rd month
   NVL(removal_count,0)      AS removal_q3,
   NVL(unsched_removal,0)    AS unsched_removal_q3,
   NVL(justified_failure,0)  AS justified_failure_q3,
   NVL(cycles,0)             AS total_cycle_q3,
   NVL(flight_hours,0)       AS total_hour_q3,
   NVL(mtbr,0)         AS mtbr_q3,
   NVL(mcbr,0)         AS mcbr_q3,
   NVL(mtbur,0)        AS mtbur_q3,
   NVL(mcbur,0)        AS mcbur_q3,
   NVL(mtbf,0)         AS mtbf_q3,
   NVL(mcbf,0)         AS mcbf_q3,
   NVL(failure_rate,0) AS f_rate_q3,
   NVL(urr_hours,0)    AS urr_hour_q3,
   NVL(urr_cycles,0)   AS urr_cycle_q3
FROM
   rvw_rmvl_q3_avg;