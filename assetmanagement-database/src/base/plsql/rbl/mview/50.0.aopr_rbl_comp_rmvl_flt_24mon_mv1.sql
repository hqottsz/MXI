...
--changeSet 50.0.aopr_rbl_comp_rmvl_flt_24mon_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_RBL_COMP_RMVL_24MON_MV1');
END;
/

--changeSet 50.0.aopr_rbl_comp_rmvl_flt_24mon_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW aopr_rbl_comp_rmvl_24mon_mv1
BUILD DEFERRED
...
AS
  (
      SELECT
         ...
         QPA,
         year_month,
         TO_NUMBER(TO_CHAR(ADD_MONTHS(to_date(year_month,'YYYYMM'),-23),'YYYYMM')) ym_24mon
      FROM
         rvw_rmvl_mon
  ),
rvw_rmvl_24mon
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN rvw_rmvl_mon ON
              ...
              rvw_rmvl_mon.year_month BETWEEN rvw_rmvl_unq.ym_24mon AND rvw_rmvl_unq.year_month
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN aopr_monthly_operator_usage_v1 monthly_usage ON
              ...
              AND -- previous remonths
              TO_NUMBER(monthly_usage.year_code || monthly_usage.month_code) BETWEEN rvw_rmvl_unq.ym_24mon AND rvw_rmvl_unq.year_month
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
...
AS
   (
      SELECT
         rvw_rmvl_24mon.operator_id,
         rvw_rmvl_24mon.fleet_type,
         rvw_rmvl_24mon.config_slot,
         rvw_rmvl_24mon.part_number,
         rvw_rmvl_24mon.year_month,
         removal_count,
         unsched_removal,
         justified_failure,
         cycles,
     ...
     FROM
        rvw_rmvl_24mon
        INNER JOIN rvw_usage_24mon ON
           rvw_rmvl_24mon.operator_id  = rvw_usage_24mon.operator_id AND
           rvw_rmvl_24mon.fleet_type   = rvw_usage_24mon.fleet_type AND
           rvw_rmvl_24mon.config_slot  = rvw_usage_24mon.config_slot AND
           rvw_rmvl_24mon.part_number  = rvw_usage_24mon.part_number AND
           rvw_rmvl_24mon.year_month   = rvw_usage_24mon.year_month
   )
SELECT
   operator_id,
   fleet_type,
   ...
   part_number,
   year_month,
   -- 24mon - between 1st and 3rd month
   NVL(removal_count,0)      AS removal_24mon,
   NVL(unsched_removal,0)    AS unsched_removal_24mon,
   NVL(justified_failure,0)  AS justified_failure_24mon,
   NVL(cycles,0)             AS total_cycle_24mon,
   NVL(flight_hours,0)       AS total_hour_24mon,
   NVL(mtbr,0)         AS mtbr_24mon,
   NVL(mcbr,0)         AS mcbr_24mon,
   NVL(mtbur,0)        AS mtbur_24mon,
   NVL(mcbur,0)        AS mcbur_24mon,
   NVL(mtbf,0)         AS mtbf_24mon,
   NVL(mcbf,0)         AS mcbf_24mon,
   NVL(failure_rate,0) AS f_rate_24mon,
   NVL(urr_hours,0)    AS urr_hour_24mon,
   NVL(urr_cycles,0)   AS urr_cycle_24mon
FROM
   rvw_rmvl_24mon_avg;