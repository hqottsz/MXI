...
--changeSet 50.1.aopr_rbl_fault_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_RBL_FAULT_MV1');
END;
/

--changeSet 50.1.aopr_rbl_fault_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW aopr_rbl_fault_mv1
BUILD DEFERRED
...
WITH
   ...
   ( -- previous 3 months
     ...
     FROM
        ...
        LEFT JOIN rvw_raw ON
           ...
           rvw_unq.tail_number       = rvw_raw.tail_number AND
   ...
   SELECT
      ...
      rvw_unq.fleet_type,
      rvw_unq.tail_number            AS registration_code,
      rvw_unq.config_slot_code,
      rvw_unq.fault_source_code,
      ...
      NVL(tail_ucl.ucl_fh,0)  AS m12_m3to15_ucl_fh,
      NVL(tail_ucl.ucl_fc,0)  AS m12_m3to15_ucl_fc
   FROM
      rvw_unq
      LEFT JOIN rvw_raw m1 ON
         -- current month
         rvw_unq.operator_id       = m1.operator_id AND
         rvw_unq.tail_number       = m1.tail_number AND
      ...
      LEFT JOIN rvw_raw m2 ON
         ...
         rvw_unq.tail_number       = m2.tail_number AND
      ...
      LEFT JOIN rvw_raw m3 ON
         ...
         rvw_unq.tail_number       = m3.tail_number AND
      ...
      INNER JOIN rvw_fault_m3 ON
         ...
         rvw_unq.tail_number       = rvw_fault_m3.tail_number AND
      ...
      INNER JOIN rvw_fault_m6 ON
         ...
         rvw_unq.tail_number       = rvw_fault_m6.tail_number AND
      ...
      INNER JOIN rvw_fault_m12 ON
         ...
         rvw_unq.tail_number       = rvw_fault_m12.tail_number AND
      ...
      LEFT JOIN aopr_rbl_fault_tail_ucl_mv1 tail_ucl ON
         -- check the materialized view opr_rbl_fault_tail_ucl_mv1 for computation
         rvw_unq.operator_id       = tail_ucl.operator_id AND
         rvw_unq.tail_number       = tail_ucl.tail_number AND
         rvw_unq.config_slot_code  = tail_ucl.config_slot_code
         AND -- previous year
         TO_NUMBER(SUBSTR(rvw_unq.year_month,1,4)-1) = tail_ucl.year_code;
