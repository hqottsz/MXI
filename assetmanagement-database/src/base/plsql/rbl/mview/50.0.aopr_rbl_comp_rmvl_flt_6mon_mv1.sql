...
--changeSet 50.0.aopr_rbl_comp_rmvl_flt_6mon_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_RBL_COMP_RMVL_6MON_MV1');
END;
/

--changeSet 50.0.aopr_rbl_comp_rmvl_flt_6mon_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW aopr_rbl_comp_rmvl_6mon_mv1
BUILD DEFERRED
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN rvw_rmvl_mon ON
              ...
              rvw_rmvl_mon.year_month BETWEEN rvw_rmvl_unq.ym_6mon AND rvw_rmvl_unq.year_month
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
   ...
   (
      ...
      FROM
         ...
         LEFT JOIN aopr_monthly_operator_usage_v1 monthly_usage ON
              ...
              AND -- previous remonths
              TO_NUMBER(monthly_usage.year_code || monthly_usage.month_code) BETWEEN rvw_rmvl_unq.ym_6mon AND rvw_rmvl_unq.year_month
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
...
AS
   (
      SELECT
         rvw_rmvl_6mon.operator_id,
         rvw_rmvl_6mon.fleet_type,
         rvw_rmvl_6mon.config_slot,
         rvw_rmvl_6mon.part_number,
         rvw_rmvl_6mon.year_month,
         removal_count,
         unsched_removal,
         justified_failure,
         cycles,
     ...
     FROM
        rvw_rmvl_6mon
        INNER JOIN rvw_usage_6mon ON
           rvw_rmvl_6mon.operator_id  = rvw_usage_6mon.operator_id AND
           rvw_rmvl_6mon.fleet_type   = rvw_usage_6mon.fleet_type AND
           rvw_rmvl_6mon.config_slot  = rvw_usage_6mon.config_slot AND
           rvw_rmvl_6mon.part_number  = rvw_usage_6mon.part_number AND
           rvw_rmvl_6mon.year_month   = rvw_usage_6mon.year_month
   )
SELECT
   operator_id,
   fleet_type,
   ...
   part_number,
   year_month,
   -- 6mon - between 1st and 3rd month
   NVL(removal_count,0)      AS removal_6mon,
   NVL(unsched_removal,0)    AS unsched_removal_6mon,
   NVL(justified_failure,0)  AS justified_failure_6mon,
   NVL(cycles,0)             AS total_cycle_6mon,
   NVL(flight_hours,0)       AS total_hour_6mon,
   NVL(mtbr,0)         AS mtbr_6mon,
   NVL(mcbr,0)         AS mcbr_6mon,
   NVL(mtbur,0)        AS mtbur_6mon,
   NVL(mcbur,0)        AS mcbur_6mon,
   NVL(mtbf,0)         AS mtbf_6mon,
   NVL(mcbf,0)         AS mcbf_6mon,
   NVL(failure_rate,0) AS f_rate_6mon,
   NVL(urr_hours,0)    AS urr_hour_6mon,
   NVL(urr_cycles,0)   AS urr_cycle_6mon
FROM
   rvw_rmvl_6mon_avg;