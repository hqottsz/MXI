...
--changeSet acor_non_routine_fault_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('ACOR_NON_ROUTINE_FAULT_MV1');
END;
/

--changeSet acor_non_routine_fault_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW acor_non_routine_fault_mv1
BUILD DEFERRED
...
AS
  (
    SELECT
       sched_db_id,
       sched_id,
       LISTAGG(action_ldesc, ' : ') WITHIN GROUP (ORDER BY sched_db_id, sched_id) action_description
    FROM
      sched_action
    GROUP BY
       sched_db_id,
...
SELECT
   ...
   rvw_action.action_description    AS corr_action_description
FROM
   sd_fault
   -- found date
   ...
   -- corrective action
   LEFT JOIN rvw_action ON
      corr_task.sched_db_id = rvw_action.sched_db_id AND
      corr_task.sched_id    = rvw_action.sched_id
   -- work package
