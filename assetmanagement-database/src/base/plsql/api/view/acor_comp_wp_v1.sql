...
--changeSet acor_comp_wp_v1:1 stripComments:false
CREATE OR REPLACE FORCE VIEW acor_comp_wp_v1 
AS
SELECT
    ...
    inv_inv.alt_id                AS component_id,
    inv_inv.serial_no_oem         AS component_serial_number,
    eqp_part_no.part_no_oem       AS component_part_number,
    eqp_part_no.manufact_cd       AS component_manufacturer,
    eqp_part_no.alt_id            AS component_part_id,
...
FROM
   ...
   LEFT JOIN acor_comp_usg_wp_completion_v1 hr_usg ON
      sched_stask.alt_id = hr_usg.sched_id AND
      hr_usg.data_type_code = 'HOURS'
   -- completion usage cycles
   LEFT JOIN acor_comp_usg_wp_completion_v1 cyc_usg ON
