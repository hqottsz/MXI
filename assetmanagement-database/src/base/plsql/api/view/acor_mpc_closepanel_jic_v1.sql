...
--changeSet acor_mpc_closepanel_jic_v1:1 stripComments:false
CREATE OR REPLACE FORCE VIEW acor_mpc_closepanel_jic_v1
AS
WITH
rvw_task_panel
AS (
...
SELECT
   ...
   org_org.alt_id                    AS orgnization_id,
...
FROM
   ...
   INNER JOIN rvw_task_panel task_panel ON
      task_task.alt_id    = task_panel.task_id
      AND
      task_task.assmbl_db_id = task_panel.assmbl_db_id AND
      task_task.assmbl_cd    = task_panel.assmbl_cd
   -- assmbl bom
...
WHERE
   ...
   ref_task_class.task_class_cd    = 'CLOSEPANEL'
;