...
--changeSet OPER-28579:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet OPER-28579:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet OPER-28579:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
--comment added task created event publish after genschedtask
CREATE OR REPLACE PACKAGE BODY SCHED_STASK_PKG IS
   ...
   /* Create all on condition REQs for a new mandatory block */
   ...
   *        ad_PreviousCompletionDt      (date) - The completion date of the installation task, that triggered
   *                                          the create_on_install logic. NULL otherwise.
   *        an_MaxRangeQt            (number) - Caps the task's forecast_range_qt.
   *                                            A value of 0 or less will skip GenForecastedTasks
   * Return:
      ...
      END IF;
         ...
         IF ab_historic = false THEN
            ...
            IF lrec_TaskDefinitionInfo.class_mode_cd <> 'JIC' THEN
               ...
               /* Publish task created event */
	           axon_event_publish_pkg.publish_task_created_event(
	              on_SchedDbId,
	              on_SchedId,
	              an_TaskDbId,
	              an_TaskId,
            ...
            IF ( lrec_TaskDefinitionInfo.class_mode_cd = 'REQ' AND lrec_TaskDefinitionInfo.task_class_cd <> 'CORR') THEN
               AutoCreateChildTasks( on_SchedDbId, on_SchedId, an_MaxRangeQt, on_Return );
               IF on_Return < 1 THEN
                  RETURN;
               END IF;
            ...
            ELSIF lrec_TaskDefinitionInfo.class_mode_cd = 'BLOCK' AND lrec_TaskDefinitionInfo.on_condition_bool = 0 THEN
               AutoCreateOnCondChildTasks( on_SchedDbId, on_SchedId, an_MaxRangeQt, on_Return );
               IF on_Return < 1 THEN
                  RETURN;
               END IF;
            END IF;
              ...
              IF lrec_TaskDefinitionInfo.forecast_range_qt > 0 AND NVL(an_MaxRangeQt,lrec_TaskDefinitionInfo.Forecast_Range_Qt) > 0 THEN
                 GenForecastedTasks( on_SchedDbId, on_SchedId, an_MaxRangeQt, ls_ExitCd, on_Return );
                 IF on_Return < 1 THEN
                    RETURN;
                 END IF;
              END IF;
      ...
      IF ( an_PreviousTaskDbId <> -1 AND an_PreviousTaskId <> -1 AND an_NextTaskDbId <> -1 AND an_NextTaskId <> -1 ) THEN
        ...
        IF ab_historic = false THEN
           ...
           IF lrec_TaskDefinitionInfo.class_mode_cd = 'BLOCK' AND lrec_TaskDefinitionInfo.on_condition_bool = 0 THEN
               AutoCreateOnCondChildTasks( on_SchedDbId, on_SchedId, NULL, on_Return );
               IF on_Return < 1 THEN
                  RETURN;
               END IF;
            END IF;
...
/********************************************************************************
   ...
   PROCEDURE GenOneSchedTask (
    ...
    BEGIN
      ...
      IF (ln_PreviousTaskDbId <> -1 ) THEN
         ...
         -- but only if the task is recurring OR part of a block chain OR the task has a non-zero forecast range
         IF ( ln_PreviousTaskHistBool <> 1
              ...
              (
                 ...
                 OR
                 lrec_TaskDefinition.forecast_range_qt <> 0
              )
            ) THEN
   ...
   *********************************************************************************/
   ...
   BEGIN
      ...
      /* Use the smaller value between the task's forecast range and the input argument */
      lrec_TaskDetails.forecast_range_qt := LEAST(NVL(an_MaxRangeQt,lrec_TaskDetails.forecast_range_qt),lrec_TaskDetails.forecast_range_qt);
      ...
      /* do not forecast non-historical tasks with forecast range (or forecast override) set to 0 or less. */
   ...
   *
   * Description:
   *    The following ensures that an individual task has accurate Boolean values
   *       1. Default value in sched_stask table is 1/TRUE for both Booleans
   *       2. Insert into sched_stask in GenOneSchedTask does not include the
   *          Boolean columns so the default of TRUE will be used on initial insert
   *       3. GenerateRemoveInstallPartReq flips parts_ready_bool to FALSE if any
   *          sched_inst_part rows were added
   *       4. GenOneSchedTask flips tools_ready_bool to FALSE if any evt_tool rows
   *          were added
   *
   *    Since the above takes care of the immediate data, when generating child
   *    tasks we are only concerned with cases where the child might cause the
   *    parent to flip from TRUE to FALSE.  This is why the parent keys are provided
   *    after generating child tasks.
   *
   * Orig.Coder:
   * Recent Coder: Guillaume Turcotte
   * Recent Date:  22 Nov 2018
   *
   *********************************************************************************
   *
         ...
         EXISTS
            ...
            FROM
               ...
               INNER JOIN sched_stask child_stask ON
                  evt_event.event_db_id = child_stask.sched_db_id AND
                  evt_event.event_id    = child_stask.sched_id
            WHERE
               evt_event.nh_event_db_id = sched_stask.sched_db_id AND
               evt_event.nh_event_id    = sched_stask.sched_id
               AND
               child_stask.parts_ready_bool <> 1
            );
      ...
      WHERE
         ...
         EXISTS
            ...
            WHERE
               ...
               evt_event.nh_event_id    = sched_stask.sched_id
               AND
               child_stask.tools_ready_bool <> 1
            );
