...
PROCEDURE CalculateTaskPriority(
   ...
   ad_SchedDeadDt   IN evt_sched_dead.sched_dead_dt%TYPE,
   av_DomainTypeCd  IN mim_data_type.domain_type_cd%TYPE,
   av_DataTypeCd    IN mim_data_type.data_type_cd%TYPE,
   af_RefMultQt     IN ref_eng_unit.ref_mult_qt%TYPE,
...
PROCEDURE SetDRVTASKForRootOrBlock(
   ...
   ln_DrivingRecordFound      NUMBER := 0;

   -- Return the driving task for children under a block
   CURSOR lcur_BlockDriver
   IS
   SELECT
   ...
   WHERE
      sched_stask.h_sched_db_id     = an_SchedDbId AND -- Use the highest scheduling event
      sched_stask.h_sched_id        = an_SchedId
      AND
      evt_sched_dead.sched_driver_bool = 1
      AND
      sched_stask.soft_deadline_bool = 0 AND
   ...
   -- Return the driving task for children under a root
   CURSOR lcur_RootDriver
   IS
   SELECT
      sched_stask.sched_db_id,
      sched_stask.sched_id,
   ...
   FROM
      ...
      INNER JOIN ref_eng_unit ON
         ...
         ref_eng_unit.eng_unit_cd    = mim_data_type.eng_unit_cd
   WHERE
      evt_event.h_event_db_id = an_SchedDbId AND -- Use the highest overall event
   ...
   lrec_Deadline lcur_BlockDriver%ROWTYPE := NULL;
...
BEGIN
   ...
   -- Get the driving event if one exists
   -- Root events and block events get children from using a different path
   IF an_IsRoot = 1 THEN

      OPEN lcur_RootDriver;
      FETCH lcur_RootDriver INTO lrec_Deadline;

      IF lcur_RootDriver%FOUND THEN

         ln_DrivingEventDbId := lrec_Deadline.sched_db_id;
         ln_DrivingEventId   := lrec_Deadline.sched_id;
         ln_DrivingRecordFound := 1;

      END IF;

      CLOSE lcur_RootDriver;

   ELSE

      OPEN lcur_BlockDriver;
      FETCH lcur_BlockDriver INTO lrec_Deadline;

      IF lcur_BlockDriver%FOUND THEN
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      IF lcur_RootDriver%ISOPEN THEN CLOSE lcur_RootDriver; END IF;
      IF lcur_BlockDriver%ISOPEN THEN CLOSE lcur_BlockDriver; END IF;
      RETURN;
...
PROCEDURE UpdateWormvlDeadline(
   ...
   CURSOR lcur_DrvTaskDeadline (
      ...
      SELECT
         ReplTaskInv.inv_no_db_id repl_inv_no_db_id,
      ...
      FROM
         ...
         mim_data_type
      WHERE
         wormvl_rel.rel_type_db_id  = 0            AND
         wormvl_rel.rel_type_cd     ='WORMVL'
         AND
         ...
         AND
         drvtask_rel.rel_type_db_id = 0                       AND
         drvtask_rel.rel_type_cd = 'DRVTASK'
         AND
...
BEGIN
   ...
   FOR lrec_ReplTask IN lcur_ReplTask
   LOOP
      DELETE
         evt_sched_dead
      WHERE
         event_db_id = lrec_ReplTask.event_db_id AND
         event_id    = lrec_ReplTask.event_id;
   END LOOP;

   /* Update REPL deadlines to the WorkOrder driving task */
   FOR lrec_DrvTaskDeadline IN lcur_DrvTaskDeadline(an_WorkPkgDbId, an_WorkPkgId)
   LOOP
      ...
      IF lcur_TsnQt%FOUND OR lrec_DrvTaskDeadline.domain_type_cd = 'CA' THEN
         ...
         INSERT INTO evt_sched_dead (
            ...
            lrec_DrvTaskDeadline.event_db_id,
         ...
         IF (
               NOT (lrec_DrvTaskDeadline.repl_inv_no_db_id = lrec_DrvTaskDeadline.drv_inv_no_db_id) OR
            ...
            WHERE
               event_db_id = lrec_DrvTaskDeadline.event_db_id AND
               event_id    = lrec_DrvTaskDeadline.event_id
               AND
               data_type_db_id = lrec_DrvTaskDeadline.data_type_db_id AND
               data_type_id    = lrec_DrvTaskDeadline.data_type_id
               AND
               (
                  sched_dead_qt <> lrec_TsnQt.Tsn_Qt+lrec_DrvTaskDeadline.usage_rem_qt
                  OR
      ...
      ELSE
         ...
         -- start yesterday at one second to midnight
         ld_StartDt := TO_DATE(TO_CHAR(SYSDATE-1, 'DD-MM-YYYY ')||'23:59:59','DD-MM-YYYY HH24:MI:SS');

         IF lrec_DrvTaskDeadline.sched_dead_dt IS NULL THEN
            ld_DeadlineDt := TO_DATE(TO_CHAR(SYSDATE, 'DD-MM-YYYY ')||'23:59:59','DD-MM-YYYY HH24:MI:SS');
         ELSE
            ld_DeadlineDt := lrec_DrvTaskDeadline.sched_dead_dt;
         END IF;

         -- days remaining is the difference between the deadline date and the start date
         lf_RemainingQt := ld_DeadlineDt - ld_StartDt;
...
*
* Orig.Coder:     Neil Ouellette
* Orig.Date:      Dec 19, 2006
* Recent Coder:   Guillaume Turcotte
* Recent Date:    Feb 12, 2018
*
*********************************************************************************
*
* Copyright 1998-2018 Mxi Technologies.  All Rights Reserved.
* Any distribution of the MxI source code by any other party than
* Mxi Technologies is prohibited.
*
*********************************************************************************/
...
IS
   ...
   lCurrentYear        NUMBER;
   --
   ld_CurrStartDate    DATE;
   ld_CurrEndDate      DATE;
   ln_CurrForecastRate NUMBER;
   --
   ln_YearCount        NUMBER           :=   0; -- Stores the total number of years to be added to the bulk collection
   lnc_YearCap         CONSTANT NUMBER  := 200; -- Capped to avoid unessessary processing.  200 years is well beyond the reasonable planning schedule

   -- Generic ranges covering the entire current calendar year
   CURSOR lCurRanges
   IS
      SELECT
         CASE
            WHEN start_date >= end_date THEN ADD_MONTHS(start_date,-12) -- Start date cannot be after the end date
         ELSE
            start_date
         END AS start_date,
         end_date,
      ...
      FROM
         (
         SELECT
            TO_DATE( lCurrentYear || '-' || start_month || '-' || start_day, 'YYYY-MM-DD' ) AS start_date,
            TO_DATE( lCurrentYear || '-' || NVL(end_month,first_month) || '-' || NVL(end_day,first_day), 'YYYY-MM-DD' ) AS end_date,
            start_month,
            end_month,
            forecast_rate
         FROM
            ...
            SELECT
               ...
               fc_range.start_day,
               LEAD (fc_range.start_month) OVER(ORDER BY fc_range.range_id) AS end_month,
               LEAD (fc_range.start_day)   OVER(ORDER BY fc_range.range_id) AS end_day,
               FIRST_VALUE (fc_range.start_month) OVER(ORDER BY fc_range.range_id) AS first_month, -- The last range must return to the first range
               FIRST_VALUE (fc_range.start_day) OVER(ORDER BY fc_range.range_id)   AS first_day,
               NVL(fc_rate.forecast_rate_qt, 1) AS forecast_rate
            FROM
                  inv_ac_reg
               LEFT JOIN fc_range ON
                  inv_ac_reg.forecast_model_db_id = fc_range.model_db_id AND
                  inv_ac_reg.forecast_model_id    = fc_range.model_id
               LEFT JOIN fc_rate ON
                  fc_range.model_db_id = fc_rate.model_db_id AND
                  fc_range.model_id    = fc_rate.model_id AND
                  fc_range.range_id    = fc_rate.range_id
                  AND
   ...
   -- Blackouts that end after the current date
   CURSOR lCurBlackouts
   IS
      ...
      WHERE
         ...
         AND
         evt_event.event_dt > lCurrentDate
      ORDER BY
         start_date,
         end_date DESC -- ordering by descending end date will avoid problems with overlapping or duplicate blackout entries
      ;

   -- Setup collections
   TYPE ltt_Ranges    IS TABLE OF lCurRanges%ROWTYPE     INDEX BY BINARY_INTEGER;
   TYPE ltt_Blackouts IS TABLE OF lCurBlackouts%ROWTYPE  INDEX BY BINARY_INTEGER;

   ltab_Ranges ltt_Ranges;       -- Stores the generic ranges
   ltab_Blackouts ltt_Blackouts; -- Stores blackouts

BEGIN

   -- Initialize local variables
   lRemainingUsage := NVL(aRemainingUsageQt,0);
   lCurrentDate    := NVL(aStartDate,SYSDATE);

   -- Exit if usage is overdue
   ...
   -- Initialize local year variable
   lCurrentYear    := TO_CHAR(lCurrentDate,'YYYY');

   -- Fetch range data
   OPEN lCurRanges;
   FETCH lCurRanges BULK COLLECT INTO ltab_Ranges;
   CLOSE lCurRanges;

   -- Exit if no forecase model was found
   IF ltab_Ranges.COUNT = 0 THEN
   ...
   -- Fetch blackout data
   OPEN lCurBlackouts;
   FETCH lCurBlackouts BULK COLLECT INTO ltab_Blackouts;
   CLOSE lCurBlackouts;

   -- Loop through all the blackouts
   FOR blk IN 1..ltab_Blackouts.COUNT LOOP

      -- Evaluate what to do about the blackout
      IF lCurrentDate >= ltab_Blackouts(blk).start_date AND lCurrentDate < ltab_Blackouts(blk).end_date THEN

         -- Currently in the blackout - skip to end of blackout and get the next blackout
         lCurrentDate := ltab_Blackouts(blk).end_date;
         CONTINUE;

      ELSIF lCurrentDate >= ltab_Blackouts(blk).end_date THEN

         -- Blackout ends before the current date - get the next blackout.  This can happen with overlapping blackouts
         CONTINUE;

      END IF;

      -- Adjust year count
      ln_YearCount := TRUNC(MONTHS_BETWEEN(lCurrentDate,aStartDate)/12);

      -- Return if we have exceeded the cap
      IF ln_YearCount > lnc_YearCap THEN
      ...
      FOR YearCount IN ln_YearCount..lnc_YearCap LOOP

         -- Loop through the bulk collection forecast ranges
         FOR indx IN 1.. ltab_Ranges.COUNT LOOP

            -- Build index range - the collection must be shifted to match the current year
            ld_CurrStartDate    := ADD_MONTHS(ltab_Ranges(indx).start_date, YearCount * 12);
            ld_CurrEndDate      := ADD_MONTHS(ltab_Ranges(indx).end_date  , YearCount * 12);
            ln_CurrForecastRate := ltab_Ranges(indx).forecast_rate;

            -- Skip records out of range or adjust the start date of the first applicable record
            IF ld_CurrEndDate <= lCurrentDate THEN
               -- Current date is not in range - go to next range
               CONTINUE;
            ELSIF ld_CurrStartDate <= lCurrentDate THEN
               -- First range should start with the current date
               ld_CurrStartDate := lCurrentDate;
            END IF;

            -- Decide what to do for this range
            IF ld_CurrEndDate >= ltab_Blackouts(blk).start_date AND
               lRemainingUsage - ln_CurrForecastRate * ( ltab_Blackouts(blk).start_date - ld_CurrStartDate) >= 0 THEN

               -- Range will intersect blackout but usage will remain - update usage, date and exit loop
               lRemainingUsage := lRemainingUsage - ln_CurrForecastRate * ( ltab_Blackouts(blk).start_date - ld_CurrStartDate);
               lCurrentDate    := ltab_Blackouts(blk).end_date;
               EXIT;

            ELSIF ( lRemainingUsage - ln_CurrForecastRate * ( ld_CurrEndDate - ld_CurrStartDate) ) <= 0 THEN
               -- Usage will run out during the current forecast range.  Calculate the exact date.
               aForecastDt := ld_CurrStartDate + lRemainingUsage / ln_CurrForecastRate;
            ...
            ELSE
               -- Usage will not run out during the current forecast range.  Substract the interval usage and move to the next forecast range.
               lCurrentDate    := ld_CurrEndDate;
               lRemainingUsage := lRemainingUsage - ln_CurrForecastRate * ( ld_CurrEndDate - ld_CurrStartDate);

            END IF;

         END LOOP;

         -- Exit the second loop if we are in a blackout
         IF lCurrentDate = ltab_Blackouts(blk).end_date THEN
   ...
   --
   --  Processing if there were no blackouts or if usage remains after the last blackout
   --

   -- Adjust year count
   ln_YearCount := TRUNC(MONTHS_BETWEEN(lCurrentDate,aStartDate)/12);

   -- Have we exceeded the cap?
   IF ln_YearCount > lnc_YearCap THEN
   ...
   FOR YearCount IN ln_YearCount..lnc_YearCap LOOP
      ...
      FOR indx IN 1.. ltab_Ranges.COUNT LOOP
         ...
         -- Skip records out of range and adjust the start date of the first applicable record
         IF ld_CurrEndDate <= lCurrentDate THEN
            -- Current date is not in range - go to next range
            CONTINUE;
         ELSIF ld_CurrStartDate <= lCurrentDate THEN
         ...
         -- Decide what to do for this range
         IF ( lRemainingUsage - ln_CurrForecastRate * ( ld_CurrEndDate - ld_CurrStartDate) ) <= 0 THEN
            -- Usage will run out during the current forecast range.  Calculate the exact date.
            aForecastDt := ld_CurrStartDate + lRemainingUsage / ln_CurrForecastRate;
   ...
   -- To get here we should have reached the cap
   IF ln_YearCount >= lnc_YearCap - 1 THEN
...
*********************************************************************************
*
* Copyright 2013 Mxi Technologies.  All Rights Reserved.
* Any distribution of the MxI source code by any other party than
* Mxi Technologies is prohibited.
*
*********************************************************************************/
...
* Arguments:      aDate : date to use,
*                 aYearCount : the year count to add
* Returns:        Date holding the passed in date plus the year count
*
* Description:    This method a new date representingthe result of adding years
*                 to a date taking into consideration leap years
*
*********************************************************************************
*
* Copyright 2013 MxI Technologies. All Rights Reserved.
...
FUNCTION AddYearsToDate(
           aDate IN DATE,
           aYearCount IN NUMBER
         )
RETURN DATE is
BEGIN
  -- since this method takes years but the add_months method takes months,
  -- we need to mutliple the year value by the number of months in a year
  -- hence the magic 12
  RETURN add_months(aDate, aYearCount  * 12);
END AddYearsToDate;
...
IS
   ...
   lIndex                NUMBER;
   lNumRecs              NUMBER;
   lEndDate_null         EXCEPTION;
   lForecastRec          ForecastPeriod := ForecastPeriod(null,null,null);
...
BEGIN
   ...
   LOOP
      FOR i IN 1 .. lNumRecs LOOP
         ...
         lForecastRec.in_StartDate := AddYearsToDate( lForecastRec.in_StartDate, lYearCount );
         lForecastRec.in_EndDate   := AddYearsToDate( lForecastRec.in_EndDate, lYearCount );
   ...
   EXCEPTION
      WHEN lEndDate_null THEN
         APPLICATION_OBJECT_PKG.SetMxiError('MX-11000','event_pkg.GetForecastPeriods. ' || 'The end date must have a value');
         RETURN null;
...
*********************************************************************************
*
* Copyright 2007 Mxi Technologies.  All Rights Reserved.
* Any distribution of the MxI source code by any other party than
* Mxi Technologies is prohibited.
*
*********************************************************************************/
...
IS
   ...
   ld_DeadlineDate   evt_sched_dead.sched_dead_dt%TYPE;
   ln_DeadlineQt     evt_sched_dead.sched_dead_qt%TYPE;
   ld_ExtendedDate   evt_sched_dead.sched_dead_dt%TYPE;
...
BEGIN
    ...
    IF aLastSchedBool = 1 THEN
      ...
      IF lcur_LastDue%FOUND THEN
         ...
         ld_DeadlineDate := lrec_LastDue.sched_dead_dt;
         ln_DeadlineQt   := lrec_LastDue.sched_dead_qt;
         ld_ExtendedDate := lrec_LastDue.extended_dt;
      END IF;
      CLOSE lcur_LastDue;
    ELSE
      ...
      IF lcur_FirstDue%FOUND THEN
         ...
         ld_DeadlineDate := lrec_FirstDue.sched_dead_dt;
         ln_DeadlineQt   := lrec_FirstDue.sched_dead_qt;
         ld_ExtendedDate := lrec_FirstDue.extended_dt;
      END IF;
      CLOSE lcur_FirstDue;
    END IF;
    ...
    IF ln_DataTypeDbId IS NOT NULL THEN
     ...
     /* publish axon event */
     axon_event_publish_pkg.publish_task_deadl_resched_evt(
       aEventDbId,
       aEventId,
       ln_DataTypeDbId,
       ln_DataTypeId,
...
PROCEDURE SetPriority(
   ...
   lExtDeadlineDt       DATE;
   lNotificationDt      DATE;
   lv_ClassModeCd       ref_task_class.class_mode_cd%TYPE;
   ...
   CURSOR lcur_MaxChildPriority (
      ...
      SELECT
         MAX(ref_sched_priority.priority_ord) priority_no
      FROM
         evt_event
         INNER JOIN ref_sched_priority ON
            evt_event.sched_priority_db_id = ref_sched_priority.sched_priority_db_id AND
            evt_event.sched_priority_cd    = ref_sched_priority.sched_priority_cd
      WHERE
         evt_event.nh_event_db_id = al_NHEventDbId AND
         evt_event.nh_event_id    = al_NHEventId
         AND
   ...
   CURSOR lcur_EventDead (
      ...
      WHERE
         ...
         evt_sched_dead.event_id          = cl_EventId AND
         evt_sched_dead.sched_driver_bool = 1;
...
BEGIN
   ...
   /* Determine if the event is a task, with a soft deadline and retrieve the class mode code. */
   SELECT
   ...
   WHERE
      sched_stask.sched_db_id = al_EventDbId AND
      sched_stask.sched_id    = al_EventId
      AND
      sched_stask.rstat_cd    = 0;
   ...
   /* Job cards don't need to be evaluated - JICs are always priority 1 */
   IF lv_ClassModeCd <> 'JIC' THEN

      /* Part 1: Determine priority based on the event's own Deadlines */
      FOR lrec_EventDead IN lcur_EventDead (al_EventDbId, al_EventId) LOOP

         CalculateTaskPriority( lrec_EventDead.usage_rem_qt,
                       lrec_EventDead.notify_qt,
                       lrec_EventDead.deviation_qt,
                       lb_SoftDeadlineBool,
                       lrec_EventDead.sched_dead_dt,
                       lrec_EventDead.domain_type_cd,
                       lrec_EventDead.data_type_cd,
                       lrec_EventDead.ref_mult_qt,
                       ll_TempPriority,
                       ol_Return );
      ...
      /* Only continue if the maximum priority hasn't already been achieved */
      IF ll_MaxPriority < 4 THEN
   ...
   /* Use recursion to update parent's priority.  Skip for JICs as they don't affect their parent's priority.*/
   IF ll_NHEventDbId IS NOT NULL AND ll_NHEventId IS NOT NULL AND lv_ClassModeCd <> 'JIC'THEN
...
/**
*  Procedure to get the previous task given a current task.
*  [23Feb15] - JIRA Issue: OPER-2591
*/
PROCEDURE GetPreviousTask(
   an_SchedDbId     IN sched_stask.sched_db_id%TYPE,
   an_SchedId       IN sched_stask.sched_id%TYPE,
...
/**
*  Recalculates US deadlines for the given task and for all of its FORECAST tasks as well
*/
PROCEDURE UpdateUsageDeadlinesForTask (
   lcur_DepTasks lDepTaskTable,
   ...
   -- [23Feb15] - JIRA Issue: OPER-2591
   ln_PrevTaskDbId     sched_stask.sched_db_id%TYPE;
   ln_PrevTaskId       sched_stask.sched_id%TYPE;
...
BEGIN
   ...
   FOR lrec_USDeadlines IN lcur_USDeadlines (an_SchedDbId, an_SchedId) LOOP
      ...
      IF lrec_USDeadlines.Sched_From_Cd != 'CUSTOM' AND lrec_USDeadlines.Start_Dt IS NOT NULL THEN
         ...
         ELSE
            ...
            IF lrec_USDeadlines.Sched_From_Cd = 'WPSTART' OR
               ...
               lrec_USDeadlines.Sched_From_Cd = 'LASTDUE' OR
               lrec_USDeadlines.Sched_From_Cd = 'LASTEND'   THEN

               getPreviousTask( an_SchedDbId, an_SchedId, ln_PrevTaskDbId, ln_PrevTaskId );

               prep_deadline_pkg.GetRescheduleFromQtValues(
                 ln_PrevTaskDbId,
                 ln_PrevTaskId,
                 lrec_USDeadlines.data_type_db_id,
                 lrec_USDeadlines.data_type_id,
                 lrec_USDeadlines.Sched_From_Cd,
                 an_AcftDbId,
                 an_AcftId,
                 ld_StartDt,
                 lf_StartQt,
      ...
      SELECT
         COUNT(1) INTO ln_PreExecutionCount
      FROM
         evt_sched_dead
      WHERE
   ...
   FOR lrec_DepTasksIndex IN 1..lcur_DepTasks.COUNT LOOP
      /**
      ...
      lrec_DepTask := lcur_DepTasks(lrec_DepTasksIndex);
      -- process the US deadlines of the forecast task
      FOR lrec_USDeadlines IN lcur_USDeadlines (lrec_DepTask.Rel_Event_Db_Id, lrec_DepTask.Rel_Event_Id) LOOP
         ls_HashKey := lrec_DepTask.task_level ||':'|| lrec_USDeadlines.Data_Type_Db_Id || ':' || lrec_USDeadlines.Data_Type_Id;
         -- if the forecast task has the same US deadline as the previous task, start at the due qt of the previous.
         lf_StartQt := NULL;
         ld_BeginDate := NULL;
         FOR lf_PrevLevel IN REVERSE lf_RootLevel..(lrec_DepTask.task_level-1) LOOP
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ...
      IF lcur_USDeadlines%ISOPEN THEN CLOSE lcur_USDeadlines; END IF;
      RETURN;
END UpdateUsageDeadlinesForTask;

/**
*  Recalculates CA deadlines for the given task and for all of its FORECAST tasks as well
*/
PROCEDURE UpdateCalendarDeadlinesForTask (
   lcur_DepTasks lDepTaskTable,
   an_SchedDbId IN sched_stask.sched_db_id%TYPE,
   an_SchedId   IN sched_stask.sched_id%TYPE,
   on_Return    OUT NUMBER
...
BEGIN
   ...
   FOR lrec_DepTasksIndex IN 1..lcur_DepTasks.COUNT LOOP
       ...
       */
       ...
       lrec_DepTask := lcur_DepTasks(lrec_DepTasksIndex);
       -- process the CA deadine if there is one
      OPEN lcur_CADeadlines(lrec_DepTask.Rel_Event_Db_Id, lrec_DepTask.Rel_Event_Id);
      FETCH lcur_CADeadlines INTO lrec_CADeadlines ;
      IF lcur_CADeadlines%FOUND THEN
         ls_HashKey := lrec_DepTask.task_level ||':'|| lrec_CADeadlines.Data_Type_Db_Id || ':' || lrec_CADeadlines.Data_Type_Id;
         ld_StartDt := NULL;
         FOR lf_PrevLevel IN REVERSE lf_RootLevel..(lrec_DepTask.task_level-1) LOOP
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ...
      IF lcur_CADeadlines%ISOPEN THEN CLOSE lcur_CADeadlines; END IF;
      RETURN;
END UpdateCalendarDeadlinesForTask;
...
BEGIN
   ...
   -- make sure the provided task is actv
   SELECT
      evt_event.hist_bool
   INTO
      lb_HistoricalTask
   ...
   OPEN lcur_DepTasks;
   FETCH lcur_DepTasks BULK COLLECT INTO lRec_DepTasks;
   CLOSE lcur_DepTasks;

   UpdateCalendarDeadlinesForTask( lRec_DepTasks, an_SchedDbId, an_SchedId, on_Return );

   UpdateUsageDeadlinesForTask( lRec_DepTasks, an_AcftDbId, an_AcftId, an_SchedDbId, an_SchedId, an_Forecast, at_GetDt, at_GetQt, on_Return );
...
BEGIN
    ...
    IF av_DomainTypeCd = 'CA' THEN

       -- Calendar based deadline.

       -- The extended deadline is the scheduled deadline plus any deviation quantity.
       lExtDeadlineDt  := getExtendedDeadlineDt(
                             af_DeviationQt,
                             ad_SchedDeadDt,
                             av_DomainTypeCd,
                             av_DataTypeCd,
       ...
       -- The notification date is the scheduled deadline minus any notification quantity.
       lNotificationDt := getExtendedDeadlineDt(
                             (-1.0 * af_NotifyQt),
                             ad_SchedDeadDt,
                             av_DomainTypeCd,
                             av_DataTypeCd,
       ...
       IF ab_SoftDeadBool = 0 AND lExtDeadlineDt < SYSDATE THEN
          -- The extended deadline is in the past, so the task is overdue.
          an_Priority := 4;
       ELSIF af_NotifyQt >= 0 AND lNotificationDt > SYSDATE THEN
          -- The notification date is in the future, so the priority is LOW.
          an_Priority := 2;
       ELSE
          -- Otherwise, either the notification quantity is negative OR
          -- the notification date is in the past, so the priority is HIGH.
          an_Priority := 3;
       END IF;
...
EXCEPTION
   WHEN OTHERS THEN
      on_Return := -100;
      APPLICATION_OBJECT_PKG.SetMxIError('DEV-99999', 'event_pkg@@@CalculateTaskPriority@@@' || SQLERRM);
      RETURN;
END CalculateTaskPriority;
...
PROCEDURE SetDriverPriorityForTask(
   ...
   ld_SchedDeadDt    evt_sched_dead.sched_dead_dt%TYPE;
   ...
   lf_RefMultQt      ref_eng_unit.ref_mult_qt%TYPE;
   ln_TempPriority   NUMBER;
...
BEGIN
    ...
    IF ab_LastSchedBool = 1 THEN
      ...
      IF lcur_LastDue%FOUND THEN
         ...
         ld_SchedDeadDt  := lrec_LastDue.sched_dead_dt;
         lv_DomaintypeCd := lrec_LastDue.domain_type_cd;
         lv_DataTypeCd   := lrec_LastDue.data_type_cd;
         lf_RefMultQt    := lrec_LastDue.ref_mult_qt;
      END IF;
      CLOSE lcur_LastDue;
    ELSE
      ...
      IF lcur_FirstDue%FOUND THEN
         ...
         ld_SchedDeadDt  := lrec_FirstDue.sched_dead_dt;
         lv_DomaintypeCd := lrec_FirstDue.domain_type_cd;
         lv_DataTypeCd   := lrec_FirstDue.data_type_cd;
         lf_RefMultQt    := lrec_FirstDue.ref_mult_qt;
      END IF;
      CLOSE lcur_FirstDue;
    END IF;
    ...
    IF ln_DataTypeDbId IS NOT NULL THEN
       ...
       CalculateTaskPriority( lf_UsageRemQt,
                           ln_NotifyQt,
                           ln_DeviationQt,
                           ab_SoftDeadBool,
                           ld_SchedDeadDt,
...
PROCEDURE UpdateDeadlinesForAircraft(
   ...
   -- This is required in order to commit after each iteration of the loop
...
BEGIN
   ...
   FOR lrec_Tasks IN lcur_Tasks  LOOP
      ...
      CalculateTaskPriority( lf_UsageRem,
                          lrec_Tasks.notify_qt,
                          lrec_Tasks.deviation_qt,
                          lrec_Tasks.soft_deadline_bool,
                          lrec_Tasks.sched_dead_dt,
                          lrec_Tasks.domain_type_cd,
                          lrec_Tasks.data_type_cd,
                          lrec_Tasks.ref_mult_qt,
                          ln_TempPriority,
                          on_Return );
      IF on_Return < 0 THEN
          Application_Object_Pkg.appendmxierror('for task ' || ln_EventDbId || ':' || ln_EventId);
...
*
*  Required to work around the custom data types
*  passed in/out of UpdateDeadlinesForTask.
*/
PROCEDURE UpdateDeadlinesForTask (
   an_AcftDbId  IN inv_ac_reg.inv_no_db_id%TYPE,
   an_AcftId    IN inv_ac_reg.inv_no_id%TYPE,
   ...
   -- place holders for unused usage hashes
   at_QtAsDate hash_date_type;
   at_DateAsQt hash_float_type;

BEGIN

   UpdateDeadlinesForTask(
      an_AcftDbId,
      an_AcftId,
      an_SchedDbId,
      an_SchedId,
