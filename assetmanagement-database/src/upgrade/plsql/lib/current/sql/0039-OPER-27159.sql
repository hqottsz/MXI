...
--changeSet OPER-27159:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/********************************************************************************
*
...
BEGIN
   ...
   FOR lDeptTaskRec IN lDeptTask LOOP
      ...
      -- If the dependent task is not the calling task, check for multi inbound links
      IF NOT (lDeptTaskRec.dep_task_db_id = aTaskDbId AND lDeptTaskRec.dep_task_id = aTaskId) THEN 
         -- Look up the amount of inbound links leading to this dependent task
         -- ( Sum of CRT and POSTCRT links directed at this task)
         SELECT 
         ...
         (
            SELECT
               COUNT(1) link_count
            FROM -- POSTCRT Link count
               task_task forward_task
                  INNER JOIN task_task_dep backwards_link ON 
                              forward_task.task_db_id = backwards_link.task_db_id AND
                              forward_task.task_id    = backwards_link.task_id
                              AND
                              backwards_link.task_dep_action_db_id = 0 AND
                              backwards_link.task_dep_action_cd    = 'POSTCRT'
                  INNER JOIN task_task backward_task ON 
                              backwards_link.dep_task_defn_db_id = backward_task.task_defn_db_id AND
                              backwards_link.dep_task_defn_id    = backward_task.task_defn_id
            WHERE
                 forward_task.task_db_id = lDeptTaskRec.dep_task_db_id AND
                 forward_task.task_id    = lDeptTaskRec.dep_task_id
            UNION ALL
            SELECT
                COUNT(1) link_count
            FROM -- CRT Link count
               task_task forward_task
                  INNER JOIN task_task_dep backwards_link ON 
                             forward_task.task_defn_db_id = backwards_link.dep_task_defn_db_id AND
                             forward_task.task_defn_id    = backwards_link.dep_task_defn_id
                             AND NOT (
                                -- Do not include self referencing links (recurring task link)
                                backwards_link.task_db_id = forward_task.task_db_id AND
                                backwards_link.task_id    = forward_task.task_id
                             )
                             AND
                             backwards_link.task_dep_action_db_id = 0 AND
                             backwards_link.task_dep_action_cd    = 'CRT' 
                  INNER JOIN task_task backward_task ON
                             backwards_link.task_db_id = backward_task.task_db_id AND
                             backwards_link.task_id    = backward_task.task_id AND
                             -- only include the latest approved revisions
                             EXISTS (SELECT 
                                        NULL
                                     FROM 
                                        gtt_h_baseline_task
                                     WHERE  
                                        gtt_h_baseline_task.task_db_id = backward_task.task_db_id AND
                                        gtt_h_baseline_task.task_id    = backward_task.task_id)
            WHERE
                 forward_task.task_db_id = lDeptTaskRec.dep_task_db_id AND
                 forward_task.task_id    = lDeptTaskRec.dep_task_id 
         );
      ...
      IF lLinkCount > 1 THEN
         -- If the link count is greater than 1, skip this dependency and move on to the next one
         CONTINUE;
      END IF;
...
--changeSet OPER-27159:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
--comment Add possibly affected inventory to the baseline sync queue
DECLARE

    CURSOR c_affected_inventory IS
      WITH multi_link_tasks AS
      (
         SELECT DISTINCT
         ...
         JOIN (SELECT 
               ...
               FROM
                     (     
                        SELECT
                           COUNT(1) link_count,
                           backward_task.task_db_id,
                           backward_task.task_id
                        FROM -- POSTCRT Link count
                           task_task forward_task
                              INNER JOIN task_task_dep backwards_link ON 
                                          forward_task.task_db_id = backwards_link.task_db_id AND
                              ...
                              INNER JOIN task_task backward_task ON 
                                          ...
                                          AND NOT
                                          (
                                             backward_task.task_def_status_db_id = 0 AND
                                             backward_task.task_def_status_cd = 'OBSOLETE'
                                          )
                        GROUP BY backward_task.task_db_id,
                                 backward_task.task_id
                        ...
                        FROM -- CRT Link count
                           task_task forward_task
                              ...
                              INNER JOIN task_task backward_task ON
                                         backwards_link.task_db_id = backward_task.task_db_id AND
                                         backwards_link.task_id    = backward_task.task_id
                                         AND NOT
                                         (
                                             backward_task.task_def_status_db_id = 0 AND
                                             backward_task.task_def_status_cd = 'OBSOLETE'
               ...
               HAVING SUM(link_count) > 1) multi_link ON
                                          task_task.task_db_id = multi_link.task_db_id AND
                                          task_task.task_id    = multi_link.task_id
      )
      SELECT 
          inv_inv.inv_no_db_id,
          inv_inv.inv_no_id
      FROM
          inv_ac_reg
          -- only active aircraft
          JOIN inv_inv inv_ac ON inv_ac_reg.inv_no_db_id = inv_ac.inv_no_db_id AND
                                 inv_ac_reg.inv_no_id    = inv_ac.inv_no_id
                                 AND
                                 inv_ac.locked_bool = 0
                                 AND NOT (
                                    inv_ac.inv_cond_db_id = 0 AND
                                    inv_ac.inv_cond_cd    IN ('ARCHIVE','SCRAP')
                                 )
          -- only on wing inventory
          JOIN inv_inv ON
                 inv_inv.h_inv_no_db_id = inv_ac_reg.inv_no_db_id AND
                 inv_inv.h_inv_no_id    = inv_ac_reg.inv_no_id
                 AND
                 inv_inv.inv_class_db_id = 0 AND
                 inv_inv.inv_class_cd IN ('SYS','TRK')
          -- only config slot based tasks
          JOIN multi_link_tasks ON 
                     inv_inv.assmbl_db_id = multi_link_tasks.assmbl_db_id AND
                     inv_inv.assmbl_cd    = multi_link_tasks.assmbl_cd AND
                     inv_inv.assmbl_bom_id = multi_link_tasks.assmbl_bom_id;
             
   TYPE t_affected_inventory IS TABLE OF c_affected_inventory%ROWTYPE
   INDEX BY BINARY_INTEGER;
   l_affected_inventory t_affected_inventory;

BEGIN
 
   OPEN c_affected_inventory;
   FETCH c_affected_inventory BULK COLLECT INTO l_affected_inventory;
   CLOSE c_affected_inventory;
   
   FORALL i IN 1..l_affected_inventory.COUNT SAVE EXCEPTIONS
   
       INSERT INTO inv_sync_queue(inv_no_db_id,inv_no_id,queue_date)
       VALUES(l_affected_inventory(i).inv_no_db_id, l_affected_inventory(i).inv_no_id,SYSDATE);
