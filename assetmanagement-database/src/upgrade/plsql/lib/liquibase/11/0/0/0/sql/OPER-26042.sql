...
--changeset OPER-26042:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   upg_migr_schema_v1_pkg.table_drop('INV_CND_CHG_STAGE');
END;
/

--changeset OPER-26042:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   upg_migr_schema_v1_pkg.trigger_drop('TIBR_INV_CND_CHG_STAGE');
END;
/

--changeset OPER-26042:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   upg_migr_schema_v1_pkg.trigger_drop('TUBR_INV_CND_CHG_STAGE');
END;
/

--changeset OPER-26042:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   upg_migr_schema_v1_pkg.trigger_drop('TIBR_INV_CND_CHG_STAGE_ALT_ID');
END;
/

--changeset OPER-26042:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   upg_migr_schema_v1_pkg.trigger_drop('TIUDBR_INV_CND_CHG_STAGE');
END;
/

--changeSet OPER-26042:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   INNER JOIN (
      ...
      INNER JOIN inv_assmbl_list ON
         evt_inv.inv_no_db_id = inv_assmbl_list.inv_no_db_id AND
         evt_inv.inv_no_id    = inv_assmbl_list.inv_no_id
      WHERE
         evt_event.event_status_db_id = 0 AND
...
BEGIN
   ...
   SELECT DISTINCT
         inv_no_db_id,
         inv_no_id
   BULK COLLECT INTO v_ttb_inv_no_db_ids, v_ttb_inv_no_ids
   FROM (
      SELECT
      ...
      INNER JOIN
         ...
         ON sched_stask.main_inv_no_db_id = invlist.inv_no_db_id AND
      ...
      INNER JOIN inv_inv
         ON sched_stask.main_inv_no_db_id = inv_inv.inv_no_db_id AND
      ...
      WHERE
         ...
         (
            ...
            (
               ...
               evt_event.event_type_cd = 'TS' AND
               evt_event.event_status_db_id = 0 AND
               evt_event.event_status_cd IN ('ACTV','IN WORK','PAUSE') AND
   ...
   -- Filter out non-applicable TRK and SER inventory from aicraft tree prior to opening cursor
   SELECT
      StrStrStrStrTuple(
         inv_no_db_id,
         inv_no_id,
...
AS
      ...
      INNER JOIN evt_event
         ON
            ...
            evt_event.event_type_cd IN ('CF', 'FG', 'FL', 'TS', 'UC', 'UR')
