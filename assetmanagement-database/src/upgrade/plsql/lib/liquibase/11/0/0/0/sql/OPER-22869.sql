...
--changeSet OPER-22869:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY "MX_KEY_PKG" IS
  ...
  *********************************************************************************/
  ...
  BEGIN
    epoch := (sysdate - to_date('1970-01-01', 'YYYY-MM-DD')) *
             (24 * 60 * 60 * 1000) +
    ...
    calcCounter(epoch, l_counter, l_random);

    mostSigBits := calcMostSigBits(epoch, l_counter);
    leastSigBits := calcLeastSigBits(l_random);
    
    -- safety related to seed configuration
    if leastSigBits is null then
      return null;
    end if;
    
    return hextoraw(mostSigBits || leastSigBits);
  END;

END MX_KEY_PKG;
/

--changeSet OPER-22869:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
--preconditions onFail:MARK_RAN
--precondition-sql-check expectedResult:1 select count(1) from user_objects where object_name = 'TYPE1UUIDGENERATOR';
BEGIN
  EXECUTE IMMEDIATE 'DROP JAVA SOURCE TYPE1UUIDGENERATOR';
END;
/