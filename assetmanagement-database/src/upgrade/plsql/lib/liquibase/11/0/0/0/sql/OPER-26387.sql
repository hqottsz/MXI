...
--changeSet OPER-26387:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
   ...
   ***************************************************************************/
   ...
   IS
      ...
      FROM
         ...
         LEFT JOIN inv_cnd_chg_event ev ON
            ev.event_db_id = quar_quar.ac_event_db_id AND
            ev.event_id    = quar_quar.ac_event_id
         LEFT JOIN ref_stage_reason ON
            ref_stage_reason.stage_reason_db_id = ev.stage_reason_db_id AND
            ref_stage_reason.stage_reason_cd    = ev.stage_reason_cd
         LEFT JOIN po_header ON
            po_header.po_db_id = inv_inv.po_db_id AND
            po_header.po_id    = inv_inv.po_id
         LEFT JOIN org_hr ON
            ev.editor_hr_db_id = org_hr.hr_db_id AND
            ev.editor_hr_id    = org_hr.hr_id
      ...
      CURSOR lcur_quarantine_tag_deprecated IS
      SELECT
         evt_event.event_dt,
         utl_user.first_name,
         utl_user.last_name,
      ...
      lv_parm_value         VARCHAR2(5);
   BEGIN
       SELECT parm_value INTO lv_parm_value FROM utl_config_parm WHERE parm_name = 'IS_AC_EVT_CONVERSION_COMPLETE';
       IF UPPER(lv_parm_value) = 'TRUE' THEN
       ...
       ELSE
          ...
          OPEN lcur_quarantine_tag_deprecated;
          LOOP
             FETCH lcur_quarantine_tag_deprecated
             BULK COLLECT INTO lrec_quarantine_tag
             LIMIT gk_row_limit;
             ...
             END LOOP;
          END LOOP;
          CLOSE lcur_quarantine_tag_deprecated;       
       END IF;
