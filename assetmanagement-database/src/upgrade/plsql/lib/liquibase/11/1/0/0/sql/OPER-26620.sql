...
--changeSet OPER-26620:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY PREP_DEADLINE_PKG IS

   c_pkg_name CONSTANT VARCHAR2(30) := 'PREP_DEADLINE_PKG';
...
FUNCTION isUseSchedRule(
      an_TaskDbId       IN task_task.task_db_id%TYPE,
      an_TaskId         IN task_task.task_id%TYPE
   ) RETURN INTEGER
IS
   v_method_name VARCHAR2(30) := 'isUseSchedRule';

   vi_is_using_sched_rule INTEGER;
...
BEGIN
   ...
   SELECT
      NVL(task_task.use_sched_from_bool, 0)
   INTO
      vi_is_using_sched_rule
   FROM task_task
   WHERE
      task_task.task_db_id = an_TaskDbId AND
      task_task.task_id    = an_TaskId;

   RETURN vi_is_using_sched_rule;

EXCEPTION
   WHEN NO_DATA_FOUND THEN
      vi_is_using_sched_rule := 0;
      RETURN vi_is_using_sched_rule;
...
/********************************************************************************
*
* Function:    isSameTaskDefn
* Arguments:     an_SchedDbId     - Current dependency task primary key
*                an_SchedId
*                an_PrevSchedDbId - Previous task primary key
*                an_PrevSchedId
*                an_TaskDbId      - Task definition of the current task
*                an_TaskId
* Return:      0 task definition of two dept tasks are not the same
*              1 or greater, they are the same
*
* Description:  This function check a task and its dependency task have the same task definition or not
*
*********************************************************************************/
FUNCTION isSameTaskDefn(
...
IS
   v_method_name VARCHAR2(30) := 'isSameTaskDefn';

   ln_count                   NUMBER;
   ln_same_slot_or_part       NUMBER;
   ln_same_defn_or_chain      NUMBER;
   lv_prev_block_chain_sdesc  task_task.block_chain_sdesc%TYPE;
   lv_block_chain_sdesc       task_task.block_chain_sdesc%TYPE;

BEGIN

   v_step := 0;
   -- The previous task key is mandatory.  Without it, the tasks are always condidered not the same
   IF an_PrevSchedDbId IS NULL OR an_PrevSchedId IS NULL OR an_PrevSchedDbId = -1 OR an_PrevSchedId = -1 THEN
      RETURN 0;
   END IF;

   v_step := 10;
   -- If the task key is provided then compare it to the previous task definition
   IF an_TaskDbId IS NOT NULL AND an_TaskId IS NOT NULL THEN

      BEGIN
         -- Gather information on the previous and current task definitions
         SELECT
            CASE -- Determine if the tasks are on the same configuration slot or part number
               WHEN
                  ...
                  EXISTS
                     ...
                     FROM
                        task_part_map
                        INNER JOIN task_part_map prev_task_part_map ON
               ...
               THEN 1
               ELSE 0
            END AS same_slot_or_part,
            CASE -- Determine if the tasks are the same definition or chain
               WHEN
                  (-- Same definition
               ...
               THEN 1
               ELSE 0
            END AS same_defn_or_chain,
            prev_task.block_chain_sdesc AS prev_block_chain_sdesc,
            task_task.block_chain_sdesc
         INTO
            ln_same_slot_or_part,
            ln_same_defn_or_chain,
            lv_prev_block_chain_sdesc,
            lv_block_chain_sdesc
         FROM
            sched_stask prev_sched
            INNER JOIN task_task prev_task ON
               -- task definition of the previous task
               prev_task.task_db_id = prev_sched.task_db_id AND
               prev_task.task_id    = prev_sched.task_id
            CROSS JOIN task_task
         WHERE
            prev_sched.sched_db_id = an_PrevSchedDbId AND
      ...
      EXCEPTION
         -- If an invalid task or event key was provided then the tasks cannot be on the same slot.
         WHEN NO_DATA_FOUND THEN ln_same_slot_or_part := 0;
      END;

      -- Comparte the two task definitions
      IF ln_same_slot_or_part = 1 AND ln_same_defn_or_chain = 1 THEN
         -- If the tasks share the same slot/part and defn/chain then they are the same
         RETURN 1;
      ELSIF ln_same_slot_or_part = 1 AND lv_prev_block_chain_sdesc IS NOT NULL AND lv_block_chain_sdesc IS NOT NULL THEN

         -- Block chains require a more expensive check as the chain description may have changed over time
         SELECT
            COUNT(1)
         INTO
            ln_count
         ...
         WHERE
            EXISTS
               ...
               FROM
                  task_task all_prev
                  INNER JOIN task_task all_post ON
                     all_prev.task_defn_db_id = all_post.task_defn_db_id AND
                     all_prev.task_defn_id    = all_post.task_defn_id
               WHERE
                  all_post.block_chain_sdesc = lv_block_chain_sdesc
                  AND
                  all_prev.block_chain_sdesc = lv_prev_block_chain_sdesc
               );
   ...
   /*
    * Check if this task and the previous task have a DEPT (next-previous) link
    * in evt_event_rel, where the task defn is different. This can be created
    * in three ways:
    * 1. CRT links on task definitions
    * 2. Block chains
    * 3. BOM task transformation.
    * If it isn't #1 or #2 we will deduce that it is #3 and therefore the SAME
    * TASK DEFINITION
    */
   v_step := 20;
   IF an_SchedDbId IS NOT NULL AND an_SchedId IS NOT NULL THEN
      SELECT
         COUNT(*)
      INTO
         ln_count
      ...
      INNER JOIN   sched_stask
         ON
            sched_stask.sched_db_id = evt_event_rel.rel_event_db_id AND
            sched_stask.sched_id    = evt_event_rel.rel_event_id
      INNER JOIN   task_task
         ON
            task_task.task_db_Id = sched_stask.task_db_id AND
            task_task.task_id    = sched_stask.task_id
      INNER JOIN   sched_stask prev_sched
         ON
            prev_sched.sched_db_id = evt_event_rel.event_db_id AND
            prev_sched.sched_id    = evt_event_rel.event_id
      INNER JOIN   task_task prev_task
         ON
            prev_task.task_db_id = prev_sched.task_db_id AND
            prev_task.task_id    = prev_sched.task_id
      WHERE
         evt_event_rel.event_db_id = an_PrevSchedDbId AND
         evt_event_rel.event_id    = an_PrevSchedId
         AND
         evt_event_rel.rel_event_db_id = an_SchedDbId AND
         evt_event_rel.rel_event_id    = an_SchedId
         AND
         evt_event_rel.rel_type_db_id = 0 AND
         evt_event_rel.rel_type_cd    = 'DEPT'
         AND NOT
         (
            task_task.task_defn_db_id = prev_task.task_defn_db_id AND
            task_task.task_defn_id    = prev_task.task_defn_id
         )
         AND NOT EXISTS
         (
            SELECT 1
            FROM   task_task_dep
            WHERE
               task_task_dep.task_db_id = prev_task.task_db_id AND
               task_task_dep.task_id    = prev_task.task_id
               AND
   ...
   -- If the function got this far, the tasks definitions are not the same.
   RETURN 0;
...
BEGIN
   ...
   IF an_TaskDbId = an_OrigTaskDbId AND an_TaskId = an_OrigTaskId THEN
      /* If the task definition key is the same as the original task defininition key use the known information*/
      an_BaselineIntervalQt     := ln_OrigIntervalQt;
      ln_BaselineInitialQt      := ln_OrigInitialQt;
      an_BaselineNotifyQt       := ln_OrigNotifyQt;
      an_BaselineDeviationQt    := ln_OrigDeviationQt;
      an_BaselinePrefixedQt     := ln_OrigPrefixedQt;
      an_BaselinePostfixedQt    := ln_OrigPostfixedQt;
      ln_BaselineDeadlineExists := ln_OrigDeadlineExists;
   ELSE
      GetBaselineDeadline(
            an_DataTypeDbId ,
            an_DataTypeId ,
   ...
   IF an_RecurringBool = 1 AND ln_BaselineInitialQt IS NOT NULL THEN
      -- If statements are nested to avoid expensive execution of isSameTaskDefn
      IF an_PrevSchedId =-1 OR an_EventsOnSameInv = 0 THEN
         an_BaselineIntervalQt := ln_BaselineInitialQt;
      ELSIF isSameTaskDefn(an_PrevSchedDbId, an_PrevSchedId, an_SchedDbId, an_SchedId, an_TaskDbId, an_TaskId) = 0 THEN
         an_BaselineIntervalQt := ln_BaselineInitialQt;
      END IF;
   END IF;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      -- Unexpected error
      on_Return := icn_Error;
      APPLICATION_OBJECT_PKG.SetMxiError('DEV-99999','prep_deadline_pkg@@@GetValuesAndActionForSynch@@@'||SQLERRM || '--' || 'STEP:' || v_step || '--' || an_TaskDbId ||':' ||  an_TaskId || '--' || an_SchedDbId || ':' || an_SchedId || '--' || an_DataTypeDbId ||':' || an_DataTypeId);
      RETURN;
...
PROCEDURE UpdateDependentDeadlinesTree(
   ...
   CURSOR lcur_TaskTree (
      ...
      SELECT
         evt_event.event_db_id sched_db_id,
         evt_event.event_id    sched_id,
         nvl2(evt_sched_dead.event_Db_Id,1,0) AS has_deadline
      FROM
         evt_event
      ...
      WHERE evt_event.rstat_cd = 0 AND
            sched_stask.task_class_cd NOT IN ('CHECK', 'RO') AND
            sched_stask.wo_db_id IS NOT NULL AND sched_stask.wo_id IS NOT NULL
      START WITH
         evt_event.event_db_id = cn_StartSchedDbId AND
         evt_event.event_id    = cn_StartSchedId
      CONNECT BY
...
BEGIN
   ...
   LOOP
      ...
      IF lrec_TasksTree.has_deadline = 1 THEN
...
BEGIN
   ...
   ELSIF lb_HistBool = 1 THEN -- the previous task is COMPLETED
      ...
      CASE
       -- If the previous task is resched from "WPEND" or "WPSTART"
        WHEN (lv_ReschedFromCd = 'WPEND' OR lv_ReschedFromCd = 'WPSTART') AND
        -- And task due date and work package exists
        ld_DueDate IS NOT NULL AND ld_WoStartDt IS NOT NULL AND
        -- And due date is before work package start date which makes sure the task is overdue.
        -- Thus eliminating the need to check schedule to plan low against the usage remaining
        ld_DueDate < ld_WoStartDt AND
        -- And the schedule to plan high and schedule to plan low exist
        ln_PrefixedQt IS NOT NULL AND ln_PostfixedQt IS NOT NULL AND
        -- And the task completion date falls within the schedule to plan window
        -- which is guranteed to be after the due date because of the previous condition
        ABS(lrec_task.usage_rem_qt) <= ln_PostfixedQt THEN
              lv_ReschedFromCd := 'LASTDUE';
              ld_StartDt     := ld_DueDate;
...
PROCEDURE FindDeadlineStartDate (
   ...
   ln_SameTaskDefn            INTEGER;
   ln_UseSchedRule            INTEGER;
...
BEGIN
   ...
   v_step := 40;
   ln_SameTaskDefn := isSameTaskDefn(an_PrevSchedDbId, an_PrevSchedId, an_SchedDbId, an_SchedId, an_TaskDbId, an_TaskId);

   v_step := 50;
   ln_UseSchedRule := isUseSchedRule(an_TaskDbId, an_TaskId);

   v_step := 60;
   /* if this task is not a first task */
   IF NOT an_PrevSchedDbId =-1 AND (ln_SameTaskDefn > 0 OR ln_UseSchedRule = 0) THEN
      /* set reschedule_from */
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      -- Unexpected error
      on_Return := icn_Error;
      APPLICATION_OBJECT_PKG.SetMxiError('DEV-99999','prep_deadline_pkg@@@FindDeadlineStartDate@@@ '|| SQLERRM || '--' || 'STEP:' || v_step );
      RETURN;
...
BEGIN
   ...
   IF NOT an_PrevSchedId =-1 AND (ln_SameTaskDefn > 0 OR ln_UseSchedRule = 0) THEN
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      -- Unexpected error
      on_Return := icn_Error;
      APPLICATION_OBJECT_PKG.SetMxiError('DEV-99999','prep_deadline_pkg@@@FindDeadlineStartQt@@@: '|| SQLERRM || '--' || 'STEP:' || v_step );
      RETURN;
...
BEGIN
   ...
   IF abSyncWithBaseline AND ls_TaskClassCd <> 'CORR' AND ls_TaskClassCd <> 'REPL' THEN
      ...
      GetValuesAndActionForSynch(
         NVL(an_OrigTaskTaskDbId,ln_TaskDbId), -- If the original task wasn't provided use the task definition instead
         NVL(an_OrigTaskTaskId,ln_TaskId),
         ln_TaskDbId,
         ln_TaskId,
         ln_PrevSchedDbId,
         ln_PrevSchedId,
   ...
   IF ls_SchedFromCd = 'EFFECTIV' AND ln_SchedFromLatest = 1 THEN
      ...
      IF ld_ManufactDt IS NOT NULL THEN
...
BEGIN
   ...
   LOOP
      ...
      IF on_Return < 0 THEN
         RETURN;
      ELSE
         ln_DeadlineCount := ln_DeadlineCount + 1;         
      END IF;

   END LOOP;

   /* If deadlines exist or existed, update the deadlines for this Task */
   IF ln_DeadlineCount > 0 THEN
...
--changeSet OPER-26620:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
DECLARE
   CURSOR lcur_GetIncorrectDeadlineTasks IS
   SELECT
      sched_stask.sched_db_id,
      sched_stask.sched_id
   FROM   
   ...
   JOIN   evt_inv_usage ON 
      evt_event_rel.event_db_id = evt_inv_usage.event_db_id AND
      evt_event_rel.event_id    = evt_inv_usage.event_id
   JOIN evt_sched_dead ON 
      evt_event_rel.rel_event_db_id  = evt_sched_dead.event_db_id AND
      evt_event_rel.rel_event_id     = evt_sched_dead.event_id AND
      evt_sched_dead.data_type_db_id = evt_inv_usage.data_type_db_id AND
      evt_sched_dead.data_type_id    = evt_inv_usage.data_type_id
   JOIN sched_stask ON 
      evt_sched_dead.event_db_id = sched_stask.sched_db_id AND
      evt_sched_dead.event_id    = sched_stask.sched_id
   JOIN evt_event ON 
      ...
      sched_stask.sched_id    = evt_event.event_id
   JOIN sched_stask ON 
      evt_inv_usage.event_db_id = sched_stask.sched_db_id AND
      evt_inv_usage.event_id    = sched_stask.sched_id
   JOIN evt_event eep ON 
      sched_stask.sched_db_id = eep.event_db_id AND
      sched_stask.sched_id    = eep.event_id
   LEFT OUTER JOIN evt_event eewp ON 
      eep.h_event_db_id = eewp.event_db_id AND
      eep.h_event_id    = eewp.event_id
   LEFT OUTER JOIN sched_stask sswp ON 
      eewp.event_db_id   = sswp.sched_db_id AND
      eewp.event_id      = sswp.sched_id AND
      sswp.task_class_cd = 'CHECK'
   WHERE  evt_inv_usage.tsn_qt != evt_sched_dead.start_qt AND
       evt_sched_dead.sched_from_cd = 'LASTEND' AND
       evt_event.hist_bool = 0 AND
...
BEGIN
   FOR incorrect_deadline_tasks_rec IN lcur_GetIncorrectDeadlineTasks
   LOOP
      PREP_DEADLINE_PKG.UpdateDependentDeadlinesTree(
         incorrect_deadline_tasks_rec.sched_db_id, 
         incorrect_deadline_tasks_rec.sched_id,
         v_return
      );
   END LOOP;
END;
