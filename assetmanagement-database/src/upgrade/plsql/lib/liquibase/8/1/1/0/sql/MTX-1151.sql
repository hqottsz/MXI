...
--changeSet MTX-1151:1 stripComments:false
-- Add PREVENT_DEADLINE_SYNC column
-- This column will indicate that baseline sync should not sync this instance with the baseline
-- It will also allow users with permission to modify the deadline
ALTER TRIGGER TUBR_SCHED_STASK DISABLE;

--changeSet MTX-1151:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
utl_migr_schema_pkg.table_column_add('
Alter table SCHED_STASK add (
   PREVENT_DEADLINE_SYNC_BOOL Number(1,0) Check (PREVENT_DEADLINE_SYNC_BOOL IN (0, 1) ) DEFERRABLE
)
');
END;
/ 

--changeSet MTX-1151:3 stripComments:false
UPDATE sched_stask SET prevent_deadline_sync_bool = 0 WHERE prevent_deadline_sync_bool IS NULL; 

--changeSet MTX-1151:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
utl_migr_schema_pkg.table_column_modify('
Alter table SCHED_STASK modify (
   PREVENT_DEADLINE_SYNC_BOOL Number(1,0) DEFAULT 0 NOT NULL DEFERRABLE  Check (PREVENT_DEADLINE_SYNC_BOOL IN (0, 1) ) DEFERRABLE
)
');
END;
/

--changeSet MTX-1151:5 stripComments:false
ALTER TRIGGER TUBR_SCHED_STASK ENABLE;

--changeSet MTX-1151:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'ACTION_ENABLE_MANUAL_SCHEDULING', 
      'Permission to enable manual scheduling for tasks based on a task definition.',
      'TRUE/FALSE',
      'FALSE',
      1,
      'Maint - Tasks',
...
--changeSet MTX-1151:7 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'ACTION_RESET_TO_BASELINE_DEADLINE', 
      'Permission to reset deadline details back to those from the task definition.',
      'TRUE/FALSE',
      'FALSE',
      1,
      'Maint - Tasks',
...
--changeSet MTX-1151:8 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'API_PLANNING_ITEM_MANUAL_SCHEDULING', 
      'Permission to perform enable/reset manual scheduling API calls.',
      'TRUE/FALSE',
      'FALSE',
      1,
      'API - MAINTENANCE',
...
--changeSet MTX-1151:9 stripComments:false
