...
--changeSet 50.0.aopr_rbl_comp_rmvl_flt_q2_mv1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_RBL_COMP_RMVL_Q2_MV1');
END;
/

--changeSet 50.0.aopr_rbl_comp_rmvl_flt_q2_mv1:2 stripComments:false
CREATE MATERIALIZED VIEW aopr_rbl_comp_rmvl_q2_mv1
BUILD DEFERRED
...
AS
  (
      SELECT
         ...
         QPA,
         year_month,
         TO_NUMBER(TO_CHAR(ADD_MONTHS(to_date(year_month,'YYYYMM'),-3),'YYYYMM')) ym_4mon,
   ...
   (
      ...
      FROM 
         ...
         LEFT JOIN rvw_rmvl_mon ON
              ...
              rvw_rmvl_mon.year_month BETWEEN rvw_rmvl_unq.ym_6mon AND rvw_rmvl_unq.ym_4mon
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
   ...
   (
      ...
      FROM 
         ...
         LEFT JOIN aopr_monthly_operator_usage_v1 monthly_usage ON
              ...
              AND -- previous remonths
              TO_NUMBER(monthly_usage.year_code || monthly_usage.month_code) BETWEEN rvw_rmvl_unq.ym_6mon AND rvw_rmvl_unq.ym_4mon
      GROUP BY
         rvw_rmvl_unq.operator_id,
         rvw_rmvl_unq.fleet_type,
         rvw_rmvl_unq.config_slot,
...
AS 
   (
      SELECT
         rvw_rmvl_q2.operator_id,
         rvw_rmvl_q2.fleet_type,
         rvw_rmvl_q2.config_slot,   
         rvw_rmvl_q2.part_number,
         rvw_rmvl_q2.year_month,
         removal_count,
         unsched_removal,
         justified_failure,         
         cycles,
     ...
     FROM
        rvw_rmvl_q2
        INNER JOIN rvw_usage_q2 ON 
           rvw_rmvl_q2.operator_id  = rvw_usage_q2.operator_id AND
           rvw_rmvl_q2.fleet_type   = rvw_usage_q2.fleet_type AND
           rvw_rmvl_q2.config_slot  = rvw_usage_q2.config_slot AND
           rvw_rmvl_q2.part_number  = rvw_usage_q2.part_number AND     
           rvw_rmvl_q2.year_month   = rvw_usage_q2.year_month        
   )
SELECT
   operator_id,
   fleet_type,
   ...
   part_number,
   year_month,
   -- q2 - between 1st and 3rd month
   NVL(removal_count,0)      AS removal_q2,
   NVL(unsched_removal,0)    AS unsched_removal_q2,
   NVL(justified_failure,0)  AS justified_failure_q2,
   NVL(cycles,0)             AS total_cycle_q2,
   NVL(flight_hours,0)       AS total_hour_q2,
   NVL(mtbr,0)         AS mtbr_q2,
   NVL(mcbr,0)         AS mcbr_q2,
   NVL(mtbur,0)        AS mtbur_q2,
   NVL(mcbur,0)        AS mcbur_q2,
   NVL(mtbf,0)         AS mtbf_q2,
   NVL(mcbf,0)         AS mcbf_q2,
   NVL(failure_rate,0) AS f_rate_q2,   
   NVL(urr_hours,0)    AS urr_hour_q2,
   NVL(urr_cycles,0)   AS urr_cycle_q2
FROM 
   rvw_rmvl_q2_avg;