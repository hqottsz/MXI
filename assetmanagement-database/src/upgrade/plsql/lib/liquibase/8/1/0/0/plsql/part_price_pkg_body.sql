...
--changeSet part_price_pkg_body:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
/********************************************************************************
*
* Procedure: getStdUnitOfMeasure
* Arguments: aPartDbId IN (INTEGER) - Part Db Id
*            aPartId IN (INTEGER) - Part Id
*            on_QtyUnitOfMeasureDbId OUT (INTEGER)
*            on_QtyUnitOfMeasureCd OUT (STRING)
*
* Description: Retrieves the Standard Quantity Unit of Measure for the provided 
*              part key.
*
*
* Orig.Coder:     Stephane Levert
* Recent Coder:
* Recent Date:    Dec 17, 2013
*
*********************************************************************************
*
* Copyright 2000-2013 Mxi Technologies Ltd.  All Rights Reversed.
* Any distribution of the Mxi source code by any other party than
* Mxi Technologies Ltd is prohibited.
*
*********************************************************************************/
...
(
   ...
   on_QtyUnitOfMeasureDbId OUT ref_qty_unit.qty_unit_db_id%TYPE,
   on_QtyUnitOfMeasureCd OUT ref_qty_unit.qty_unit_cd%TYPE
) IS
BEGIN
   ...
   SELECT 
      ...
      eqp_part_no.qty_unit_cd
   INTO 
      on_QtyUnitOfMeasureDbId,
      on_QtyUnitOfMeasureCd
   FROM
      eqp_part_no
   WHERE
...
*            aUnitOfMeasureDbId   ref_qty_unit.qty_unit_db_id%TYPE - May be null.
*            aUnitOfMeasureCd     ref_qty_unit.qty_unit_cd%TYPE - May be null.
...
FUNCTION getLowestPartPrice(aVendorDbId eqp_part_vendor_price.vendor_db_id%TYPE,
                            ...
                            aPartDbId   eqp_part_vendor_price.part_no_db_id%TYPE,
                            aPartId     eqp_part_vendor_price.part_no_id%TYPE,
                            aUnitOfMeasureDbId   ref_qty_unit.qty_unit_db_id%TYPE,
                            aUnitOfMeasureCd     ref_qty_unit.qty_unit_cd%TYPE,
                            aQuantity   eqp_part_vendor_price.min_order_qt%TYPE )
  RETURN T_TAB_VENDOR_PRICE is

   lLowestPrice T_VENDOR_PRICE;
   lUnitOfMeasureDbId ref_qty_unit.qty_unit_db_id%TYPE;
   lUnitOfMeasureCd   ref_qty_unit.qty_unit_cd%TYPE;
BEGIN

  IF aUnitOfMeasureCd IS NULL THEN
     getStdUnitOfMeasure( aPartDbId, aPartId, lUnitOfMeasureDbId, lUnitOfMeasureCd );
  ELSE
     lUnitOfMeasureDbId := aUnitOfMeasureDbId;
     lUnitOfMeasureCd := aUnitOfMeasureCd;
  END IF;
  ...
  ELSE
    ...
    IF lLowestPrice IS NOT NULL THEN
       RETURN T_TAB_VENDOR_PRICE ( lLowestPrice );
    ELSE
       lLowestPrice := getVolumePartPrice( aVendorDbId, aVendorId, aPartDbId, aPartId, lUnitOfMeasureDbId, lUnitOfMeasureCd, aQuantity );
       IF lLowestPrice IS NOT NULL THEN
          RETURN T_TAB_VENDOR_PRICE ( lLowestPrice );
       END IF;
    END IF;
...
BEGIN
   ...
   FROM
      (SELECT
         MIN( price.standard_unit_price ) OVER (
            PARTITION BY
               price.part_no_db_id,
               price.part_no_id,
               price.vendor_db_id,
      ...
      FROM
         (SELECT
            ( vendor_price.unit_price / currency.exchg_qt ) / NVL(price_unit.qty_convert_qt, 1) AS standard_unit_price,
         ...
         FROM
            ...
            INNER JOIN ref_currency currency ON
               vendor_price.currency_db_id = currency.currency_db_id AND
               vendor_price.currency_cd    = currency.currency_cd
            LEFT OUTER JOIN eqp_part_alt_unit price_unit ON
               vendor_price.part_no_db_id = price_unit.part_no_db_id AND
               vendor_price.part_no_id    = price_unit.part_no_id
               AND
               vendor_price.qty_unit_db_id = price_unit.qty_unit_db_id AND
               vendor_price.qty_unit_cd    = price_unit.qty_unit_cd
         WHERE
            vendor_price.hist_bool = 0
            AND
            vendor_price.vendor_db_id = aVendorDbId AND
            vendor_price.vendor_id = aVendorId
            AND
            vendor_price.part_no_db_id = aPartDbId AND
            vendor_price.part_no_id = aPartId
            AND
            (
               vendor_price.effective_to_dt IS NULL
               OR
               vendor_price.effective_to_dt > SYSDATE
            )
            AND
            vendor_price.effective_from_dt <= SYSDATE
            AND
            vendor_price.contract_ref_sdesc IS NULL
            AND
            vendor_price.min_order_qt <= aQuantity
            AND
            vendor_price.qty_unit_db_id = aUnitOfMeasureDbId AND
            vendor_price.qty_unit_cd = aUnitOfMeasureCd
         ) price
      ) min_price
   WHERE
   ...
   (
      ...
      (
         ...
         FROM
            ...
            LEFT OUTER JOIN eqp_part_alt_unit price_unit ON
               vendor_price.part_no_db_id = price_unit.part_no_db_id AND
               vendor_price.part_no_id    = price_unit.part_no_id AND
               vendor_price.qty_unit_db_id = price_unit.qty_unit_db_id AND
               vendor_price.qty_unit_cd    = price_unit.qty_unit_cd
         WHERE
