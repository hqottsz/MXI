...
--changeSet OPER-835:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
PROCEDURE GetRescheduleFromQtValues(
   ...
   CURSOR lcur_task (
        ...
        SELECT
           ...
           root_event.hist_bool AS root_hist_bool,
           sched_stask.task_class_cd,
           root_evt_inv_usage.tsn_qt root_tsn,
        ...
        FROM
           ...
           evt_event root_event,
           sched_stask,
           evt_inv_usage root_evt_inv_usage,
           mim_data_type
        WHERE
           evt_event.event_db_id =  cn_EventDbId AND
           evt_event.event_id    =  cn_EventId   AND
           ...
           sched_stask.sched_id    = root_event.event_id
           AND
           root_evt_inv_usage.event_db_id     (+)= root_event.event_db_id AND 
           root_evt_inv_usage.event_id        (+)= root_event.event_id    AND
           root_evt_inv_usage.data_type_db_id (+)= cn_DataTypeDbId       AND
           root_evt_inv_usage.data_type_id    (+)= cn_DataTypeId
           AND
           mim_data_type.data_type_db_id (+)= root_evt_inv_usage.data_type_db_id AND 
           mim_data_type.data_type_id    (+)= root_evt_inv_usage.data_type_id 
        ;
   lrec_task lcur_task%ROWTYPE;
   ...
   ld_RootSchedEndDt   evt_event.sched_end_gdt%TYPE;
   ld_RootSchedStartDt evt_event.sched_start_gdt%TYPE;
   ln_RootTsn          evt_inv_usage.tsn_qt%TYPE;
