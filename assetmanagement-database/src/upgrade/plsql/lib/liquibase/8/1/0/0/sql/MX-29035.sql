...
--changeSet MX-29035:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   FOR lrec_EventDead IN lcur_EventDead (al_EventDbId, al_EventId) LOOP
      ...
      IF lrec_EventDead.domain_type_cd = 'CA' THEN
         ...
         IF lb_SoftDeadlineBool = 0 AND lExtDeadlineDt < SYSDATE THEN
            -- The extended deadline is in the past, so the task is overdue.
            ll_TempPriority := 4;
         ELSIF lrec_EventDead.notify_qt >= 0 AND lNotificationDt > SYSDATE THEN
