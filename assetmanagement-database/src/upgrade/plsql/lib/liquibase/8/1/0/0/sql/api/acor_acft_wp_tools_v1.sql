...
--changeSet acor_acft_wp_tools_v1:1 stripComments:false
CREATE OR REPLACE FORCE VIEW ACOR_ACFT_WP_TOOLS_V1
AS
...
FROM
   ...
   INNER JOIN evt_tool ON
      evt_tool.event_db_id = sched_labour_tool.event_db_id AND
      evt_tool.event_id    = sched_labour_tool.event_id AND
      evt_tool.tool_id     = sched_labour_tool.tool_id
   -- part group
...
UNION ALL
-- for tool requirements that have not been assigned off
SELECT
   wp_workscope.wo_sched_id,
   wp_workscope.sched_id,
   eqp_bom_part.bom_part_cd  AS part_group,
...
FROM
   ...
   INNER JOIN sched_stask ON
      wp_workscope.sched_id = sched_stask.alt_id
   INNER JOIN evt_tool ON
      evt_tool.event_db_id = sched_stask.sched_db_id AND
      evt_tool.event_id    = sched_stask.sched_id
   -- part group
   INNER JOIN eqp_bom_part ON
      evt_tool.bom_part_db_id = eqp_bom_part.bom_part_db_id AND
      evt_tool.bom_part_id    = eqp_bom_part.bom_part_id
