  ...
  ---------------------------------------------------------------------------------
   ...
   BEGIN
      ...
      (
         ...
         WHERE
            TRUNC(aircraft_usage.usage_date) BETWEEN aid_start_date AND aid_end_date AND
   ...
   BEGIN
      ...
      (
         ...
         WHERE
            TRUNC(flight.actual_departure_date) BETWEEN aid_start_date AND aid_end_date
      ) mx_flight
      ON (
             flight.flight_number            = mx_flight.flight_number            AND
             flight.actual_departure_date    = mx_flight.actual_departure_date    AND
   ...
   BEGIN
      ...
      (
          ...
          FROM
             ...
             LEFT JOIN acor_aircraft_fault_v1 fault ON
      ...
      (
         ...
         mx_flight_disruption.actual_departure_date    = flight_disruption.actual_departure_date    AND
      ...
      WHEN NOT MATCHED THEN
         ...
         (
            ...
            flight_disruption.actual_departure_date,
         ...
         (
            ...
            mx_flight_disruption.actual_departure_date,
   ...
   BEGIN
      ...
      (
          ...
          FROM
             ...
             INNER JOIN opr_calendar_month report_period ON
                cancels.actual_departure_date BETWEEN report_period.start_date AND report_period.end_date
             INNER JOIN opr_rbl_flight flight ON
                flight.flight_number            = cancels.flight_number     AND
                flight.departure_airport        = cancels.departure_airport AND
                flight.actual_departure_date    = cancels.actual_departure_date
             INNER JOIN opr_fleet_movement fleet_movement ON
                flight.serial_number = fleet_movement.serial_number AND
                (
                    cancels.actual_departure_date >= fleet_movement.phase_in_date   AND
                    cancels.actual_departure_date <= coalesce(fleet_movement.phase_out_date, cancels.actual_departure_date)
                )
          WHERE
   ...
   BEGIN
      ...
      (
         ...
         FROM
            ...
            INNER JOIN opr_calendar_month report_period ON
               flight_disruption.actual_departure_date BETWEEN report_period.start_date AND report_period.end_date
            INNER JOIN opr_fleet_movement fleet_movement ON
               fault.serial_number = fleet_movement.serial_number AND
               (
                  flight_disruption.actual_departure_date >= fleet_movement.phase_in_date   AND
                  flight_disruption.actual_departure_date <= coalesce(fleet_movement.phase_out_date, flight_disruption.actual_departure_date)
               )
         WHERE
            TRUNC(flight_disruption.actual_departure_date) BETWEEN aid_start_date and aid_end_date AND
...
--            monthly_delay.source_system      = mx_delay.source_system;
   ...
   BEGIN
      ...
      (
          ...
          WHERE
             TRUNC(fault_incident.found_on_date) BETWEEN aid_start_date and aid_end_date
          GROUP BY
             report_period.year_code,
             report_period.month_code,
             fleet_movement.fleet_type,
   ...
   BEGIN
      ...
      (
          ...
          FROM
             ...
             INNER JOIN opr_calendar_month report_period ON
               flight.actual_departure_date BETWEEN
                  report_period.start_date AND report_period.end_date
             INNER JOIN opr_fleet_movement fleet_movement ON
               flight.serial_number = fleet_movement.serial_number AND
               (
                   flight.actual_departure_date >= fleet_movement.phase_in_date   AND
                   flight.actual_departure_date <= coalesce(fleet_movement.phase_out_date, flight.actual_departure_date)
               )

          WHERE
             TRUNC(flight.actual_departure_date) BETWEEN aid_start_date and aid_end_date AND
             flight.flight_status in ('MXCMPLT')
          GROUP BY
             report_period.year_code,
             report_period.month_code,
             fleet_movement.fleet_type,
