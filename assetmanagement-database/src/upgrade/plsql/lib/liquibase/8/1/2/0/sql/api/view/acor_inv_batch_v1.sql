...
FROM
   ...
   -- highest inventory
   LEFT JOIN inv_inv h_inv_inv ON
   ...
   -- airport 
   -- Using left outer join to include the inventory that is IN TRANSIT.
   -- e.g. for a shipment, after sending an inventory from airport A to airport B, 
   -- and before airport B receives it, the inventory's location is not an airport,
   -- but "OPS". Since this inventory is available, we need to include it. If location
   -- code is "OPS", acor_location_v1.airport_alt_id is null, so using left outer join
   -- here to not be removed when join with inv_loc table.
   LEFT JOIN inv_loc airport_loc ON
