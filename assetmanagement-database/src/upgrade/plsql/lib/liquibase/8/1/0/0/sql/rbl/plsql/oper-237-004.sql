...
--changeSet oper-237-004:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY opr_report_period_pkg IS
   ...
   BEGIN
      ...
      l_start_date := last_day(add_months(nvl(p_start_date, SYSDATE),-1))+1;
      --
      --
      -- set the end of the period to the last day of the year
      l_end_date := last_day(nvl(p_end_date, SYSDATE));
      --
      --
      ...
      ON (
            generated_period.start_date = calendar_month.start_date AND
            generated_period.end_date   = calendar_month.end_date
         )
