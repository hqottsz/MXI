...
--changeSet acor_block_lastdone_nextdue_v1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('ACOR_BLOCK_LASTDONE_NEXTDUE_V1');
END;
/

--changeSet acor_block_lastdone_nextdue_v1:2 stripComments:false
CREATE MATERIALIZED VIEW acor_block_lastdone_nextdue_v1
BUILD DEFERRED
REFRESH COMPLETE
AS 
WITH
...
AS
  (
      ...
      WHERE
         ...
         ref_task_class.class_mode_cd = 'BLOCK'
         AND
