...
SELECT
   ...
   maint_delay_time_qt,
   ref_delay_code.tech_delay_bool        AS technical_delay_flag
FROM
   fl_leg
   INNER JOIN fl_leg_disrupt ON
      fl_leg.leg_id = fl_leg_disrupt.leg_id
   LEFT JOIN ref_delay_code ON 
      fl_leg_disrupt.delay_code_db_id = ref_delay_code.delay_code_db_id AND
      fl_leg_disrupt.delay_code_cd    = ref_delay_code.delay_code_cd
