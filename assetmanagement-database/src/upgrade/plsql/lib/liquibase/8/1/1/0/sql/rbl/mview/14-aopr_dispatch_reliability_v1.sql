...
--changeSet 14-aopr_dispatch_reliability_v1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('AOPR_DISPATCH_RELIABILITY_V1');
END;
/

--changeSet 14-aopr_dispatch_reliability_v1:2 stripComments:false
  ...
  FROM
    ...
    (
       ...
       (
          ...
          delay_time               AS '06:Delays Duration (Total Hours)',
...
SELECT
   tdrl.year_code,
   tdrl.month_code,
   tdrl.year_code || '-' ||  tdrl.month_code AS year_month,
   fleet_type,
   operator_code,
   start_date,
   trunc(end_date,'DD')    AS end_date,
...
FROM
   ...
   INNER JOIN
      opr_calendar_month calendar_month ON
         calendar_month.year_code  = tdrl.year_code AND
         calendar_month.month_code = tdrl.month_code;    