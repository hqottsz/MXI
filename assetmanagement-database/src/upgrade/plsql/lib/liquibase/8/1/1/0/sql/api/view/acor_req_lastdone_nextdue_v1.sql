...
--changeSet acor_req_lastdone_nextdue_v1:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.MATERIALIZED_VIEW_DROP('ACOR_REQ_LASTDONE_NEXTDUE_V1');
END;
/

--changeSet acor_req_lastdone_nextdue_v1:2 stripComments:false
...
AS
   (
     SELECT
         ...
         row_number() OVER (PARTITION BY rvw_req.inventory_id,rvw_req.task_defn_id ORDER BY rvw_req.event_dt DESC) rn
     FROM
        rvw_req
     WHERE
         rvw_req.hist_bool = 1
...
SELECT
   ...
   --
   rvw_last_done.sched_id last_sched_id,
   rvw_last_done.task_id  last_task_id,
   ...
   --
   rvw_next_due.sched_id next_sched_id,
   rvw_next_due.task_id next_task_id,
