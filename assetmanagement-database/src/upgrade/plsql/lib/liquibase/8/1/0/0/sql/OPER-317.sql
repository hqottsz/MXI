...
--changeSet OPER-317:1 stripComments:false
/*******Report Configuration 

/***************************************************************
* Create new menu for the report configuration page
****************************************************************/
...
--changeSet OPER-317:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/***************************************************************
* Create new action configuration parameters for create/edit/delete report configs
****************************************************************/
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'ACTION_CREATE_REPORT_CONFIGURATION', 
      'Permission to create report configurations.',
      'TRUE/FALSE', 	  
      'FALSE',  
      1, 
      'Common - Integration', 
...
--changeSet OPER-317:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'ACTION_DELETE_REPORT_CONFIGURATION', 
      'Permission to delete report configurations.',
      'TRUE/FALSE', 	  
      'FALSE',  
      1, 
      'Common - Integration', 
...
--changeSet OPER-317:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_data_pkg.action_parm_insert(
      'ACTION_EDIT_REPORT_CONFIGURATION', 
      'Permission to edit report configurations.',
      'TRUE/FALSE', 	  
      'FALSE',  
      1, 
      'Common - Integration', 
...
--changeSet OPER-317:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/***************************************************************
* Create new report configuration package
****************************************************************/
...
/* subtype declarations */
 ...
 ics_MenuViewExcel CONSTANT typn_ViewCode := 'MSExcel';
 ics_MenuViewWord  CONSTANT typn_ViewCode := 'MSWord';
...
--changeSet OPER-317:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   IF (lTemplatePos > 0) THEN
      ...
      --search for aViewMSExcel=true     
      ELSIF (INSTR(as_MenuURLLink, 'aViewMSExcel=true') > 0) THEN
         lReturn := ics_MenuViewExcel;
      --search for aViewMSWord=true     
      ELSIF (INSTR(as_MenuURLLink, 'aViewMSWord=true') > 0) THEN
   ...
   IF (lb_System = 0) THEN
      BEGIN
         ...
         SELECT menu_id         
           INTO ln_MenuId
           FROM utl_menu_item
          WHERE menu_link_url like (ics_MenuUrlPrefix || as_Report || chr(38) || '%' )
            AND rownum < 2;
         lb_CreateMenu := false;
      EXCEPTION
         WHEN OTHERS THEN
         ...
         INSERT INTO
            UTL_MENU_ITEM(
               ...
               SELECT
                  ...
                  NULL,
                  as_MenuName,
                  ics_MenuUrlPrefix || as_Report || chr(38) || 'aView' || as_MenuViewOutput || '=true',
                  ln_NewWindow,
                  0
                FROM
                  dual;
      ELSE
         UPDATE UTL_MENU_ITEM
            SET
                 ...
                 MENU_LINK_URL   = ics_MenuUrlPrefix || as_Report || chr(38) || 'aView' || as_MenuViewOutput || '=true',
                 NEW_WINDOW_BOOL = ln_NewWindow
            WHERE
                 MENU_LINK_URL like (ics_MenuUrlPrefix || as_Report || chr(38) || '%' );
      END IF;
...
BEGIN
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         ...
         APPLICATION_OBJECT_PKG.SetMxiError(icn_InvalidReportName,'report_config_pkg@@@deleteReportConfig@@@Report name '||as_Report||' was not found in the UTL_REPORT_TYPE table.');
      WHEN SYSTEM_REPORT_UNDELETABLE THEN
         -- invalid report name
         on_Return := icn_SystemReportReadonly;
         APPLICATION_OBJECT_PKG.SetMxiError(icn_SystemReportReadonly,'report_config_pkg@@@deleteReportConfig@@@Report '||as_Report||' is a system required report and cannot be deleted.');
      WHEN OTHERS THEN
         -- Unexpected error
         on_Return := icn_Error;
...
--changeSet OPER-317:7 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/***************************************************************
* Validate the menu sequence
****************************************************************/
BEGIN
   report_config_pkg.validateMenuSequence();
END;
/