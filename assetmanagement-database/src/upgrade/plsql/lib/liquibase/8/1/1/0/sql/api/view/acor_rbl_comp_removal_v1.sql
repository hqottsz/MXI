...
CREATE OR REPLACE FORCE VIEW acor_rbl_comp_removal_v1
AS 
SELECT
   ...
   END removal_type,
   CASE
      WHEN fault_event.event_status_cd = 'CFCERT' THEN
         1
      ELSE
         0
...
FROM
   ...
   INNER JOIN inv_inv inv_acft ON
      inv_inv.h_inv_no_db_id = inv_acft.inv_no_db_id AND
      inv_inv.h_inv_no_id    = inv_acft.inv_no_id
      AND
