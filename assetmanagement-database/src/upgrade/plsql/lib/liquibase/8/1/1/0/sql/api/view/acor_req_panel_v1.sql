...
--changeSet acor_req_panel_v1:1 stripComments:false
CREATE OR REPLACE FORCE VIEW acor_req_panel_v1
AS 
SELECT
   task.alt_id             AS task_id,
   eqp_task_panel.panel_cd AS panel_code
FROM
   (
      SELECT
         req_task.alt_id,
   ...
   INNER JOIN task_panel ON
      task.task_db_id = task_panel.task_db_id AND
      task.task_id    = task_panel.task_id
   INNER JOIN eqp_task_panel ON
      task_panel.panel_db_id = eqp_task_panel.panel_db_id AND
      task_panel.panel_id    = eqp_task_panel.panel_id
GROUP BY
   task.alt_id,
   eqp_task_panel.panel_cd;