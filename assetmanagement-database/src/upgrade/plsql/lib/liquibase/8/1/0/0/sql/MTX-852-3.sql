...
--changeSet MTX-852-3:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet MTX-852-3:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY "UTL_MIGR_SCHEMA_PKG" IS
   ...
   *---------------------------------------------------------*/
   ...
   BEGIN
      ...
      v_step := 10;
      -- Lookup the unique constraint for the given table and column
      SELECT
      ...
      WHERE
         upper(user_constraints.table_name) = upper(p_table_name)
         AND
         upper(user_cons_columns.column_name) = upper(p_column_name)
         AND
         user_constraints.constraint_type = 'U'
         -- Make sure there's only a single column in this constraint
         AND (
            SELECT
               COUNT(*)
            FROM
               user_cons_columns dup_cons
            WHERE
               dup_cons.constraint_name = user_constraints.constraint_name
         ) = 1;
      
      v_step := 20;
      -- Call the private procedure to drop the unique constraint
      drop_constraint(p_table_name, v_cons_name);

   EXCEPTION
      WHEN no_data_found THEN
         -- Do nothing, no constraint existed
         -- Inform the user that the constraint does not exist
         dbms_output.put_line('INFO: Column ' || p_column_name ||
                              ' on table ' || p_table_name ||
                              ' does not have a unique constraint, therefore, it was not dropped.');
      WHEN OTHERS THEN
         v_err_code    := SQLCODE;
         v_method_name := 'table_column_cons_unq_drop';
         v_err_msg     := substr(c_pkg_name || '.' || v_method_name ||
                                 ', STEP: ' || v_step || ', ERROR: ' || SQLERRM,
                                 1,
                                 2000);
   ...
   END table_column_cons_unq_drop;
