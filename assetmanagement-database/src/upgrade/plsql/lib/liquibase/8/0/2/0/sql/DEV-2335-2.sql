...
--changeSet DEV-2335-2:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   on_Return := icn_NoProc;
          ...
          SELECT DISTINCT
             FIRST_VALUE(maint_prgm.actv_dt) OVER (
                 PARTITION BY maint_prgm.maint_prgm_defn_db_id, maint_prgm.maint_prgm_defn_id, maint_prgm_carrier_map.carrier_db_id, maint_prgm_carrier_map.carrier_id
                 ORDER BY maint_prgm.revision_ord ASC
             ) AS actv_dt
          INTO 
             ad_MPActvDt
          FROM 
             task_task
             INNER JOIN maint_prgm_task ON maint_prgm_task.task_defn_db_id = task_task.task_defn_db_id AND
                                           maint_prgm_task.task_defn_id    = task_task.task_defn_id
             INNER JOIN maint_prgm ON maint_prgm.maint_prgm_db_id = maint_prgm_task.maint_prgm_db_id AND
                                      maint_prgm.maint_prgm_id    = maint_prgm_task.maint_prgm_id
             INNER JOIN maint_prgm_carrier_map ON maint_prgm_carrier_map.maint_prgm_db_id = maint_prgm.maint_prgm_db_id AND
