...
--changeSet MX-28104:1 stripComments:false
--
-- Changes for MX-28104
--
-- Updated vw_baseline_task, vw_h_baseline_task and sched_stask_pkg.GenForecastedTasks
--
--
-- 1. Change to vw_baseline_task
--
      ...
      WHERE
         ...
         (
            ...
            (
               ...
               maint_prgm.maint_prgm_status_cd = 'BUILD'
               AND
               maint_prgm_task.unassign_bool = 0
            )
         )
...
--changeSet MX-28104:2 stripComments:false
--
-- 2. Change to vw_h_baseline_task
--
...
--changeSet MX-28104:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
--
-- 3. Change to sched_stask_pkg.GenForecastedTasks
--
...
BEGIN
   ...
   LOOP
      ...
      LOOP
         ...
         /**
          * NO_DATA_FOUND can be caused by two problems:
          * The first is that the task is in a maintenance program that is in-build and applies to no approved
          * maintenance program. In this situation, we don't want to create new tasks. If we're creating the dependent
          * tasks however, we should create the next one so that when the maintenance program is approved, there will be
          * a task to base it's scheduled start dates.
          * The second scenario is when the task no longer applies to the inventory. This can occur in multiple scenarios,
          * such as part-based tasks, inventory locked etc. If that is the case, we want to return a success with a 'NO_DATA'
          * exit code.
          */
