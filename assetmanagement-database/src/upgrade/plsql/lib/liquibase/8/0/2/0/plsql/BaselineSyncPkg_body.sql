...
BEGIN
   ...
   FOR h in lBlockList.first .. lBlockList.last
   LOOP
      ...
      (
         ...
         FROM
            ...
            INNER JOIN TABLE(lInventoryList) inventory ON
               sched_stask.main_inv_no_db_id = inventory.db_id AND 
               sched_stask.main_inv_no_id    = inventory.id
         WHERE
            task_block_req_map.block_task_db_id = lBlockList(h).db_id AND
            task_block_req_map.block_task_id    = lBlockList(h).id
            AND
         ...
         FROM
            ...
            INNER JOIN TABLE(lInventoryList) inventory ON
               ...
               sched_stask.main_inv_no_id    = inventory.id
         WHERE
            sched_stask.task_db_id = lBlockList(h).db_id AND
            sched_stask.task_id    = lBlockList(h).id
            AND
            sched_stask.hist_bool_ro = 0
         UNION
         SELECT /*+ cardinality (blocks 10) */
         ...
         FROM
            ...
            INNER JOIN TABLE(lInventoryList) inventory ON
               ...
               sched_stask.main_inv_no_id    = inventory.id
         WHERE
            task_task.task_db_id = lBlockList(h).db_id AND
            task_task.task_id    = lBlockList(h).id
            AND
            sched_stask.hist_bool_ro = 0
      );
      ...
      BULK COLLECT INTO lZipId, lSchedDbId, lSchedId, lClassModeCd, lAssmblInvNoDbId, lAssmblInvNoId
      FROM
         TABLE(lSchedTaskList) tasks 
         INNER JOIN sched_stask ON
            tasks.db_id = sched_stask.sched_db_id AND
            tasks.id    = sched_stask.sched_id
         INNER JOIN ref_task_class ON
            sched_stask.task_class_db_id = ref_task_class.task_class_db_id AND
            sched_stask.task_class_cd    = ref_task_class.task_class_cd
         INNER JOIN inv_inv ON
      ...
      WHERE
         NOT EXISTS (
            ...
            FROM
               zip_task
            WHERE
               zip_task.zip_id = aZipId
               AND
               zip_task.sched_db_id = sched_stask.sched_db_id AND
               zip_task.sched_id    = sched_stask.sched_id
         );
