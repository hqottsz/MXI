...
--changeSet DEV-1885:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
PROCEDURE getTaskDetails(
            ...
            os_TaskClassModeCd     OUT ref_task_class.class_mode_cd%TYPE,
...
PROCEDURE getPTForReqTask(
  ...
  ls_TaskClassModeCd           ref_task_class.class_mode_cd%TYPE;
...
/********************************************************************************
*
* Procedure:    isCorrectiveTask
* Arguments:
*               as_TaskClassCd       Task Class Code
*               as_TaskClassModeCd   Task Class Mode Code
*
*
* Description:  Checks if this is a Corrective task
*
* Orig.Coder:   Stephane Levert
*
*********************************************************************************/
FUNCTION isCorrectiveTask(as_taskclasscd     IN sched_stask.task_class_cd%TYPE,
                          as_taskclassmodecd IN ref_task_class.class_mode_cd%TYPE)
  RETURN BOOLEAN IS
BEGIN
  return as_taskclassmodecd = 'REQ' AND as_taskclasscd = 'CORR';
END isCorrectiveTask;
   ...
   CURSOR cur_WPToCal(an_LrpDbId lrp_plan.lrp_db_id%Type,an_LrpId lrp_plan.lrp_id%Type) IS
         ...
         -- workscope flag overrides the task class workscope flag.
          ...
          WHERE -- Lookup all non-cancelled, non-suppressed subtasks of the check
            ...
            AND
            evt_event.event_status_cd NOT IN( 'CANCEL', 'COMPLETE', 'ERROR', 'TERMINATE' )
            AND
   ...
   FETCH cur_WPToCal BULK COLLECT INTO ltab_WPToCal;
      ...
      --For each workscope item within a package
       ...
       getTaskDetails(
        ...
        -- 2. Adhoc task
        IF ( ( ls_TaskClassModeCd = 'ADHOC' AND ls_TaskClassCd = 'ADHOC' ) OR
             ( isCorrectiveTask(ls_TaskClassModeCd,ls_TaskClassCd) AND israisedFromTask(ln_SchedDbId,ln_SchedId)=0 )) THEN
        ...
        ELSE IF ( isCorrectiveTask(ls_TaskClassModeCd,ls_TaskClassCd) AND israisedFromTask(ln_SchedDbId,ln_SchedId)>0 ) THEN
