...
--changeSet ALERT_PKG_body:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY "ALERT_PKG" IS
  ...
  PROCEDURE insert_alert(p_alert_type_id  IN utl_alert_type.alert_type_id%TYPE,
     ...
     BEGIN
        ...
        --check for duplicate alert if exists.
        v_alert_id := get_existing_alert(v_parm_hash,p_alert_type_id);
  ...
  THEN
     ...
     IF v_alert_id IS NULL
       THEN
           ...
           -- generate sequence id for alert.
    ...
    ELSE
      ...
      -- update exisitng row into  alert tables
        ...
        v_heighest_priority := get_highest_priority(v_alert_id,p_alert_priority);
    ...
    -- Enter a notification record for each user into UTL_USER_ALERT
     ...
     EXCEPTION
        WHEN OTHERS THEN
           -- Transaction will be rolled back should there be any errors
           ROLLBACK;
