...
--changeSet OPER-10140:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   WHERE
      ...
      (
          ...
          EXISTS
             (
               SELECT 1 FROM warranty_init_task 
                WHERE (sched_db_id,sched_id) IN
                      (SELECT event_db_id,event_id 
                       FROM evt_event 
                       START WITH event_db_id = an_SchedDbId AND
                                 event_id = an_SchedId
                       CONNECT BY nh_event_db_id = PRIOR event_db_id AND
                                  nh_event_id = PRIOR event_id
                       )
                AND   warranty_init_db_id = an_WtyInitDbId
                AND   warranty_init_id = an_WtyInitId
              ) 
           )
         OR
         (
