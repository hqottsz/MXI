...
--changeSet OPER-3345:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_schema_pkg.table_column_add('
      Alter table SCHED_STASK add (
         "WO_DB_ID" Number(10,0) Check (WO_DB_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
      )
   ');
END;
/

--changeSet OPER-3345:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_schema_pkg.table_column_add('
      Alter table SCHED_STASK add (
         "WO_ID" Number(10,0) Check (WO_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
      )
   ');
END;
/

--changeSet OPER-3345:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
...
--changeSet OPER-3345:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_schema_pkg.index_create('
      Create Index IX_FK_WO_SCHEDSTASK ON SCHED_STASK (WO_DB_ID,WO_ID)
   ');
END;
/

--changeSet OPER-3345:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet OPER-3345:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
  ...
  BaselineDeadlinesChanged(
   ...
   IF ln_BaselineDeadlineChanged = 1 THEN
      ...
      IF ln_OrigDeadlineExists = 1 THEN
         ...
         END IF;
      END IF;
   ELSIF ( an_BaselineIntervalQt <> ln_ActualIntervalQt ) OR ( an_BaselineNotifyQt <> ln_ActualNotifyQt ) THEN
      an_UpdateActualDeadline := 1;
      as_sched_from_cd        := ls_ActualSchedFromCd;
      ad_start_dt             := ld_ActualStartDt;
      an_start_qt             := ln_ActualStartQt;
...
BEGIN
   ...
   IF lb_HistBool = 0 AND ld_DueDate IS NOT NULL THEN
      IF an_ServiceCheck = 0 THEN
        ...
        --
        IF ln_PostfixedQt >= ln_Deviation THEN
      ...
      ELSE
         ...
         ELSIF ad_EffectiveDt IS NOT NULL THEN
            ...
            IF ( lschedfromEffectiveDt = 1 ) THEN
               ...
               /* Effective date in the future or current. */
               ELSE
                  GetCurrentInventoryUsage(
                        an_SchedDbId,
                        an_SchedId,
...
BEGIN
   ...
   IF
      ...
      (
         ...
         ad_PreviousCompletionDt IS NOT NULL
      )
   THEN
