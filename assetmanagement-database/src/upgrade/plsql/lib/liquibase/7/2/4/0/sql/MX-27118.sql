...
--changeSet MX-27118:1 stripComments:false
-- update precision of CWK, CMON, CLMON
UPDATE
   mim_data_type
SET
   entry_prec_qt = 2
WHERE
   data_type_db_id = 0 AND
   data_type_id IN (22, 23, 25)
   AND
   entry_prec_qt = 0 -- ensure it has not been updated by the customer
   ;   

--changeSet MX-27118:2 stripComments:false
-- update precision of CYR
UPDATE
   mim_data_type
SET
   entry_prec_qt = 3
...
--changeSet MX-27118:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- getDeadlineDt
/** 
...
--changeSet MX-27118:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   /* if the data type is month then use months to calculate new deadline (not days) */
   ELSIF aDataTypeCd = 'CLMON' THEN
      lExtDeadlineDt :=  ADD_MONTHS( LAST_DAY(aSchedDt), aDeltaQt ) + TRUNC((aDeltaQt - TRUNC(aDeltaQt)) * aMultiplier);
      
   /* if the data type is year then use 12 months to calculate new deadline (not days) */
   ELSIF aDataTypeCd = 'CYR' THEN
      lExtDeadlineDt := ADD_MONTHS( aSchedDt, aDeltaQt*12 ) + TRUNC((aDeltaQt - TRUNC(aDeltaQt * 12)/12) * aMultiplier);
...
--changeSet MX-27118:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- prep_deadline_pkg_body
...
--changeSet MX-27118:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- event_pkg_body
