...
--changeSet MX-17107:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   FOR lrec_Tasks IN lcur_Tasks  LOOP
       ...
       -- only fire this logic on non-forecast tasks on the specified inventory
       IF lrec_Tasks.recalc_task = 1 THEN

          -- 1. Update all CA and US deadlines for the task with deadlines
          UpdateDeadlinesForTask(an_AircraftDbId, an_AircraftId, lrec_Tasks.Event_Db_Id, lrec_Tasks.Event_Id, 0, at_QtAsDate, at_DateAsQt, on_Return);
          IF on_Return<0 THEN 
             Application_Object_Pkg.appendmxierror('for task ' || ln_EventDbId || ':' || ln_EventId); 
             RETURN; 
          END IF;
...
BEGIN
   ...
   ELSIF ln_IsLoose = 1 THEN
      UpdateDeadlinesForLooseInv(ln_AircraftDbId, ln_AircraftId, an_InvNoDbId, an_InvNoId, on_Return);
   ELSE
      UpdateDeadlinesForAttachedInv(ln_AircraftDbId, ln_AircraftId, an_InvNoDbId, an_InvNoId, on_Return);
   END IF;
