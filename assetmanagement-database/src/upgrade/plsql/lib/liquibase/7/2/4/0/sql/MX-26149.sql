...
--changeSet MX-26149:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet MX-26149:2 stripComments:false
...
SELECT
   ...
   -- flight locations
   current_flight.departure_loc_db_id AS dep_loc_db_id,
   current_flight.departure_loc_id AS dep_loc_id,
   current_flight.departure_loc_cd AS dep_loc_cd,
   current_flight.arrival_loc_db_id AS arr_loc_db_id,
   current_flight.arrival_loc_id AS arr_loc_id,
   current_flight.arrival_loc_cd AS arr_loc_cd,
   -- sorting info
...
FROM
   ...
   (  SELECT DISTINCT
         ...
         dep_inv_loc.loc_cd AS departure_loc_cd,
         fl_leg.arrival_loc_db_id,
         fl_leg.arrival_loc_id,
         arr_inv_loc.loc_cd AS arrival_loc_cd,
         FIRST_VALUE( fl_leg.leg_id )
            OVER ( 
                 PARTITION BY 
                    ...
                    fl_leg.aircraft_id 
                 ORDER BY 
                    fl_leg.actual_departure_dt 
            ) AS leg_id         
      FROM
         fl_leg
         INNER JOIN inv_loc dep_inv_loc ON 
               dep_inv_loc.loc_db_id = fl_leg.departure_loc_db_id AND
               dep_inv_loc.loc_id    = fl_leg.departure_loc_id  
         INNER JOIN inv_loc arr_inv_loc ON 
               arr_inv_loc.loc_db_id = fl_leg.arrival_loc_db_id AND
               arr_inv_loc.loc_id    = fl_leg.arrival_loc_id
      WHERE
         fl_leg.flight_leg_status_cd IN ('MXOUT', 'MXOFF', 'MXON', 'MXIN', 'MXDIVERT') AND
         fl_leg.hist_bool = 0
   ) current_flight ON 
     current_flight.aircraft_db_id  = inv_inv.inv_no_db_id AND
     current_flight.aircraft_id     = inv_inv.inv_no_id
WHERE
