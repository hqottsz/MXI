...
--changeSet MX-18521:1 stripComments:false
   -- usage deadlines on loose components should never have a sched_dead_dt
   UPDATE   evt_sched_dead d
   SET      d.sched_dead_dt =  NULL
   WHERE  ( d.event_db_id, d.event_id, d.data_type_db_id, d.data_type_id )
          IN
          ( SELECT evt_sched_dead.event_db_id,
            ...
            FROM   evt_sched_dead,
                   ...
                   sched_stask,
                   mim_data_type
            WHERE  root_inv.nh_inv_no_id IS NULL   AND
                   root_inv.inv_class_cd <> 'ACFT'
                   AND
                   inv_inv.h_inv_no_db_id = root_inv.inv_no_db_id AND
                   inv_inv.h_inv_no_id    = root_inv.inv_no_id    AND
                   inv_inv.rstat_cd       = 0  
                   AND
...
--changeSet MX-18521:2 stripComments:false
   -- the start quantity should also never be estimated
   UPDATE   evt_sched_dead d
   SET      d.start_qt      = ( SELECT NVL(inv_curr_usage.tsn_qt, 0)
                                FROM   sched_stask,
                                       inv_curr_usage
                                WHERE  sched_stask.sched_db_id = d.event_db_id  AND
                                       sched_stask.sched_id    = d.event_id
                                       AND
                                       inv_curr_usage.data_type_db_id = d.data_type_db_id  AND
                                       inv_curr_usage.data_type_id    = d.data_type_id     AND
                                       inv_curr_usage.inv_no_db_id    = sched_stask.main_inv_no_db_id AND
                                       inv_curr_usage.inv_no_id       = sched_stask.main_inv_no_id
                               )
   WHERE  ( d.event_db_id, d.event_id, d.data_type_db_id, d.data_type_id )
          IN
          ( SELECT evt_sched_dead.event_db_id,
            ...
            WHERE  root_inv.nh_inv_no_id  IS NULL  AND
                   ...
                   evt_sched_dead.event_db_id   = evt_event.event_db_id AND
                   evt_sched_dead.event_id      = evt_event.event_id   	AND
                   evt_sched_dead.sched_from_cd = 'EFFECTIV'            AND
                   evt_sched_dead.start_dt      IS NOT NULL             AND
                   evt_sched_dead.start_dt      > SYSDATE
                   AND
                   mim_data_type.data_type_db_id   = evt_sched_dead.data_type_db_id AND
                   mim_data_type.data_type_id      = evt_sched_dead.data_type_id    AND
                   mim_data_type.domain_type_db_id = 0                              AND
                   ...
                   NOT EXISTS ( SELECT 1 
                                FROM   inv_curr_usage
                                WHERE  inv_curr_usage.inv_no_db_id    = sched_stask.main_inv_no_db_id  AND
                                       inv_curr_usage.inv_no_id       = sched_stask.main_inv_no_id     AND
                                       inv_curr_usage.data_type_db_id = evt_sched_dead.data_type_db_id AND
                                       inv_curr_usage.data_type_id    = evt_sched_dead.data_type_id    AND
                                       inv_curr_usage.tsn_qt          = evt_sched_dead.start_qt
                              )                   
          );                    

--changeSet MX-18521:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
