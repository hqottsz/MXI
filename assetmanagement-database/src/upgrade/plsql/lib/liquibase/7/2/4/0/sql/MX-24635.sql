...
--changeSet MX-24635:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
utl_migr_schema_pkg.index_create('
Create Index "IX_UTLCONFIGPARM_UTLUSERPARM" ON "UTL_USER_PARM" ("PARM_NAME","PARM_TYPE")
');
END;
/

--changeSet MX-24635:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
utl_migr_schema_pkg.index_create('
Create Index "IX_UTLUSER_UTLUSERPARM" ON "UTL_USER_PARM" ("USER_ID")
');
END;
/

--changeSet MX-24635:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
        ...
        ELSE
         ...
         /* if the task is scheduled from install date */
          ...
          ELSIF (ab_RelativeBool = 0 ) THEN
              /* if scheduled from receive date and received date is in the past */
              IF an_FromRcvdDt=1 AND ld_ReceivedDt IS NOT NULL AND ld_ReceivedDt < SYSDATE THEN
               ...
               ELSIF an_FromRcvdDt=0  THEN
                   ...
                   av_SchedFromCd := 'BIRTH';
                   an_StartQt     := 0;
               /* scheduled from receive date but received date is null (or it is in future) */
               ELSE
                   av_SchedFromCd := 'BIRTH';
                   an_StartQt     := 0;
               END IF;
...
--changeSet MX-24635:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
