...
--changeSet MX-20745:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_schema_pkg.table_column_add('
      Alter table TASK_TASK add (
         "SCHED_FROM_LATEST_BOOL" Number(1,0) Check (SCHED_FROM_LATEST_BOOL IN (0, 1) ) DEFERRABLE
      )
   ');
END;
/

--changeSet MX-20745:2 stripComments:false
UPDATE task_task
SET sched_from_latest_bool = 1
WHERE relative_bool = 1 AND sched_from_latest_bool IS NULL;

--changeSet MX-20745:3 stripComments:false
UPDATE task_task
SET sched_from_latest_bool = 0
WHERE sched_from_latest_bool IS NULL;

--changeSet MX-20745:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   utl_migr_schema_pkg.table_column_modify('
      Alter table TASK_TASK modify (
         "SCHED_FROM_LATEST_BOOL" Number(1,0) DEFAULT 0 NOT NULL DEFERRABLE 
      )
   ');
END;
/

--changeSet MX-20745:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
