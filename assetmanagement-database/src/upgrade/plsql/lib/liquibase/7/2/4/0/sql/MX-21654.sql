...
--changeSet MX-21654:1 stripComments:false
-- Clear out start_dt values for custom usage deadlines
UPDATE
   evt_sched_dead
SET
   evt_sched_dead.start_dt = NULL
WHERE
   ( evt_sched_dead.event_db_id, evt_sched_dead.event_id, evt_sched_dead.data_type_db_id, evt_sched_dead.data_type_id ) IN
   (
      SELECT
         evt_sched_dead_filter.event_db_id,
         evt_sched_dead_filter.event_id,
         evt_sched_dead_filter.data_type_db_id,
         evt_sched_dead_filter.data_type_id
      FROM
         evt_sched_dead evt_sched_dead_filter
         JOIN mim_data_type ON
            mim_data_type.data_type_db_id = evt_sched_dead_filter.data_type_db_id AND
            mim_data_type.data_type_id    = evt_sched_dead_filter.data_type_id
            AND
            mim_data_type.domain_type_cd = 'US'
      WHERE
         evt_sched_dead_filter.start_dt is NOT NULL
         AND
         evt_sched_dead_filter.sched_from_cd = 'CUSTOM'
   );

--changeSet MX-21654:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   UPDATE req_part
    ...
    WHERE req_part_db_id   = al_EventDbId
      ...
      AND
	  (
        (req_by_dt IS NOT NULL AND ad_NeededByDate IS NULL)
         OR
        req_by_dt <> ad_NeededByDate
         OR
        (req_by_dt IS NULL AND ad_NeededByDate IS NOT NULL)
      );
...
*                 and also update task's parents part request Needed By Date
*
* Orig.Coder:     Michal Bajer
* Recent Coder:   N/A
* Recent Date:    October 1, 2005
...
PROCEDURE SyncPRRequiredByDt(
   ...
   /* the original needed by date from the part request */
   ld_OrigReqByDt Date;

   /* flag indicating the task has part requests */
   lb_PartRequestFlag NUMBER;
  ...
  -- get all task under given task including the task itself and all task above this given task
      ...
      SELECT event_db_id,
       ...
       WHERE rstat_cd = 0
             ...
             CONNECT BY
                    ...
                    nh_event_id   =   PRIOR event_id                    
       UNION ALL          
      SELECT nh_event_db_id,
             nh_event_id
        FROM evt_event 
       WHERE rstat_cd = 0 
             AND 
			 nh_event_db_id IS NOT NULL
             START WITH
                    event_db_id = cl_SchedDbId AND
                    event_id    = cl_SchedId
             CONNECT BY
...
BEGIN
   -- Initialize the return value
     ...
     FOR lrec_GetTaskTree IN lcur_GetTaskTree(al_EventDbId, al_EventId) LOOP
      ...
      -- reset/initialize the original date to null and flag back to 0
      ld_OrigReqByDt := NULL;
      lb_PartRequestFlag := 0;
       ...
       FOR lrec_GetPartRequestsForTask IN lcur_GetPartRequestsForTask(lrec_GetTaskTree.event_db_id, lrec_GetTaskTree.event_id) LOOP
         ...
         IF ld_OrigReqByDt IS NULL THEN
          
            ld_OrigReqByDt := lrec_GetPartRequestsForTask.req_by_dt;
            lb_PartRequestFlag := 1;
          
         END IF;
       -- update required by date of the part request to due date of the task
         SetPRRequiredByDate (
               lrec_GetPartRequestsForTask.req_part_db_id,
      ...
      IF lb_PartRequestFlag = 1 AND ld_OrigReqByDt <> ld_DueDate THEN
         -- call trigger to queue a new req update row
         TRG_MX_QUEUE_REQ_UPDATE(
               lrec_GetTaskTree.event_db_id, 
               lrec_GetTaskTree.event_id,
               ld_OrigReqByDt,
               ld_DueDate,
               ol_Return
            );
...
EXCEPTION
   WHEN OTHERS THEN
      on_Return := -100;
      IF lcur_PartRequests%ISOPEN THEN CLOSE lcur_PartRequests; END IF;
