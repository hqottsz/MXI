...
--changeSet DEV-418:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet DEV-418:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- Migration script for 7.2 Quantas CR's.
-- Author: Rangarajan Sundararajan
-- Date  : April 28, 2010
-- Brief Description:  
-- For all non historic REPL-tasks,
-- Copy the task priority from the driving task to the REPL.
DECLARE
-- Get the list of non-historic REPL tasks in the system.
CURSOR lcur_repltasks IS
  SELECT 
  ...
  ref_task_class.task_class_cd = 'REPL' AND
  evt_event.hist_bool = 0 AND
  sched_stask.rstat_cd = 0;
--  Variable to hold a single row of lcur_repltasks
lrec_repltask lcur_repltasks%ROWTYPE;
-- Variable to hold the task priority id of driving task of REPL task.
ln_DrvTaskPriorityDbId ref_task_priority.task_priority_db_id%TYPE;
-- Variable to hold the task priority cd of driving task of REPL task.
ln_DrvTaskPriorityCd ref_task_priority.task_priority_cd%TYPE;
-- Temporary counter variable.
ln_Count INTEGER;
ln_TaskPriorityCount INTEGER;

BEGIN

-- Code to copy the task priority from the driving task to the REPL.

FOR lrec_repltask IN lcur_repltasks

LOOP
    -- Check if a driver task is present and the driver of REPL task is curently
    -- pointing to itself.
    SELECT COUNT(*) 
    INTO ln_Count
    FROM sched_stask driver_task
    WHERE
    (driver_task.sched_db_id,driver_task.sched_id)
    IN                 
    (SELECT wp_evt_event_rel.rel_event_db_id, wp_evt_event_rel.rel_event_id  
    FROM 
    evt_event_rel repl_evt_event_rel 
    INNER JOIN evt_event_rel wp_evt_event_rel ON
    repl_evt_event_rel.rel_event_db_id = wp_evt_event_rel.event_db_id AND
    repl_evt_event_rel.rel_event_id = wp_evt_event_rel.event_id
    INNER JOIN evt_event_rel repl_cur_driver ON
    repl_cur_driver.event_db_id = repl_evt_event_rel.event_db_id AND
    repl_cur_driver.event_id = repl_evt_event_rel.event_id        
    WHERE 
    repl_evt_event_rel.event_db_id = lrec_repltask.sched_db_id AND
    repl_evt_event_rel.event_id = lrec_repltask.sched_id AND
    repl_evt_event_rel.rel_type_cd = 'WORMVL' AND
    wp_evt_event_rel.rel_type_cd = 'DRVTASK'  AND
    repl_cur_driver.rel_type_cd = 'DRVTASK' AND
    repl_cur_driver.event_db_id = repl_cur_driver.rel_event_db_id AND
    repl_cur_driver.event_id = repl_cur_driver.rel_event_id    
    );
   
    -- Check if a driving task exists for the REPL task.
    IF ln_Count > 0 THEN        
         
        -- Check if a task priority exists for the driving task.
        SELECT 
        COUNT(*)
        INTO 
        ln_TaskPriorityCount
        FROM
        sched_stask driver_task    
        WHERE
        (driver_task.sched_db_id, driver_task.sched_id)
        IN 
        (SELECT 
        ...
        FROM evt_event_rel repl_evt_event_rel 
        INNER JOIN evt_event_rel wp_evt_event_rel ON
        repl_evt_event_rel.rel_event_db_id = wp_evt_event_rel.event_db_id AND
        repl_evt_event_rel.rel_event_id = wp_evt_event_rel.event_id
        WHERE 
        ...
        repl_evt_event_rel.event_id = lrec_repltask.sched_id AND
        repl_evt_event_rel.rel_type_cd = 'WORMVL' AND
        wp_evt_event_rel.rel_type_cd = 'DRVTASK')
        AND
        driver_task.rstat_cd = 0;
       
        -- Get the task prioirty of the driving task.
        IF (ln_TaskPriorityCount > 0 ) THEN
        SELECT 
        driver_task.task_priority_db_id, 
        driver_task.task_priority_cd
        INTO 
        ln_DrvTaskPriorityDbId,
        ln_DrvTaskPriorityCd
        FROM
        sched_stask driver_task    
        WHERE
        (driver_task.sched_db_id, driver_task.sched_id)
        ...
        -- Update the priority of REPL Task with priority of the driving task.
        UPDATE sched_stask SET 
        sched_stask.task_priority_db_id = ln_DrvTaskPriorityDbId,
        sched_stask.task_priority_cd = ln_DrvTaskPriorityCd
        WHERE
        sched_stask.sched_db_id = lrec_repltask.sched_db_id AND
        sched_stask.sched_id = lrec_repltask.sched_id AND
        NOT (
        sched_stask.task_priority_db_id = ln_DrvTaskPriorityDbId AND
        sched_stask.task_priority_cd = ln_DrvTaskPriorityCd        
        );
