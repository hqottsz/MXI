...
--changeSet MX-18678:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- COMMENTING OUT UPGRADE STEPS THAT WERE ALREADY RAN IN THE SOURCE VERSION
-- --  Create a temporary function      
-- CREATE OR REPLACE FUNCTION addInvToQueue
-- (
--     aStartRowIndex NUMBER,
--     aEndRowIndex NUMBER
-- ) RETURN NUMBER
-- IS     
--     TYPE lDbId IS TABLE OF inv_inv.inv_no_db_id%TYPE
--     TYPE lId IS TABLE OF inv_inv.inv_no_id%TYPE     
--     lInvNoDbId lDbId            
--     lInvNoId lId
--     lOutput NUMBER
--     lCount NUMBER
     --
--      -- Get inventory to be inserted into the queue within the range specified
--     CURSOR lcurr_record IS
--         SELECT 
--             inv_no_db_id,
--             inv_no_id
--         FROM 
...
--                    inv_inv
--              WHERE
--                    -- Exclude BATCH, SER and ACFT inventory. The config position description should be NULL for these type of inventory
--                    inv_inv.inv_class_cd IN ('ASSY', 'TRK', 'SYS')
--                    AND
--                    -- get the inventory up to the maximum row specified
--                    ROWNUM <= aEndRowIndex
--              ORDER BY 
--                    inv_no_db_id, 
--                    inv_no_id
--         )
--         WHERE     
--             row_index >= aStartRowIndex
--     lRec_rec lcurr_record%ROWTYPE  
-- BEGIN
--
-- lOutput:= 0
-- lCount := 0
--
        --
--     -- Loop through all inventory found and insert it into the queue 
--     FOR lRec_rec IN lcurr_record
--     LOOP
--         INV_DESC_PKG.UpdateInvConfigPosDesc ( lRec_rec.inv_no_db_id, lRec_rec.inv_no_id, lOutput )
         --
--         IF lOutput = -1 THEN
--            RAISE_APPLICATION_ERROR (-20999, APPLICATION_OBJECT_PKG.getmxierror )
--         END IF
         --
--         lCount := lCount + 1
--     END LOOP
...
--
-- DECLARE
--      lStartRowIndex NUMBER
--      lEndRowIndex NUMBER
--      lRange NUMBER      
--      lNumOfRecordInserted NUMBER
          --
-- BEGIN
--       -- This parameter defines the range of inventory to be executed
--       lRange               := 5000
--       lStartRowIndex       := 0
--       lEndRowIndex         := 0
--       lNumOfRecordInserted := 0
       --
--       LOOP
--           lStartRowIndex := lEndRowIndex + 1
--           lEndRowIndex   := lEndRowIndex + lRange
           --
--           lNumOfRecordInserted := addInvToQueue( lStartRowIndex, lEndRowIndex )   
           --
--           -- If the number of records last inserted into the queue less than the range specified, then stop the loop
--           -- because there are no more inventory left to be added to the queue         
--           EXIT WHEN lNumOfRecordInserted < lRange
--       END LOOP     
-- END
-- 
--
-- exec utl_migr_schema_pkg.function_drop('addInvToQueue')
...
BEGIN
   ...
   ELSIF as_InvClassCd = is_BATCH THEN
      ...
      /*
       * Default format: (<bin_qt>x) <part_no_sdesc> (PN: <part_no_oem>, SN: <serial_no_oem>)
       */

      -- Build the base description (round the BinQt to one decimal place)
      is_InvDesc := '('|| rtrim(to_char(af_BinQt, 'FM99999999999999999990D9'), '.') || 'x) ' || as_PartDesc;

      -- Append the batch control number
      is_InvDesc := is_InvDesc || ls_PN || as_PartNoOem || ls_BN || as_SerialNo || ')';
