...
--changeSet DEV-1285:1 stripComments:false
-- migration script for DEV-1285
CREATE OR REPLACE VIEW VW_EVT_PO
(po_db_id, po_id, po_number, po_type_db_id, po_type_cd, po_type_sdesc, vendor_db_id, vendor_id, vendor_cd, event_status_db_id, event_status_cd, user_status_cd, event_status_sdesc, issued_dt, closed_dt, hist_dt, hist_bool, needed_by_dt, promised_by_dt, contact_hr_db_id, contact_hr_id, contact_user_id, contact_hr_sdesc, contact_username, req_priority_db_id, req_priority_cd, req_priority_sdesc, currency_db_id, currency_cd, currency_sdesc, currency_bitmap_db_id, currency_bitmap_tag, total_price_qt, exchg_qt, terms_conditions_db_id, terms_conditions_cd, terms_conditions_sdesc, fob_db_id, fob_cd, fob_sdesc, vendor_account_db_id, vendor_account_id, vendor_account_cd, vendor_loc_db_id, vendor_loc_id, vendor_loc_cd, vendor_contact_name, vendor_phone_ph, org_db_id, org_id, created_by_org_db_id, created_by_org_id, ship_to_loc_db_id, ship_to_loc_id, ship_to_loc_cd, ship_to_address_pmail_1, ship_to_address_pmail_2, ship_to_city_name, ship_to_state_cd, ship_to_country_db_id, ship_to_country_cd, ship_to_zip_cd, ship_to_code, spec2k_cust_db_id, spec2k_cust_cd, transport_type_db_id, transport_type_cd, transport_type_sdesc, vendor_note, ext_key_sdesc, po_auth_flow_db_id, po_auth_flow_cd, po_auth_flow_sdesc, borrow_rate_db_id, borrow_rate_cd, borrow_rate_sdesc, last_mod_dt, receipt_insp_bool, auth_bool, po_revision_no, broker_db_id, broker_id, broker_cd, broker_contact_name, broker_phone_ph, broker_account_db_id, broker_account_id, broker_account_cd)
AS
SELECT
      po_header.po_db_id,
      po_header.po_id,
      ...
      org_vendor.vendor_loc_db_id,
      org_vendor.vendor_loc_id,
      vendor_loc.loc_cd,
      ...
      po_header.po_revision_no,
      po_header.broker_db_id,
      po_header.broker_id,
      org_broker.vendor_cd,
      broker_contact.contact_name,
      broker_contact.phone_ph,
      po_header.broker_account_db_id,
      po_header.broker_account_id,
      po_header.broker_account_cd
   FROM
      evt_event,
      po_header,
      ref_event_status,
      ...
      org_vendor org_broker,
      inv_loc broker_loc,
      inv_loc_contact broker_contact,
      ref_currency,
      org_hr,
      utl_user,
   ...
   WHERE
      ...
      AND
      vendor_loc.loc_db_id = org_vendor.vendor_loc_db_id AND
      vendor_loc.loc_id    = org_vendor.vendor_loc_id
      AND
      ...
      AND
      org_broker.vendor_db_id = po_header.broker_db_id AND
      org_broker.vendor_id    = po_header.broker_id
      AND
      broker_loc.loc_db_id = org_broker.vendor_loc_db_id AND
      broker_loc.loc_id    = org_broker.vendor_loc_id
      AND
      broker_contact.loc_db_id    (+)= broker_loc.loc_db_id AND
      broker_contact.loc_id       (+)= broker_loc.loc_id AND
      broker_contact.default_bool (+)= 1
      AND
...
--changeSet DEV-1285:2 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- migration script to remove the VENDOR_LOC_* FK columns from PO_HEADER table
BEGIN
   UTL_MIGR_SCHEMA_PKG.INDEX_DROP('INVLOCSHIPTO_POHEADER_FK');
END;
/

--changeSet DEV-1285:3 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TABLE_CONSTRAINT_DROP('PO_HEADER', 'FK_INVLOCSHIPTO_POHEADER');
END;
/

--changeSet DEV-1285:4 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TABLE_COLUMN_DROP('PO_HEADER', 'VENDOR_LOC_DB_ID');
END;
/

--changeSet DEV-1285:5 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TABLE_COLUMN_DROP('PO_HEADER', 'VENDOR_LOC_ID');
END;
/