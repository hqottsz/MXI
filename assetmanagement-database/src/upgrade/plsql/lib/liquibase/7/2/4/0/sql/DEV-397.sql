...
--changeSet DEV-397:1 stripComments:false
INSERT INTO utl_job (job_cd, job_name, start_time, start_delay, repeat_interval, active_bool, utl_id)
SELECT 'MX_COMMON_PURGE_RECORDS', 'Alert Purging job', '00:00', null, 86400, 0, 0
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM utl_job WHERE utl_job.job_cd = 'MX_COMMON_PURGE_RECORDS');

--changeSet DEV-397:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/*
...
--changeSet DEV-397:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet DEV-397:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY "ALERT_PKG" IS
  ...
  ---------------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------------
    PROCEDURE raise_job_failed_alert(p_error_text  IN utl_alert_parm.parm_value%TYPE,
                                p_priority IN utl_alert.priority%TYPE)  is 
                                             
               t_tab_alert_parm alert_pkg.gt_tab_alert_parm;                  
               c_alert_type     CONSTANT NUMBER := 2; 
      
          begin
            t_tab_alert_parm.DELETE;
                  t_tab_alert_parm(1).parm_ord := 1;
                  t_tab_alert_parm(1).parm_type := c_alert_type;
                  t_tab_alert_parm(1).parm_value_sdesc := p_error_text;
                  t_tab_alert_parm(1).parm_value := p_error_text;
      
                  raise_alert(c_alert_type,p_priority,
                                        p_alert_parm_tb => t_tab_alert_parm);
                                        
    end raise_job_failed_alert;
