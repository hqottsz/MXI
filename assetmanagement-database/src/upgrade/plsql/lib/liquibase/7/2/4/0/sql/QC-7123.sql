...
--changeSet QC-7123:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
SELECT
   SUM( NVL(task_labour_list.man_pwr_ct,0) * ( NVL(task_labour_list.work_perf_hr,0) + NVL(task_labour_list.cert_hr,0) + NVL(task_labour_list.insp_hr,0) ) )
INTO ln_ManHours
FROM
   task_labour_list
WHERE
