...
--changeSet MX-17209:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TYPE_DROP('MXCODEKEY');
END;
/

--changeSet MX-17209:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE TYPE MXCODEKEY AS OBJECT
(
                db_id   NUMBER,
                cd      VARCHAR2(8)
);
...
--changeSet MX-17209:3 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TYPE_DROP('INVENTORYSTATUS');
END;
/

--changeSet MX-17209:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE TYPE INVENTORYSTATUS
 AS OBJECT (

   h_inv_db_id              NUMBER,
   h_inv_id                 NUMBER,
   aircraft_bool            NUMBER(1, 0),
...
--changeSet MX-17209:5 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_SCHEMA_PKG.TYPE_DROP('INVENTORYSTATUSTABLE');
END;
/

--changeSet MX-17209:6 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE TYPE INVENTORYSTATUSTABLE AS TABLE OF InventoryStatus;
/

--changeSet MX-17209:7 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet MX-17209:8 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
-- Returns true if the inventory is a KIT inv
FUNCTION IsKitInv( arec_HInvKey IN mxKey )   RETURN BOOLEAN;

-- Returns the kit condition calculated based on its sub-kit inventory condition
FUNCTION GetKitCondition( arec_HInvKey IN mxKey, ao_Condition IN mxCodeKey )
   RETURN mxCodeKey;
...
BEGIN
   ...
   lb_IsKit      := IsKitInv( lrec_HInvKey );	

   -- Process fault information for the highest inventory.
   lrec_FaultsInfo := GetListOfFaults( lrec_HInvKey );
   IF ( lrec_FaultsInfo.has_AOG_faults ) THEN
      -- AOG faults ground an aircraft.
      lo_NewOpStatus     := AOG;
      lo_NewCond         := REPREQ;
      ls_ReasonForChange := STATUS_FAULTS_AOG;

   ELSIF ( lrec_FaultsInfo.has_unknown_faults ) THEN
      -- UNKNOWN fauls ground an aircraft.
      lo_NewOpStatus     := AOG;
      lo_NewCond         := REPREQ;
   ...
   -- Faults are processed.  At this point, AOG status will cause other status
   -- logic to be skipped.
         ...
         IF ( lo_NewCond.cd <> REPREQ.cd ) THEN
            ...
            ELSE
               ...
               IF ( lb_IsAircraft ) THEN
                  lo_NewCond      := INSRV;
			   -- Inventory is a Kit inventory
               ELSIF ( lb_IsKit ) THEN
			      lo_NewCond         := GetKitCondition( lrec_HInvKey, lo_OldCond);
               ELSE
                  lo_NewCond      := RFI;
               END IF;
...
BEGIN
  ...
  FOR lrec_Fault IN lcur_FaultInfo( arec_HInvKey.db_id, arec_HInvKey.id ) LOOP
     ...
     IF    ( lrec_Fault.is_aog     = 1 ) THEN
        ...
        EXIT;
     ELSIF ( lrec_Fault.is_unknown = 1 ) THEN
        -- UNKNOWN faul
        orec_FaultsInfo.has_unknown_faults := TRUE;
     ELSIF ( lrec_Fault.is_loose   = 1 AND lrec_Fault.is_active = 1 ) THEN
...
* Arguments:      arec_HInvKey IN mxKey: highest inventory key for the query.
*
* Returns:        True if O/D tasks exist in the inventory's tree.
*
...
* Arguments:      arec_HInvKey IN mxKey: highest inventory key for the query.
*
* Returns:        TRUE if this inventory is a Kit inventory
*
*/
FUNCTION IsKitInv( arec_HInvKey mxKey )
   RETURN BOOLEAN
AS
...
/********************************************************************************
*
* Function:       GetKitCondition
*
* Arguments:      arec_HInvKey IN mxKey: highest inventory key for the query.
*                       ao_Condition IN mxCodeKey: kit inv old condition.
*
* Returns:        The kit condition calculated based on its sub-kit inventory condition
*
*/
...
BEGIN
   ...
   BEGIN
      ...
      WHERE
         ...
         AND
         inv_inv.inv_cond_cd != QUAR.cd
         AND
         ROWNUM = 1;

   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         -- Not really an exception; kit does not have unserviceable sub_kit_inv.
         lo_Result  := ao_Condition;
   END;
   RETURN lo_Result;
END GetKitCondition;
