...
--changeSet DEV-396:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/*
 ...
 */
 CREATE OR REPLACE PACKAGE purge_pkg IS
	...
	/****************************
   	Package Exceptions
	*****************************/
	gc_ex_purge_err CONSTANT NUMBER := -30100;
...
--changeSet DEV-396:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/*
 * Confidential, proprietary and/or trade secret information of
 * Mxi Technologies, Ltd.
 *
	...
	BEGIN
		...
		FOR rec_purge_policy IN cur_get_policy LOOP
			...
			FOR rec_purge_strategy IN cur_get_policy_strategy ( rec_purge_policy.purge_policy_cd ) LOOP         
				...
				-- that only active or soft-deleted data is deleted, and that the retention 
			...
			-- Commit each time a purging policy is successfully processed
		   COMMIT;
