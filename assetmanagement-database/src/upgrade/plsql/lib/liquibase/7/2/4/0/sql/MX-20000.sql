...
--changeSet MX-20000:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   ELSIF as_InvClassCd = is_BATCH THEN
      ...
      -- For quantities with no fractional value, the *locale specific* decimal point
      -- has to be stripped from the end of the formatted quantity.
      -- This query returns the locale-specific decimal separators.
      SELECT
         value
      INTO
         ls_nlsNumericCharacters
      FROM
         nls_session_parameters
      WHERE parameter = 'NLS_NUMERIC_CHARACTERS';

      -- Build the base description (round the BinQt to one decimal place)
      is_InvDesc := '('|| rtrim(to_char(af_BinQt, 'FM99999999999999999990D9'), ls_nlsNumericCharacters ) || 'x) ' || as_PartDesc;
