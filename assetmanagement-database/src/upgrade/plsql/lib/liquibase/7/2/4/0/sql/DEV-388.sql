...
--changeSet DEV-388:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
 BEGIN
...
--changeSet DEV-388:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
  BEGIN
   utl_migr_schema_pkg.table_column_modify('
...
--changeSet DEV-388:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
IS
   ...
   SELECT
      ...
      CASE WHEN inv_inv.inv_class_cd IN ('SER', 'BATCH', 'ACFT', 'KIT')
           ...
           ELSE
             ...
             CASE WHEN ( inv_inv.nh_inv_no_db_id IS NULL
                         OR
                         ( nh_inv_inv.assmbl_db_id = h_inv_inv.assmbl_db_id AND nh_inv_inv.assmbl_cd = h_inv_inv.assmbl_cd )
                         OR
   ...
   FROM
      ...
      LEFT OUTER JOIN inv_inv nh_inv_inv ON
