...
--changeSet MX-10975:1 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
    UTL_MIGR_SCHEMA_PKG.TABLE_COLUMN_CONS_CHK_DROP('CLAIM_PART_LINE', 'RMV_INV_NO_DB_ID');
END;
/ 

--changeSet MX-10975:2 stripComments:true endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
    UTL_MIGR_SCHEMA_PKG.TABLE_COLUMN_CONS_CHK_DROP('CLAIM_PART_LINE', 'RMV_INV_NO_ID');
END;
/ 

--changeSet MX-10975:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
 BEGIN
  utl_migr_schema_pkg.table_column_modify('
  Alter table CLAIM_PART_LINE modify (
    "RMV_INV_NO_DB_ID" Number(10,0) Check (RMV_INV_NO_DB_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE 
  )
  ');
 END;
 / 

--changeSet MX-10975:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
 BEGIN
  utl_migr_schema_pkg.table_column_modify('
  Alter table CLAIM_PART_LINE modify (
    "RMV_INV_NO_ID" Number(10,0) Check (RMV_INV_NO_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
  )
  ');
 END;
 / 

--changeSet MX-10975:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   --Check if the primary key exist before inserting a row
   SELECT
      COUNT(*)
   INTO
      ln_count_wtyInv
   FROM
      warranty_init_inv
   WHERE
      warranty_init_inv.warranty_init_db_id = an_warrantyDbId AND
      warranty_init_inv.warranty_init_id    = an_warrantyId
      AND
