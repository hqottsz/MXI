...
--changeSet MX-16908:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
/* This procedure will flag a task for Baseline Sync Zipping */
...
/* Add all of the assembly measurements.  */
   ...
   THEN
      RequestZipForNewBlock( an_InvNoDbId, an_InvNoId, ln_EventDbId, ln_EventId, on_Return );
   ELSE
      IF ( ls_TaskClassMode = 'REQ' )
      THEN
         RequestZipForNewReq( an_InvNoDbId, an_InvNoId, ln_EventDbId, ln_EventId, on_Return );
      END IF;
   END IF;
...
BEGIN
   ...
   (
    ...
    INNER JOIN task_task block_chain
      ...
      WHERE
         block_stask.sched_db_id = an_BlockDbId AND
         block_stask.sched_id    = an_BlockId AND
         block_stask.rstat_cd = 0
   );
   ...
   (
      ...
      FROM
         ...
         INNER JOIN sched_stask ON
            sched_stask.task_db_id = req_task.task_db_id AND
   ...
   BaselineSyncPkg.removeDuplicateQueues(ln_ZipId, on_Return);
...
PROCEDURE RequestZipForNewReq(
   ...
   CURSOR lcur_ZipList (
      ...
      WHERE
         ...
         req_stask.sched_id    = an_ReqId AND
         req_stask.rstat_cd   = 0
         AND
...
/* cursor used to find latest active task based on older task revision. The task must be in INWORK or maybe COMMIT check */
   CURSOR lcur_FindOlderPreviousTask(
     ...
     WHERE
         ...
         AND
         utl_config_parm.parm_name = 'BASELINESYNCH_COMMITTED_TASKS'
         AND
         (
           (
             lower(utl_config_parm.parm_value) = 'true' AND
             check_event.event_status_cd = 'IN WORK'
           )
           OR
           (
             lower(utl_config_parm.parm_value) = 'false' AND
...
--changeSet MX-16908:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
  utl_migr_schema_pkg.table_column_modify('
  Alter table SCHED_STASK modify (
    "TOOLS_READY_BOOL" Number(1,0) Default 1 NOT NULL DEFERRABLE  Check (TOOLS_READY_BOOL IN (0, 1) ) DEFERRABLE 
  )
  ');
 END;
 /

--changeSet MX-16908:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
  utl_migr_schema_pkg.table_column_modify('
  Alter table SCHED_STASK modify (
    "PARTS_READY_BOOL" Number(1,0) Default 1 NOT NULL DEFERRABLE  Check (PARTS_READY_BOOL IN (0, 1) ) DEFERRABLE
  )
  ');
 END;
 /