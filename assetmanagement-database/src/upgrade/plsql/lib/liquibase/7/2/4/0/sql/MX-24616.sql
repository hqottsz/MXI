...
--changeSet MX-24616:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
/**
...
CREATE OR REPLACE PROCEDURE deleteFlightPlans(
   ...
   CURSOR lcur_FlightPlan IS
      ...
      FROM
         inv_ac_flight_plan
      WHERE
         inv_ac_flight_plan.inv_no_db_id = aInvNoDbId AND
         inv_ac_flight_plan.inv_no_id    = aInvNoId
      FOR UPDATE
      ;
   
   lrec_FlightPlan lcur_FlightPlan%ROWTYPE;

BEGIN
   -- SELECT FOR UPDATE allows us to do row-locks when deleting all results
   -- returned from the select clause. Using DELETE FROM directly causes a
   -- table-lock that leads to contention problems. Adding an index was
   -- considered, however it was not used since it may lead to further
   -- contention problems.
   FOR lrec_FlightPlan IN lcur_FlightPlan LOOP
      DELETE FROM inv_ac_flight_plan
         WHERE CURRENT OF lcur_FlightPlan;
   END LOOP;
END;
/

--changeSet MX-24616:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
