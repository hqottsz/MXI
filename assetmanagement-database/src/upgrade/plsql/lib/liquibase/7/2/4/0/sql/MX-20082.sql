...
--changeSet MX-20082:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
(
   ...
   an_WPSchedId   IN sched_stask.sched_db_id%TYPE
)
IS
...
*  Orig. Coder:  sbicharr
*  Recent Coder:
*  Recent Date:  2008-09-11
*
*********************************************************************************
*
*  Copyright 2008-09-11 Mxi Technologies Ltd.  All Rights Reserved.
*  Any distribution of the MxI source code by any other party than
*  MxI Technologies Ltd is prohibited.
*
*********************************************************************************/
...
(
   ...
   -- of the work package*/
   lWtyDbId warranty_init.warranty_init_db_id%TYPE;
   lWtyId   warranty_init.warranty_init_id%TYPE;
   lWtyTypeDbId warranty_defn.warranty_type_db_id%TYPE;
   lWtyTypeCd   warranty_defn.warranty_type_cd%TYPE;
...
(
   ...
   lWtyDbId warranty_init_task.warranty_init_db_id%TYPE;
   lWtyId   warranty_init_task.warranty_init_id%TYPE;
   lWtyTypeDbId warranty_defn.warranty_type_db_id%TYPE;
   lWtyTypeCd   warranty_defn.warranty_type_cd%TYPE;
...
(
  ...
  --with main inventory of the task*/
   lWtyDbId warranty_init_inv.warranty_init_db_id%TYPE;
   lWtyId   warranty_init_inv.warranty_init_id%TYPE;
   lWtyTypeDbId warranty_defn.warranty_type_db_id%TYPE;
   lWtyTypeCd   warranty_defn.warranty_type_cd%TYPE;
   ...
   CURSOR lCur_AssCompWarranties IS
      SELECT warranty_init_inv.warranty_init_db_id,
             ...
             warranty_defn.warranty_type_cd
        FROM evt_inv,
             --evt_inv first_assembl_inv,
             --evt_inv second_assembl_inv,
             --inv_inv,
             warranty_init_inv,
             warranty_init,
             warranty_defn
...
BEGIN
   ...
   IF ln_LaborCount + ln_PartCount >= 1
   THEN
      RETURN 1;
   END IF;
      RETURN 0;
   ...
   WHERE
      ...
      AND
                ...
                ( an_SchedDbId, an_SchedId ) IN
                    (
                    SELECT
                      warranty_tasks.event_db_id,
                      warranty_tasks.event_id
                    FROM
                    (
                      SELECT
                                *
                    ...
                    evt_event
                        START WITH
                                warranty_init_task.warranty_init_db_id = an_WtyInitDbId AND
                                warranty_init_task.warranty_init_id    = an_WtyInitId
                                AND
...
IS
   ...
   ls_ConfigSlotCd eqp_assmbl_bom.assmbl_cd%TYPE;
   ls_ConfigWtyCd eqp_assmbl_bom.assmbl_cd%TYPE;
   ...
   ls_ConfigSlotStatusCd warranty_defn.config_slot_state_cd%TYPE;
   ls_WarrantyTypeCd     warranty_defn.warranty_type_cd%TYPE;
   lCur_ConfigSlotChildren SYS_REFCURSOR;
BEGIN
   --Get the status of the warranty config slot
   SELECT warranty_defn.config_slot_state_db_id,
          warranty_defn.config_slot_state_cd,
          warranty_defn.warranty_type_cd
     INTO ls_ConfigSlotStatusDbId,
          ls_ConfigSlotStatusCd,
          ls_WarrantyTypeCd
     FROM warranty_init,
          warranty_defn
    WHERE warranty_init.warranty_init_db_id = an_WtyInitDbId AND
          warranty_init.warranty_init_id    = an_WtyInitId
          AND
          warranty_defn.warranty_defn_db_id = warranty_init.warranty_defn_db_id AND
          warranty_defn.warranty_defn_id    = warranty_init.warranty_defn_id;

    IF ls_WarrantyTypeCd <> 'ASSEMBLY'
    THEN
      RETURN 0;
    END IF;

    --If the config slot status is 'all' return 1
    IF ls_ConfigSlotStatusDbId = 0 AND
          ls_ConfigSlotStatusCd   = 'ALL'
          THEN
          ln_ConfigSlotCount:= 1;
    ELSIF ls_ConfigSlotStatusDbId = 0 AND
          ls_ConfigSlotStatusCd   = 'NONE'
          THEN
     ...
     ELSE
          ...
          SELECT eqp_assmbl_bom.assmbl_db_id,
           ...
           WHERE sched_stask.sched_db_id = an_SchedDbId AND
                 ...
                 eqp_assmbl_bom.assmbl_bom_id = task_task.assmbl_bom_id;
          ELSIF
             ln_ConfigSlotExist = 0
          THEN
             --Get the config slot of the inventory
             SELECT evt_inv.assmbl_db_id,
                    evt_inv.assmbl_cd,
          ...
          END IF;
             ...
             SELECT warranty_terms_config_slots.assmbl_db_id,
                 ...
                 EXIT WHEN lCur_ConfigSlotChildren%NOTFOUND;
                    ...
                    FROM
                       ...
                       START WITH
                          ...
                          eqp_assmbl_bom.assmbl_cd     = ls_ConfigWtyCd   AND
                    ...
                    WHERE
                       ...
                       assembl_children.assmbl_cd     = ls_ConfigSlotCd AND
          ...
          IF
             ...
             ls_ConfigSlotStatusCd   = 'EXCLUDE'
          THEN
             IF
                ln_ConfigSlotCount >= 1
             THEN
                --return o if config slot exist
                ln_ConfigSlotCount:= 0;
             ELSE
                ln_ConfigSlotCount:= 1;
             END IF;
...
*
*  Orig. Coder:  Tony Hooper
*  Recent Coder:
*  Recent Date:  May 19, 2009
*
*********************************************************************************
*
*  Copyright ? 1998 MxI Technologies Ltd.  All Rights Reserved.
...
BEGIN
   ...
   -- all non-repairable parts from part-group
      ...
      FROM
           sched_part JOIN eqp_part_baseline
           ON eqp_part_baseline.bom_part_db_id = sched_part.sched_bom_part_db_id AND
              eqp_part_baseline.bom_part_id = sched_part.sched_bom_part_id
           JOIN eqp_part_no
           ON eqp_part_no.part_no_db_id = eqp_part_baseline.part_no_db_id AND
      ...
      FROM
          sched_part JOIN eqp_part_no ON
   ...
   IF ln_RemovedPartCount + ln_NonRepairablePart + ln_NonRepairableGroup >= 1
   THEN
      RETURN 1;
   END IF;
      RETURN 0;
...
PROCEDURE UpdateWarrantyEvalPartByInv
   (
    ...
    as_statusCd         IN warranty_eval.status_cd%TYPE,
...
IS
   ln_count_wtyEval NUMBER := 0;
   ln_count_wtyEvalTask NUMBER := 0;
...
IS
   ln_count_wtyevaltaskpart NUMBER := 0;
   ln_count_wtyEval NUMBER := 0;
   ln_count_wtyEvalTask NUMBER := 0;
   ln_warrantyEvalId warranty_eval.warranty_eval_id%TYPE;
   ln_warrantyEvalTaskId warranty_eval_task.warranty_eval_task_id%TYPE;
   warrantyEvalPartId warranty_eval_part.warranty_eval_part_id%TYPE;
BEGIN
   --WARRANTY_EVAL
   --Check if the a row exist in warranty_eval table
   SELECT
   ...
   IF ln_count_wtyevaltaskpart <= 0 THEN
      --Get the next sequence value in from the warranty eval table.
      SELECT
         warranty_eval_part_id.NEXTVAL
      INTO
...
IS
   ...
   ls_PartNoStatusCd warranty_defn.rep_parts_state_cd%TYPE;
BEGIN
   --Get the status of the warranty config slot
   SELECT warranty_defn.rep_parts_state_db_id,
          warranty_defn.rep_parts_state_cd
     INTO ln_PartNoStatusDbId,
          ls_PartNoStatusCd
     FROM warranty_init,
          warranty_defn
    WHERE warranty_init.warranty_init_db_id = an_WtyInitDbId AND
       ...
       IF ln_PartNoStatusDbId = 0 AND
          ls_PartNoStatusCd   = 'ALL'
          THEN
          ln_PartNoCount:= 1;
          --If the config slot status is 'none' return 0
    ELSIF ln_PartNoStatusDbId = 0 AND
          ls_PartNoStatusCd   = 'NONE'
          THEN
          ln_PartNoCount:= 0;
       ELSE
...
IS
   ...
   ls_PartNoStatusCd warranty_defn.non_rep_parts_state_cd%TYPE;
BEGIN
   --Get the status of the warranty non rep parts
   SELECT warranty_defn.non_rep_parts_state_db_id,
    ...
    WHERE warranty_init.warranty_init_db_id = an_WtyInitDbId AND
       ...
       THEN
          IF
          isPartNoAupAboveThreshhold(an_PartNoDbId,
                                     an_PartNoId,
                                     an_WtyInitDbId,
                                     an_WtyInitId) = 1
          THEN
   ...
   LOOP
      ...
      IF
        ...
        THEN
          ...
          THEN
             ...
             THEN
              --Check if warranty contract transfers terms to removed inventory
               ...
               THEN
                   ...
                   UpdateWarrantyInitInv(an_WtyInitDbId,
                                         an_WtyInitId,
                                         ln_InvDbId,
                                         ln_InvId
                                         );
             ...
             UpdateWarrantyEvalPartByInv(an_LocalDbId,
                                         0,
                                         'PENDING',
                                         an_WtyInitDbId,
                                         an_WtyInitId,
      ...
      ELSE
        IF
           ...
           -- if task qualifies
           isRoutineTaskIncluded(an_WtyInitDbId,
                                 an_WtyInitId,
                                 an_SchedDbId,
                                 an_SchedId) = 1
           AND
           ...
           isRepPart(ln_PartNoDbId,
                      ln_PartNoId) = 0
           AND
           -- and it's under warranty & unit price threshold
           IsNonRepPartUnderWarranty(an_WtyInitDbId,
                                     an_WtyInitId,
                                     ln_PartNoDbId,
                                     ln_PartNoId) = 1
        THEN
           IF
             ...
             isAupLessThanTurnInThreshold(an_WtyInitDbId,
                                     an_WtyInitId,
                                     ln_PartNoDbId,
                                     ln_PartNoId) = 1
           THEN
          ...
          --Update the warranty eval part
           UpdateWarrantyEvalPartByReq(an_LocalDbId,
                               0,
                               'PENDING',
                               an_WtyInitDbId,
                               an_WtyInitId,
      ...
      LOOP
         ...
         IF
            ...
            isRepPart(ln_PartNoDbId,
                      ln_PartNoId) = 1
            AND
            IsRepPartUnderWarranty(an_WtyInitDbId,
                                   an_WtyInitId,
                                   ln_PartNoDbId,
                                   ln_PartNoId) = 1
         THEN
            ...
            --Check if warranty contract transfers terms to removed inventory
            --Only do this is OEM only is false & these is an actual inventory removal record
            --EvaluateRemovedComponents(...) procedure will handle OEM stuff once removed removal
            --Check if warranty contract transfers terms to removed inventory
            IF
              ln_InvDbId IS NOT NULL
              AND
              isOEMOnlyPart(an_WtyInitDbId,
                             an_WtyInitId) = 0
            THEN
              SELECT
                COUNT(*)
...
BEGIN
   ...
   THEN
      ...
      UpdateWarrantyEvalLabour(an_LocalDbId,
                             0,
                             'PENDING',
                             an_WtyInitDbId,
                             an_WtyInitId,
...
IS
   ln_TaskType sched_stask.task_class_cd%TYPE;
...
BEGIN
   ...
   IF
      ...
      ln_TaskType IN ('RO','CHECK')
   THEN
      --Delete all WP existing evaluation results
      DeleteWPEvalResults(an_SchedDbId,
                          an_SchedId);
      ...
      GetAssOrCompWarrantiesFromWP(an_SchedDbId,
                                   an_SchedId);
      ...
      GetTaskWarranties(ln_TaskDbId,
                        ln_TaskId);
      END LOOP;
      CLOSE lCur_AssignedWPTask;
   ELSE
      --Delete all existing evaluation results
      Deletetaskevalresults(an_SchedDbId,
                            an_SchedId);
      --Get Assembly and Component warranties of the task
      --Store in the scratch Pad table(task_warranty_sp)
      GetAssOrCompWarrantiesFromTask(an_SchedDbId,
                                     an_SchedId);
      --Get Task warranties associated to the task and
      --Store in the scratch Pad table(task_warranty_sp)
      GetTaskWarranties(an_SchedDbId,
                        an_SchedId);
   END IF;
   --Filter out all expired warranties from the scratch pad table
   DELETE FROM task_warranty_sp where isWarrantyExpired(task_warranty_sp.warranty_init_db_id,
                                                        task_warranty_sp.warranty_init_id) = 1;
         ...
         WHERE
            ...
            LOOP
               ...
               THEN
                  ...
                  END IF;
                  IF
                    IsTaskConfigSlotUnderWarranty(ln_WtyDbId,
                                                   ln_WtyId,
                                                   ln_TaskDbId,
                                                   ln_TaskId) = 1
                     OR
                     isTaskOrCompWarranty(ln_WtyDbId,
                                          ln_WtyId,
                                          ln_TaskDbId,
                                          ln_TaskId) = 1
                  THEN
                     IF
                        ...
                        --removed parts
                        isOEMOnlyPart(ln_WtyDbId,
                                      ln_WtyId) = 1 AND
                        isCompleteTask(ln_TaskDbId,
                                       ln_TaskId) = 1 AND
                        hasRemovedPart(ln_TaskDbId,
                                       ln_TaskId) = 1
                     THEN
                        ...
                        EvaluateRemovedComponents(ln_WtyDbId,
                                                  ln_WtyId,
                                                  ln_TaskDbId,
                                                  ln_TaskId,
                                                  ln_SaveTaskDbId,
                     ...
                     ELSIF
                        ...
                        isRoutineTaskIncluded(ln_WtyDbId,
                                             ln_WtyId,
                                             ln_TaskDbId,
                                             ln_TaskId) = 1
                     THEN
                         --Evaluate the part requirements of the task
                         EvaluateTaskParts(ln_WtyDbId,
                                         ln_WtyId,
                                         ln_TaskDbId,
                                         ln_TaskId,
                                         ln_SaveTaskDbId,
...
--changeSet MX-20082:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- Script to remove duplicate associations between inventory and assembly warranty definitions 
-- (i.e. warranty_init_inv should have exactly one row for a given warranty_assembly_defn+inv_key.
DECLARE

   -- Find all warranty_init_inv rows with duplicate assembly warranties.
   CURSOR lcur_WarrantyConflicts IS
      SELECT 
         * 
      FROM (
         SELECT DISTINCT
            warranty_init_inv.inv_no_db_id, warranty_init_inv.inv_no_id, 
            warranty_defn_assembly.warranty_defn_db_id, warranty_defn_assembly.warranty_defn_id,
            COUNT(*) OVER (PARTITION BY inv_no_db_id, inv_no_id, warranty_defn_assembly.warranty_defn_db_id, warranty_defn_assembly.warranty_defn_id) AS dupe_count
         FROM
            warranty_init_inv  
            JOIN warranty_init          ON warranty_init.warranty_init_db_id = warranty_init_inv.warranty_init_db_id AND
                                           warranty_init.warranty_init_id    = warranty_init_inv.warranty_init_id
            JOIN warranty_defn_assembly ON warranty_defn_assembly.warranty_defn_db_id = warranty_init.warranty_defn_db_id AND
                                           warranty_defn_assembly.warranty_defn_id    = warranty_init.warranty_defn_id
         ) 
      WHERE dupe_count > 1;


   -- For a given warranty_def+inv key, return information on all associated warranty_init_inv rows.
   -- Contains information on whether the row is associated with a claim, and whether it's the OEM warranty.
   -- Result is sorted so that claim rows are first, then the OEM warranty, then others sorted by ascending creation_dt.
   CURSOR lcur_DuplicateInits(
      aWarrantyDefDbId warranty_defn_assembly.warranty_defn_db_id%TYPE,
      aWarrantyDefId   warranty_defn_assembly.warranty_defn_id   %TYPE,
      aInvNoDbId       inv_inv.inv_no_db_id%TYPE,
      aInvNoId         inv_inv.inv_no_id   %TYPE
   )
   IS
      SELECT 
         this_init.warranty_init_db_id, this_init.warranty_init_id, 
         this_init.inv_no_db_id, this_init.inv_no_id,
         -- Number of claims associated with this warranty_defn+inv_key.
         (
            SELECT 
               COUNT(*) 
            FROM 
               warranty_eval 
               INNER JOIN claim ON claim.warranty_eval_db_id  = warranty_eval.warranty_eval_db_id  AND
                                   claim.warranty_eval_id     = warranty_eval.warranty_eval_id
            WHERE 
               warranty_eval.warranty_init_db_id = this_init.warranty_init_db_id  AND
               warranty_eval.warranty_init_id    = this_init.warranty_init_id
         ) AS claim_count,
         -- Whether this warranty_init_inv is associated with the inventory's OEM assembly's warranty.
         (
            SELECT
               COUNT(*)
            FROM 
               warranty_init_inv master_init,
               inv_inv_oem_assmbl oem
            WHERE
               -- Assembly row for this warranty.
               master_init.warranty_init_db_id     = this_init    .warranty_init_db_id AND
               master_init.warranty_init_id        = this_init    .warranty_init_id    AND
               master_init.master_bool             = 1
               AND
               oem.inv_no_db_id            = this_init  .inv_no_db_id AND
               oem.inv_no_id               = this_init  .inv_no_id    AND
               oem.oem_assmbl_inv_no_db_id = master_init.inv_no_db_id AND
               oem.oem_assmbl_inv_no_id    = master_init.inv_no_id 
         ) AS oem_bool
      FROM 
         warranty_init,
         warranty_init_inv this_init
      WHERE 
         this_init.warranty_init_db_id     = warranty_init.warranty_init_db_id AND
         this_init.warranty_init_id        = warranty_init.warranty_init_id
         AND
         warranty_init.warranty_defn_db_id = aWarrantyDefDbId AND
         warranty_init.warranty_defn_id    = aWarrantyDefId
         AND
         this_init.inv_no_db_id = aInvNoDbId AND
         this_init.inv_no_id    = aInvNoId
      ORDER BY
         claim_count DESC,
         oem_bool DESC,
         this_init.creation_dt ASC
    ;



   lrec_WarrantyConflict lcur_WarrantyConflicts%ROWTYPE;
   lrec_DuplicateInit    lcur_DuplicateInits   %ROWTYPE;
   
   
BEGIN
  
   FOR lrec_WarrantyConflict IN lcur_WarrantyConflicts() LOOP
      
      --dbms_output.put_line( 'Getting dupes for warranty def: ' || lrec_WarrantyConflict.warranty_defn_db_id || ':' || lrec_warrantyConflict.warranty_defn_id || ', inv: ' || lrec_WarrantyConflict.inv_no_db_id || ':' || lrec_WarrantyConflict.inv_no_id );
      
      OPEN lcur_DuplicateInits( lrec_WarrantyConflict.warranty_defn_db_id, lrec_warrantyConflict.warranty_defn_id, lrec_WarrantyConflict.inv_no_db_id, lrec_WarrantyConflict.inv_no_id );
      -- Fetch the first row.  This is immediately discaded because it will not be deleted.
      -- It will not be deleted because it represents a warranty_init_inv row that:
      -- * Is associated with a claim
      -- * ... or is the OEM warranty for inventory with no claims
      -- * ... or is the earliest created warranty for inventory with no claims, and no OEM warranty
      FETCH lcur_DuplicateInits INTO lrec_DuplicateInit;
      -- dbms_output.put_line( 'First row is safe: ' || lrec_DuplicateInit.warranty_init_db_id || ':' || lrec_DuplicateInit.warranty_init_id || ', inv: ' || lrec_DuplicateInit.inv_no_db_id || ':' || lrec_DuplicateInit.inv_no_id );
      
      LOOP
         FETCH lcur_DuplicateInits INTO lrec_DuplicateInit;
         EXIT WHEN lcur_DuplicateInits%NOTFOUND;
         
         -- Do not delete warranties with claims
         IF ( lrec_DuplicateInit.claim_count = 0 ) THEN 
            --dbms_output.put_line( 'Removing row: init: ' || lrec_DuplicateInit.warranty_init_db_id || ':' || lrec_DuplicateInit.warranty_init_id || ', inv: ' || lrec_DuplicateInit.inv_no_db_id || ':' || lrec_DuplicateInit.inv_no_id );

            DELETE FROM 
               warranty_init_inv 
            WHERE  
               warranty_init_inv.warranty_init_db_id = lrec_DuplicateInit.warranty_init_db_id AND
               warranty_init_inv.warranty_init_id    = lrec_DuplicateInit.warranty_init_id
               AND
               warranty_init_inv.inv_no_db_id        = lrec_DuplicateInit.inv_no_db_id AND
               warranty_init_inv.inv_no_id           = lrec_DuplicateInit.inv_no_id;

         END IF;
         
      END LOOP;
      CLOSE lcur_DuplicateInits;
