...
--changeSet MX-24347:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- COMMENTING OUT UPGRADE STEPS THAT WERE ALREADY RAN IN THE SOURCE VERSION
-- BEGIN
-- utl_migr_schema_pkg.table_column_add('
-- Alter table SCHED_STASK add (
--   REPL_SCHED_DB_ID Number(10,0) Check (REPL_SCHED_DB_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
-- )
-- ')
-- END
-- 
...
-- utl_migr_schema_pkg.table_column_add('
-- Alter table SCHED_STASK add (
--   REPL_SCHED_ID Number(10,0) Check (REPL_SCHED_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
-- )
-- ')
-- END
-- 
...
-- utl_migr_schema_pkg.table_column_add('
-- Alter table SCHED_STASK add (
--   REPL_SCHED_PART_ID Number(10,0) Check (REPL_SCHED_PART_ID BETWEEN 0 AND 4294967295 ) DEFERRABLE
-- )
-- ')
-- END
-- 
--
-- --  add foreign key constraint from sched_stask to sched_part FK_SCHEDPART_SCHEDSTASK
-- BEGIN
-- utl_migr_schema_pkg.table_constraint_add('
-- Alter table SCHED_STASK add constraint FK_SCHEDPART_SCHEDSTASK
--   foreign key (REPL_SCHED_DB_ID, REPL_SCHED_ID, REPL_SCHED_PART_ID) 
--   references SCHED_PART (SCHED_DB_ID, SCHED_ID, SCHED_PART_ID) 
--   DEFERRABLE
-- ')
-- END
-- 
--
-- --  add index IX_SCHEDPART_SCHEDSTASK
-- BEGIN
...
-- USING (
--   SELECT DISTINCT
--      root_event.event_db_id,
--      root_event.event_id,
--      FIRST_VALUE(sched_part.sched_db_id) OVER (PARTITION BY root_event.event_db_id, root_event.event_id ORDER BY sched_part.creation_dt) AS sched_db_id, 
--      FIRST_VALUE(sched_part.sched_id) OVER (PARTITION BY root_event.event_db_id, root_event.event_id ORDER BY sched_part.creation_dt) AS sched_id, 
--      FIRST_VALUE(sched_part.sched_part_id) OVER (PARTITION BY root_event.event_db_id, root_event.event_id ORDER BY sched_part.creation_dt) AS sched_part_id
--   FROM
--      (SELECT
--         start_stask.sched_db_id,
--         start_stask.sched_id,
--         DECODE(parent_stask.task_class_cd, 'REPL', parent_stask.sched_db_id, start_stask.sched_db_id) AS repl_sched_db_id,
--         DECODE(parent_stask.task_class_cd, 'REPL', parent_stask.sched_id, start_stask.sched_id) AS repl_sched_id
--      FROM
...
--   ) tmp_table 
--   ON (
--      sched_stask.sched_db_id = tmp_table.event_db_id AND
--      sched_stask.sched_id    = tmp_table.event_id
--   )
-- WHEN MATCHED THEN
--   UPDATE SET
--      sched_stask.repl_sched_db_id  = tmp_table.sched_db_id,
--      sched_stask.repl_sched_id     = tmp_table.sched_id,
--      sched_stask.repl_sched_part_id = tmp_table.sched_part_id
-- 
...
--changeSet MX-24347:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
/* Add all the OC and Non OC Assembly Measurements.  */
   ...
   IF ab_historic = false THEN
       ...
       */
      IF ( ls_TaskClassMode = 'REQ' AND lrec_TaskDefinition.task_class_cd <> 'CORR') THEN
         ...
         IF lrec_TaskDefinition.task_class_cd = 'REPL' THEN
...
--changeSet MX-24347:3 stripComments:false
-- replace view
/*
...
-- in this select we assume the removal has an inventory key
SELECT
   sched_stask.sched_db_id, 
   sched_stask.sched_id, 
   inv_inv.inv_no_db_id AS inv_no_db_id, 
   inv_inv.inv_no_id AS inv_no_id, 
   inv_inv.config_pos_sdesc AS config_pos_sdesc
FROM
   sched_stask
   INNER JOIN sched_part ON
...
UNION ALL
-- in this select we assume the removal has no inventory key
SELECT DISTINCT
   sched_stask.sched_db_id, 
   sched_stask.sched_id, 
   NULL AS inv_no_db_id, 
