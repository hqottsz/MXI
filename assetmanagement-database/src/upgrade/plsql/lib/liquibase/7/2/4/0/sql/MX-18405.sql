...
--changeSet MX-18405:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   FROM
      ...
      sched_rmvd_part
   WHERE
      sched_part.sched_db_id = an_TaskDbId AND
      sched_part.sched_id    = an_TaskId
      AND
      -- non-repairable part-requirements need not have an entry in sched_rmvd_part
      -- hence changing the join to OUTER from INNER should bring non-repairable parts into the ambit
...
IS
   ...
   ln_NonRepairablePartCount NUMBER;
   ln_NonRepairablePartGroupCount NUMBER;
BEGIN
   SELECT
      COUNT(*)
   INTO
   ...
   SELECT
           count(1)   
   INTO 
           ln_NonRepairablePartCount
   FROM
           evt_event JOIN sched_part 
           -- all parts bound to the task
           ON sched_part.sched_db_id = evt_event.event_db_id AND
              sched_part.sched_id    = evt_event.event_id
           JOIN eqp_part_no 
           ON eqp_part_no.part_no_db_id = sched_part.spec_part_no_db_id AND
   ...
   WHERE   
           ...
           AND NOT EXISTS(
                ...
                WHERE
                        ...
                        sched_rmvd_part.sched_id    = an_Sched_Id
                        AND
   ...
   SELECT
           count(1)
   INTO 
           ln_NonRepairablePartGroupCount           
   FROM
           evt_event JOIN sched_part
           ON sched_part.sched_db_id = evt_event.event_db_id AND
              sched_part.sched_id    = evt_event.event_id
           JOIN eqp_part_baseline 
           ON eqp_part_baseline.bom_part_db_id = sched_part.bom_part_db_id AND
              eqp_part_baseline.bom_part_id    = sched_part.bom_part_id        
   ...
   IF ln_RemovedPartCount >= 1 OR ln_NonRepairablePartGroupCount >= 1 OR ln_NonRepairablePartCount >= 1
   THEN
      RETURN 1;
   END IF;
