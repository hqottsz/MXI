...
--changeSet MX-25085:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_DATA_PKG.action_parm_insert( 
      'ACTION_EDIT_MANUAL_INTERVAL',
      'Permission to edit intervals for tasks based on a task definition flagged to allow manual scheduling.', 
      'TRUE/FALSE', 
      'FALSE', 
      1, 
      'Maint - Tasks', 
...
--changeSet MX-25085:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_DATA_PKG.action_parm_insert( 
      'ACTION_SET_BLOCK_MANUAL_SCHEDULING',
      'Permission to flag the block definition as requiring manual input of the scheduling interval on each actual task based on this definition.', 
      'TRUE/FALSE', 
      'FALSE', 
      1, 
      'Maint Program - Blocks', 
      '8.0 alpha', 
      0, 
      0,
...
--changeSet MX-25085:3 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_DATA_PKG.action_parm_insert( 
      'ACTION_SET_REQ_MANUAL_SCHEDULING',
      'Permission to flag the requirement definition as requiring manual input of the scheduling interval on each actual task based on this definition.', 
      'TRUE/FALSE', 
      'FALSE', 
      1, 
...
--changeSet MX-25085:4 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
BEGIN
   UTL_MIGR_DATA_PKG.action_parm_insert( 
      'ACTION_SET_REF_DOC_MANUAL_SCHEDULING',
      'Permission to flag the Reference definition as requiring manual input of the scheduling interval on each actual task based on this definition.', 
      'TRUE/FALSE', 
      'FALSE', 
      1, 
      'Maint Program - Reference Documents', 
      '8.0 alpha', 
      0, 
      0,
      UTL_MIGR_DATA_PKG.DbTypeCdList('MASTER'));
...
--changeSet MX-25085:5 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
-- Commenting out upgrade that was already ran in the upgrade to 7.2.0.0-SP2
-- BEGIN
-- utl_migr_schema_pkg.table_column_add('
-- Alter table TASK_TASK add (
--   MANUAL_SCHEDULING_BOOL Number(1,0)  Check (MANUAL_SCHEDULING_BOOL IN (0, 1) ) DEFERRABLE
-- )
-- ')
-- END
-- 
--
-- UPDATE
--   task_task update_task
-- SET
--   update_task.manual_scheduling_bool = 1
-- WHERE
--   update_task.manual_scheduling_bool IS NULL
--   AND
--   ( update_task.task_defn_db_id, update_task.task_defn_id ) IN
--   (
--      SELECT DISTINCT
...
--         -- only BLOCK and REQ gets deadlines updated by baseline synchronization
...
--            AND
--            evt_sched_dead.data_type_db_id = task_sched_rule.data_type_db_id AND
--            evt_sched_dead.data_type_id    = task_sched_rule.data_type_id
...
--            evt_event_rel.rel_event_id    = sched_stask.sched_id
--            AND
--            evt_event_rel.rel_type_db_id = 0 AND
--            evt_event_rel.rel_type_cd    = 'DEPT'
--         LEFT JOIN sched_stask prev_sched_stask ON
--            prev_sched_stask.sched_db_id = evt_event_rel.event_db_id AND
--            prev_sched_stask.sched_id    = evt_event_rel.event_id
--         LEFT JOIN task_task prev_task_task ON
--            prev_task_task.task_db_id = prev_sched_stask.task_db_id AND
--            prev_task_task.task_id    = prev_sched_stask.task_id
--            AND
--            prev_task_task.task_defn_db_id = task_task.task_defn_db_id AND
--            prev_task_task.task_defn_id    = task_task.task_defn_id
--      WHERE  
--         (
--            (
--               prev_task_task.task_db_id IS NULL
--               AND
--               evt_sched_dead.interval_qt NOT IN (
--                  SELECT
--                     -- defaults to interval when initial is not provided
--                     NVL(task_sched_rule.def_initial_qt,task_sched_rule.def_interval_qt)
--                  FROM
--                     dual
--                  UNION ALL
--                  SELECT
--                     -- defaults to interval when initial is not provided
--                     NVL(task_interval.initial_qt, task_interval.interval_qt)
--                  FROM
--                     task_interval
--                  WHERE
--                     task_interval.task_db_id = task_sched_rule.task_db_id AND
--                     task_interval.task_id    = task_sched_rule.task_id
--                     AND
--                     task_interval.data_type_db_id = task_sched_rule.data_type_db_id AND
--                     task_interval.data_type_id    = task_sched_rule.data_type_id
--                     AND
--                     task_interval.part_no_db_id = inv_inv.part_no_db_id AND
--                     task_interval.part_no_id    = inv_inv.part_no_id
--                  UNION ALL
--                  SELECT
--                     -- defaults to interval when initial is not provided
--                     NVL(task_ac_rule.initial_qt, task_ac_rule.interval_qt) initial_qt
--                  FROM
--                     task_ac_rule
--                  WHERE
--                     task_ac_rule.task_db_id = task_sched_rule.task_db_id AND
--                     task_ac_rule.task_id    = task_sched_rule.task_id
--                     AND
--                     task_ac_rule.data_type_db_id = task_sched_rule.data_type_db_id AND
--                     task_ac_rule.data_type_id    = task_sched_rule.data_type_id
--                     AND
--                     task_ac_rule.inv_no_db_id = inv_inv.h_inv_no_db_id AND
--                     task_ac_rule.inv_no_id    = inv_inv.h_inv_no_id
--                  UNION ALL
--                  SELECT
...
--                  WHERE
--                     task_me_rule_interval.task_db_id = task_sched_rule.task_db_id AND
--                     task_me_rule_interval.task_id    = task_sched_rule.task_id
--                     AND
--                     task_me_rule_interval.rule_data_type_db_id = task_sched_rule.data_type_db_id AND
--                     task_me_rule_interval.rule_data_type_id    = task_sched_rule.data_type_id
--               )
--            )
--            OR
--            (
--               prev_task_task.task_db_id IS NOT NULL
--               AND
--               evt_sched_dead.interval_qt NOT IN (
--                  SELECT
...
--   task_task update_task
-- SET
--   update_task.manual_scheduling_bool = 0
-- WHERE
--   update_task.manual_scheduling_bool IS NULL
-- 
--
...
--   MANUAL_SCHEDULING_BOOL Number(1,0)  Default 0 NOT NULL DEFERRABLE Check (MANUAL_SCHEDULING_BOOL IN (0, 1) ) DEFERRABLE
-- )
-- ')
-- END
-- 
