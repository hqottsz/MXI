...
--changeSet MX-24769:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
CURSOR lCurSchedStask(
   ...
   SELECT
      ...
      curr_task_dead.interval_qt AS curr_task_interval,
      next_sched_stask.sched_db_id,
      next_sched_stask.sched_id,
      next_task_dead.Interval_Qt AS next_task_interval,
   ...
   FROM
      ...
      evt_sched_dead curr_task_dead,
      evt_sched_dead next_task_dead
   WHERE
      sched_stask.sched_db_id = cn_SchedDbId AND
      sched_stask.sched_id    = cn_SchedId   AND
      sched_stask.rstat_cd    = 0
      AND
      curr_task_dead.event_db_id (+)= sched_stask.sched_db_id AND
      curr_task_dead.event_id    (+)= sched_stask.sched_id
      AND
      ...
      AND
      next_task_dead.event_db_id (+)= next_sched.event_db_id AND
      next_task_dead.event_id    (+)= next_sched.event_id;
      ...
      IF ( 
            (
               ...
               (
                  ...
                  OR
                  NVL(lRecSchedStask.Curr_Task_Interval, 0) != 0 AND NVL(lRecSchedStask.Next_Task_Interval, 0) != 0
               )
            )
         OR 
            (
         ...
         ELSE 
            IF lRecSchedStask.task_db_id IS NOT NULL THEN
