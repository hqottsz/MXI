...
--changeSet MX-21281:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   IF ( lrec_FaultsInfo.has_AOG_faults  
        ...
        (
            lb_IsAircraft=TRUE 
            OR 
            ( lb_IsAircraft=FALSE AND NOT HasInWorkTasks( lrec_HInvKey ))
        )
      )   
      THEN
      -- AOG faults ground an aircraft.f
      lo_NewOpStatus     := AOG;
      lo_NewCond         := REPREQ;
   ...
   IF ( lo_NewOpStatus.cd <> 'AOG' ) THEN
      ...
      ELSE
         ...
         IF  ( lrec_FaultStatus.operating_status IS NOT NULL ) THEN
            ...
            lo_NewOpStatus     := lrec_FaultStatus.operating_status;
