...
--changeSet DEV-1083:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
   ...
   FROM
      (-- Either a specific part number
         ...
         WHERE
            ...
            req_part.rstat_cd	 = 0
            AND NOT
            (   req_part.req_spec_part_no_db_id IS NOT NULL
                AND
                po_line.part_no_db_id = req_part.req_spec_part_no_db_id AND
                po_line.part_no_id    = req_part.req_spec_part_no_id
            )
            AND EXISTS
            (  SELECT NULL
               FROM eqp_part_baseline
               WHERE
                 eqp_part_baseline.bom_part_db_id = req_part.req_bom_part_db_id AND
                 eqp_part_baseline.bom_part_id    = req_part.req_bom_part_id
                 AND
                 eqp_part_baseline.part_no_db_id = po_line.part_no_db_id AND
                 eqp_part_baseline.part_no_id    = po_line.part_no_id
            )
            AND
            ship_shipment_line.po_db_id   = po_line.po_db_id AND
            ship_shipment_line.po_id      = po_line.po_id AND
...
--changeSet DEV-1083:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet DEV-1083:3 stripComments:false
