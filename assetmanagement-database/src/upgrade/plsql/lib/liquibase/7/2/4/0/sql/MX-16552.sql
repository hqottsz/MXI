...
--changeSet MX-16552:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
IS
   ...
   SELECT DISTINCT
       ...
       WHERE inv_inv.inv_no_db_id =  cn_InvNoDbId AND
             ...
             ref_inv_class.inv_class_cd = eqp_bom_part.inv_class_cd       
             AND
             ref_inv_class.serial_bool = 1      
             AND
...
--changeSet MX-16552:2 stripComments:false
-- Sub-inventory of ACFT and ASSY should inherit the PREVENT_SYNCHRONIZATION_BOOL value of it's
-- assembly. This script corrects bad data by updating the value to the assembly value.
UPDATE
   inv_inv
SET
   inv_inv.prevent_synch_bool = (
      SELECT
         assmbl_inv_inv.prevent_synch_bool
      FROM
...
WHERE
   inv_inv.inv_class_cd NOT IN ('ACFT', 'ASSY')
   AND
   inv_inv.assmbl_inv_no_db_id IS NOT NULL
   AND
   (
      inv_inv.assmbl_inv_no_db_id <> inv_inv.inv_no_db_id
      OR
      inv_inv.assmbl_inv_no_id <> inv_inv.inv_no_id
   )
   AND
   NOT EXISTS (
      SELECT
      ...
      WHERE
         ...
         assmbl_inv_inv.inv_no_id = inv_inv.assmbl_inv_no_id
         AND
         assmbl_inv_inv.prevent_synch_bool = inv_inv.prevent_synch_bool
   )
;