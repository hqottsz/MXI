...
--changeSet OPER-21255:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
--changeSet OPER-21255:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
CREATE OR REPLACE PACKAGE BODY "APPLICATION_OBJECT_PKG" IS
   ...
   *---------------------------------------------------------*/
   ...
   FUNCTION getdbid RETURN NUMBER PARALLEL_ENABLE IS
      ll_numdbid NUMBER;
   BEGIN
   ...
   /* Function to get the current user */
   FUNCTION getuser RETURN VARCHAR2 PARALLEL_ENABLE IS
   BEGIN
      -- Fetch the current client ID
      pkg_user := sys_context('USERENV','CLIENT_IDENTIFIER');  
        
      -- If the package user is not defined yet
      IF pkg_user IS NULL
      THEN

         -- Set the package user to the database user
         pkg_user := USER;
   ...
   /* Function to get Timestamp */
   FUNCTION gettimestamp RETURN DATE PARALLEL_ENABLE IS
   BEGIN
      RETURN SYSDATE;
   END gettimestamp;

   /* Function to get Mxi's Error msgid and msgparm */
   FUNCTION getmxierror RETURN VARCHAR2 PARALLEL_ENABLE IS
   ...
   /* Function to get the current execution mode */
   FUNCTION getexecmode RETURN VARCHAR2 PARALLEL_ENABLE IS
   ...
   FUNCTION getdbtypecd(an_db_id IN NUMBER) RETURN VARCHAR2 PARALLEL_ENABLE IS
   ...
   *---------------------------------------------------------*/
   /* Function to get the current system version */
   FUNCTION getversion RETURN VARCHAR2 PARALLEL_ENABLE IS
   BEGIN
      RETURN iv_pkg_version;
   END getversion;

   /* Function to get the current system version */
   FUNCTION getdatabaseversion RETURN VARCHAR2 PARALLEL_ENABLE IS
   BEGIN
      RETURN iv_db_version;
   END getdatabaseversion;

   /* Function to get the current system version */
   FUNCTION getservicepacknumber RETURN VARCHAR2 PARALLEL_ENABLE IS
   BEGIN
