...
--changeSet OPER-20973:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
   ...
   (
      ...
      (
         ...
         INNER JOIN inv_onoff_list ON
            inv_curr_usage.inv_no_db_id      = inv_onoff_list.inv_no_db_id    AND
            inv_curr_usage.inv_no_id         = inv_onoff_list.inv_no_id
         INNER JOIN usg_usage_data ON
            inv_onoff_list.inv_no_db_id      = usg_usage_data.inv_no_db_id    AND
            inv_onoff_list.inv_no_id         = usg_usage_data.inv_no_id       AND
            inv_curr_usage.data_type_db_id   = usg_usage_data.data_type_db_id AND
            inv_curr_usage.data_type_id      = usg_usage_data.data_type_id
         INNER JOIN usg_usage_record ON
            usg_usage_record.usage_record_id = usg_usage_data.usage_record_id
         WHERE
            usg_usage_record.usage_type_cd   = 'MXCORRECTION' AND
            usg_usage_record.usage_dt > aidt_date

         -- The query above collects all usage corrections applied to the inventory since the event date
         -- The query below collects all usage accruals on the parent assembly between the inv_onoff_list Installed Timespans
         -- i.e. the accruals from the assembly were applied to this TRK/SER inventory
         UNION ALL
         ...
         INNER JOIN usg_usage_record ON
            usg_usage_record.usage_record_id   =  usg_usage_data.usage_record_id 
         WHERE
            usg_usage_record.usage_type_cd IN ('MXFLIGHT', 'MXACCRUAL') AND
