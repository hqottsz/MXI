...
--changeSet OPER-12255:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
   ...
   IS
      SELECT
         ...
         CASE WHEN inv_inv.inv_class_cd IN ('SER', 'BATCH', 'ACFT', 'KIT')
              ...
              ELSE
                ...
                CASE WHEN ( inv_inv.assmbl_inv_no_db_id IS NULL
                     ...
                     ELSE
                         ...
                         CASE WHEN ( assy_eqp_assmbl_bom.pos_ct > 1 ) OR ( nh_assmbl_bom.pos_ct > 1 ) OR ( assy_eqp_assmbl_pos.eqp_pos_cd != '1' )
              ...
              CASE WHEN ( inv_inv.nh_inv_no_db_id IS NOT NULL AND ( eqp_assmbl_bom.pos_ct > 1  OR nh_assmbl_bom.pos_ct > 1 OR  eqp_assmbl_pos.eqp_pos_cd != '1' ) )
...
--changeSet OPER-12255:2 stripComments:false
-- Add all inventory which would be affected by the logic change in the PL/SQL Function
MERGE INTO inv_pos_desc_queue
USING ( SELECT inv_inv.Inv_No_Db_Id,
               inv_inv.Inv_No_Id,
               inv_inv.rstat_cd
              FROM
                 inv_inv
                 ...
                 LEFT OUTER JOIN eqp_assmbl_bom nh_assmbl_bom ON
                    ...
                    nh_assmbl_bom.assmbl_bom_id  = eqp_assmbl_bom.nh_assmbl_bom_id
          WHERE
             -- If the Inventory class is Serialized, Track, or Aircraft - not eligable for custom descriptions
             inv_inv.inv_class_cd NOT IN ('SER', 'BATCH', 'ACFT', 'KIT') AND
             -- Loose inventory not eligable for Position Info for the Config Slot
             inv_inv.nh_inv_no_db_id IS NOT NULL AND
             -- Parent config slot must have > 1 positions
             nh_assmbl_bom.pos_ct > 1 AND
             -- Current value pos the position name is the default '1'
             eqp_assmbl_pos.eqp_pos_cd = '1' AND
			 -- No Scrapped or archived inventory
             inv_inv.inv_cond_cd NOT IN ('SCRAP','ARCHIVED')
	  ) 
   new_sync_inv ON (new_sync_inv.inv_no_db_id = inv_pos_desc_queue.inv_no_db_id AND
                    new_sync_inv.inv_no_id = inv_pos_desc_queue.inv_no_id)
          WHEN NOT MATCHED THEN
               INSERT (inv_no_db_id,inv_no_id,rstat_cd)
               VALUES (new_sync_inv.inv_no_db_id,new_sync_inv.inv_no_id,new_sync_inv.rstat_cd);
