...
--changeSet inv_status_body:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
BEGIN
   ...
   IF ( lo_NewOpStatus.cd <> 'AOG' ) THEN
      ...
      ELSIF ( NOT IsInventoryComplete( lrec_HInvKey ) ) THEN
         -- if ready for build is enabled and highest inventory is not ACFT then set as RFI
		 IF ( isReadyForBuildEnabled() AND lb_IsAircraft=FALSE ) THEN
            lo_NewCond         := RFI;
