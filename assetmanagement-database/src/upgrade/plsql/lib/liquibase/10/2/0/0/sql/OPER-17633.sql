...
--changeSet OPER-17633:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
-- Returns true if the inventory is new.
FUNCTION IsInventoryNew( arec_HInvKey IN mxKey   ) RETURN BOOLEAN;
...
-- Returns true if the RFB feature is enabled
FUNCTION isReadyForBuildEnabled RETURN BOOLEAN;
...
BEGIN
   ...
   IF ( lo_NewOpStatus.cd <> 'AOG' ) THEN
      ...
      ELSIF ( NOT IsInventoryComplete( lrec_HInvKey ) ) THEN
    	 IF ( isInventoryNew( lrec_HInvKey ) AND lo_OldCond.cd = INSPREQ.cd  ) THEN
    	    -- for new inventory with condition INSPREQ, the new condition is INSPREQ
   		 	lo_NewCond         := INSPREQ;
    		ls_ReasonForChange := STATUS_INCOMPLETE_INVENTORY;
         ELSIF ( isReadyForBuildEnabled() AND lb_IsAircraft=FALSE AND lo_OldCond.cd = INREP.cd ) THEN
            -- if ready for build is enabled, highest inventory is not aircraft, and inventory condition is INREP
            -- then the new condition is RFI
            lo_NewCond         := RFI;
    		ls_ReasonForChange := STATUS_INCOMPLETE_INVENTORY;
         ELSE
		    -- Inventory has missing parts - ground the aircraft.
            lo_NewOpStatus     := AOG;
            lo_NewCond         := REPREQ;
            ls_ReasonForChange := STATUS_INCOMPLETE_INVENTORY;
         END IF;
      ...
      ELSE
         ...
         IF  ( lrec_FaultStatus.operating_status IS NOT NULL AND lrec_FaultStatus.operating_status.cd <> lo_NewOpStatus.cd ) THEN
...
/********************************************************************************
*
* Function:       isReadyForBuildEnabled
*
* Arguments:
*
* Returns:        True if the RFB feature is enabled
...
BEGIN
   ...
   FROM
      utl_config_parm
   WHERE
      utl_config_parm.parm_name = 'ENABLE_READY_FOR_BUILD';
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
      RETURN false;
END isReadyForBuildEnabled;

/********************************************************************************
*
* Function:       IsInventoryNew
*
* Arguments:	  arec_HInvKey IN mxKey, highest inventory key for the query.
*
* Returns:        True if the inventory is new
*
*/
FUNCTION IsInventoryNew( arec_HInvKey mxKey )
   RETURN BOOLEAN
AS

   li_IsInventoryNew INTEGER;
...
BEGIN
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         -- Not really an exception; it's not a new inventory
         li_IsInventoryNew := 0;
   END;

   RETURN li_IsInventoryNew = 1;
