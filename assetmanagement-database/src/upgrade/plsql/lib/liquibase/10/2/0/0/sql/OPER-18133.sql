...
--changeSet OPER-18133:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
...
/
--changeSet OPER-18133:2 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*
...
BEGIN
   ...
   PIPE ROW(ics_LaunchJasperDashboard);
   PIPE ROW(ics_LaunchJasperLDashboard);
   RETURN;
END;
...
IS
   ...
   lTemplatePos INT;
BEGIN
   --search for the prompt parameter
   lTemplatePos := INSTR(as_MenuURLLink, 'aPromptOutput=');
   IF (lTemplatePos > 0) THEN
      IF (INSTR(as_MenuURLLink, 'aPromptOutput=true') > 0) THEN
         lReturn := 1;
      ELSE
         lReturn := 0;
      END IF;
...
IS
   ...
   lTemplatePos INT;
BEGIN
   --search for the restricted output parameter
   lTemplatePos := INSTR(as_MenuURLLink, 'aRestrictedOutput=');
   IF (lTemplatePos > 0) THEN
      lReturn := SUBSTR(as_MenuURLLink, lTemplatePos+18);
      lTemplatePos := INSTR(lReturn, '&', 1, 1);
      IF (lTemplatePos > 0) THEN
         lReturn := SUBSTR(lReturn, 1, lTemplatePos-1);
      END IF;
   ELSE
      lReturn := '';
   END IF;
...
* Orig.Coder:     Chris McGee
* Recent Coder:
* Recent Date:    Sep 5, 2016
*
*********************************************************************************
*
* Copyright 2014-2016 MxI Technologies Ltd.  All Rights Reserved.
* Any distribution of the MxI source code by any other party than
* MxI Technologies Ltd is prohibited.
*
*********************************************************************************/
...
AS
BEGIN
	createUpdateReportConfig(as_Report,
	                                as_ReportEngine,
	                                as_ReportPath,
	                                as_ReportDesc,
	                                as_MenuName,
...
*                 as_MenuName (string) IN: menu text name
*                 as_MenuViewOutput (string) IN: report output type (PDF, etc)
*                 an_PromptViewOutput (number) IN: whether the report should prompt for the output type
*                 an_PromptRestrictedOutput IN: restricted report output types
*                 an_MenuLaunchNewWindow (number) IN: report should be in a new window
*                 as_MenuArgument1 (string) IN: menu arguments (as per UTL_MENU_ARG)
*                 as_MenuArgument2 (string) IN: menu arguments (as per UTL_MENU_ARG)
...
*
* Orig.Coder:     Wayne Yuke
* Recent Coder:   Chris McGee, Yong Zhong
* Recent Date:    Sep 29, 2016
*
*********************************************************************************
*
...
IS
   ...
   ls_PromptViewOutput varchar2(5);
...
BEGIN
   ...
   IF ((as_ReportEngine <> ics_LaunchBOE) AND (as_ReportEngine <> ics_LaunchJasperREST) AND (as_ReportEngine <> ics_LaunchJasperSSO) AND (as_ReportEngine <> ics_LaunchJasperDashboard) AND (as_ReportEngine <> ics_LaunchJasperLDashboard)) THEN
   ...
   IF (as_ReportPath IS NULL) OR (LENGTH(as_ReportPath) <= 0) THEN
      IF ((as_ReportEngine = ics_LaunchJasperREST) OR (as_ReportEngine = ics_LaunchJasperSSO) OR (as_ReportEngine = ics_LaunchJasperDashboard) OR (as_ReportEngine = ics_LaunchJasperLDashboard)) THEN
   ...
   --set prompt parameter boolean string based on the provided boolean value
   IF (an_PromptViewOutput = 1) THEN
      ls_PromptViewOutput := 'true';
   ELSE
      ls_PromptViewOutput := 'false';
   END IF;
         ...
         INSERT INTO
            UTL_MENU_ITEM(
               ...
               SELECT
                  ...
                  NULL,
                  as_MenuName,
                  ics_MenuUrlPrefix || ls_ReportName || chr(38) || 'aView=' || as_MenuViewOutput || chr(38) || 'aPromptOutput=' || ls_PromptViewOutput || chr(38) || 'aRestrictedOutput=' || an_PromptRestrictedOutput,
                  ln_NewWindow,
                  0
                FROM
                  dual;
      ELSE
         UPDATE UTL_MENU_ITEM
            SET
                 ...
                 MENU_LINK_URL   = ics_MenuUrlPrefix || ls_ReportName || chr(38) || 'aView=' || as_MenuViewOutput|| chr(38) || 'aPromptOutput=' || ls_PromptViewOutput || chr(38) || 'aRestrictedOutput=' || an_PromptRestrictedOutput,
   ...
   EXCEPTION
      ...
      WHEN INVALID_REPORT_ENGINE THEN
         ...
         APPLICATION_OBJECT_PKG.SetMxiError(icn_InvalidReportEngine,'report_config_pkg@@@createuUpdateReportConfig@@@Report engine type is not a valid type (' || ics_LaunchBOE || ',' || ics_LaunchJasperREST || ',' || ics_LaunchJasperSSO || ',' || ics_LaunchJasperDashboard || ',' || ics_LaunchJasperLDashboard || ')');
