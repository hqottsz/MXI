...
--changeSet OPER-12114:1 stripComments:false endDelimiter:\n\s*/\s*\n|\n\s*/\s*$
--comment upgrading sched_stask_pkg
   ...
   *********************************************** */
   ...
   THEN
      ...
      LOOP
         ...
         EXCEPTION
		    -- if the tool part group has no part assigned then exception will be thrown
            WHEN NO_DATA_FOUND THEN
               ln_StdPartNoId := 0;
         END;

		 -- Do not insert any record in evt_tool for empty tool part group
         IF ln_StdPartNoId > 0 THEN
...
/* Add all the OC and Non OC Assembly Measurements.  */
   ...
   UpdatePartsAndToolsReadyBool( ln_EventDbId, ln_EventId, on_Return );
   IF on_Return < 1 THEN
     RETURN;
   END IF;
...
BEGIN
   ...
   BEGIN
     -- get the event key (db_id & id) info
     SELECT
        tasks.event_db_id,
        tasks.event_id
     INTO
     ...
     FROM
        (
           ...
           FROM
              ...
              evt_event
           WHERE
              sched_stask.task_db_id = an_taskdbid
              AND sched_stask.task_id = an_taskid
              AND sched_stask.orphan_frct_bool = 0
              AND sched_stask.rstat_cd = 0
              AND evt_event.event_db_id = sched_stask.sched_db_id
              AND evt_event.event_id = sched_stask.sched_id
              AND (evt_event.event_status_cd = 'ACTV' OR
              evt_event.event_status_cd = 'FORECAST' OR
              evt_event.event_status_cd = 'COMPLETE')
              AND sched_stask.main_inv_no_db_id = an_invnodbid
              AND sched_stask.main_inv_no_id = an_invnoid
              AND evt_sched_dead.event_db_id = sched_stask.sched_db_id
              AND evt_sched_dead.event_id = sched_stask.sched_id
              AND evt_sched_dead.sched_driver_bool = 1
              AND NOT EXISTS
              (
                 SELECT
                 ...
                 FROM
                    evt_event_rel
                 WHERE
                    evt_event_rel.event_db_id = evt_sched_dead.event_db_id
                    AND evt_event_rel.event_id = evt_sched_dead.event_id
                    AND evt_event_rel.rel_type_cd = 'DEPT'
              )
           ORDER BY
              decode(evt_event.hist_bool, 1, evt_event.event_dt, evt_sched_dead.sched_dead_dt ) desc
        ) tasks
     WHERE ROWNUM = 1;
   ...
   BEGIN
     ...
     IF ( lnEventDbId = -1 AND lnEventId = -1 ) THEN
        ...
        WHERE
           ...
           AND sched_stask.orphan_frct_bool = 0
           AND sched_stask.rstat_cd = 0
           AND task_event.event_db_id = sched_stask.sched_db_id
           AND task_event.event_id = sched_stask.sched_id
           AND
           (
           ...
           AND task_task.task_db_id = sched_stask.task_db_id
           AND task_task.task_id = sched_stask.task_id
           AND current_task_task.task_defn_db_id = task_task.task_defn_db_id
           AND current_task_task.task_defn_id = task_task.task_defn_id
           AND current_task_task.task_db_id = an_taskdbid
           AND current_task_task.task_id = an_taskid
           AND current_task_task.recurring_task_bool = 1
           AND NOT EXISTS
           (
              SELECT
                 evt_event_rel.event_db_id,
              ...
              FROM
                 evt_event_rel
              WHERE
                 evt_event_rel.event_db_id = task_event.event_db_id
                 AND evt_event_rel.event_id = task_event.event_id
                 AND evt_event_rel.rel_type_cd = 'DEPT'
           )
           AND ROWNUM = 1;
      END IF;
