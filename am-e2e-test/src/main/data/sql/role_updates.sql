...
-- Add station monitoring button for admin for line mechanic and line supervisor todo lists
-- line mechanic todo list
insert into UTL_TODO_LIST_BUTTON (TODO_BUTTON_ID, TODO_LIST_ID, BUTTON_ORDER, UTL_ID )
   SELECT 10069, 10003, 1, 4650
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_TODO_LIST_BUTTON where TODO_BUTTON_ID = 10069 AND TODO_LIST_ID = 10003
   );

-- line technician todo list
insert into UTL_MENU_GROUP_ITEM ( GROUP_ID, MENU_ID, MENU_ORDER, BREAK_BOOL, UTL_ID )
   SELECT 10011, 120950, 6, 1, 0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_MENU_GROUP_ITEM where GROUP_ID = 10011 AND MENU_ID = 120950
   );

-- line supervisor todo list
insert into UTL_TODO_LIST_BUTTON (TODO_BUTTON_ID, TODO_LIST_ID, BUTTON_ORDER, UTL_ID )
   SELECT 10069, 10004, 1, 4650
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_TODO_LIST_BUTTON where TODO_BUTTON_ID = 10069 AND TODO_LIST_ID = 10004
   );
...
-- Invoice Agent ROLE -------------------------------------------------------------------------------------------
-- create Invoice Agent menu

insert into utl_menu_group (GROUP_ID,GROUP_NAME,GROUP_ORDER,ROLE_ID,ALL_USERS_BOOL,UTL_ID)
   SELECT 10052,'Invoice Agent',1,10024,0,10
   FROM DUAL
   WHERE not exists (
      select 1 from utl_menu_group where GROUP_ID = 10052
   );

-- add items to the invoice agent menu

insert into UTL_MENU_GROUP_ITEM ( GROUP_ID, MENU_ID, MENU_ORDER, BREAK_BOOL, UTL_ID )
   SELECT 10052, 10120, 1, 1, 0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_MENU_GROUP_ITEM where GROUP_ID = 10052 AND MENU_ID = 10120
   );

insert into UTL_MENU_GROUP_ITEM ( GROUP_ID, MENU_ID, MENU_ORDER, BREAK_BOOL, UTL_ID )
   SELECT 10052, 10124, 1, 1, 0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_MENU_GROUP_ITEM where GROUP_ID = 10052 AND MENU_ID = 10124
   );
...
insert into UTL_TODO_LIST_TAB (TODO_TAB_ID,TODO_LIST_ID,TAB_ORDER,UTL_ID)
   SELECT 10050,10026,3,4650
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_TODO_LIST_TAB where TODO_TAB_ID = 10050 AND TODO_LIST_ID = 10026
   );

-- QC INSPECTOR ROLE -------------------------------------------

insert into UTL_MENU_GROUP_ITEM ( GROUP_ID, MENU_ID, MENU_ORDER, BREAK_BOOL, UTL_ID )
   SELECT 10013, 10127, 1, 1, 0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_MENU_GROUP_ITEM where GROUP_ID = 10013 AND MENU_ID = 10127
   );



-- STORE ROOM CLERK ROLE -------------------------------------------------------------------------------------------
-- Indicates Storeroom Clerk may turn in not issued Inventory

insert into UTL_ROLE_PARM (ROLE_ID,PARM_NAME,PARM_TYPE,PARM_VALUE,UTL_ID)
   SELECT 10017,'ALLOW_TURN_IN_NOT_ISSUED_INV','SECURED_RESOURCE','true',0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_ROLE_PARM where ROLE_ID = 10017 AND PARM_NAME = 'ALLOW_TURN_IN_NOT_ISSUED_INV'
   );

-- STORE ROOM CLERK ROLE--------------------------------------------------------------------------------------------
-- It will control if warnings will be generated for BATCH parts on turn in-------------------------------------------------------------------------------------------

insert into UTL_ROLE_PARM (ROLE_ID,PARM_NAME,PARM_TYPE,PARM_VALUE,UTL_ID)
   SELECT 10017,'TURN_IN_WARNING_FOR_BATCH','SECURED_RESOURCE','true',0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_ROLE_PARM where ROLE_ID = 10017 AND PARM_NAME = 'TURN_IN_WARNING_FOR_BATCH'
   );

-- STORE ROOM CLERK ROLE--------------------------------------------------------------------------------------------
-- Permission to allow user to accept a warning when the inventory is not issued-------------------------------------------------------------------------------------------

insert into UTL_ROLE_PARM (ROLE_ID,PARM_NAME,PARM_TYPE,PARM_VALUE,UTL_ID)
   SELECT 10017,'MESSAGE_INVENTORY_IS_NOT_ISSUED','SECURED_RESOURCE','true',0
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_ROLE_PARM where ROLE_ID = 10017 AND PARM_NAME = 'MESSAGE_INVENTORY_IS_NOT_ISSUED'
   );

-- STORE ROOM CLERK ROLE--------------------------------------------------------------------------------------------
-- Add Checked Out Tools tab to Storeroom Clerk To Do List-------------------------------------------------------------------------------------------

insert into UTL_TODO_LIST_TAB (TODO_TAB_ID,TODO_LIST_ID,TAB_ORDER,UTL_ID)
   SELECT 10036,10017,1,10
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_TODO_LIST_TAB where TODO_TAB_ID = 10036 AND TODO_LIST_ID = 10017
   );  

-- STORE ROOM CLERK ROLE--------------------------------------------------------------------------------------------
-- Add Inventory Count Button--------------------------------------------------------------------------------------------
insert into UTL_TODO_LIST_BUTTON (TODO_BUTTON_ID, TODO_LIST_ID, BUTTON_ORDER, UTL_ID )
   SELECT 10074, 10017, 1, 4650
   FROM DUAL
   WHERE not exists (
      select 1 from UTL_TODO_LIST_BUTTON where TODO_BUTTON_ID = 10074
   );