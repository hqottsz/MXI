...
public class CreatePurchaseInvoiceNoAccountStepDefinitions {
   ...
   public void
         theExternalSystemCompletesThreeWayMatchingAndSendsAnVALIDInboundCreatePurchaseInvoiceMessageWithoutAccount(
   ...
   public void
         theExternalSystemCompletesTwoWayMatchingAndSendsAnVALIDInboundCreatePurchaseInvoiceMessageWithoutAccount(
               List<Map<String, String>> aDataTable ) throws Throwable {
      // Get the PO Number to be used within inbound Create Purchase Invoice message
      // It will be appended to the Invoice Number as to make the Invoice Number unique each time
      // the scenario is executed
   ...
   public void
         anInvoiceWithAccountSetToOrderMiscellaneousLineAccountIsCreatedAndAPurchaseInvoiceCreatedMessageIsSent(
               List<Map<String, String>> aDataTable ) throws Throwable {
      ...
      for ( int i = 0; i <= lPOInvoiceLines.size() - 2; i++ ) {
         ...
         Assert.assertEquals( "The PO Invoice Lines Charge To Account is incorrect",
               aDataTable.get( 0 ).get( CHARGE_TO_ACCOUNT ), lChargeToAccount );
