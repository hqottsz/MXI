...
public class CreatePurchaseInvoicePartLineExceptionsStepDefinitions {
   ...
   // Variables to be used for validating Error Response message
   private final static String ERROR_RESPONSE_MESSAGE = "Error Message";
   private final static String NAMESPACE = "Namespace";
   private final static String PART_TYPE = "Part Type";


   // This When method covers EXCEPTION scenarios for Purchase Order Lines of PARTS (3 way matching)
   ...
   // "No Vendor and No Order"
   // "Invalid Order"
   // "No Invoice Quantity Unit and No Order Info"
   // "No Invoice Quantity Unit and Invalid Order Info"
   // "No Account and No Order Info"
   // "Not Received, No Optional Fields"
   // "Received, No Optional Fields"
   @When( "^the external system completes three way matching and sends an INVALID inbound Create Purchase Invoice message$" )
   public void
         theExternalSystemCompletesThreeWayMatchingAndSendsAnINVALIDInboundCreatePurchaseInvoiceMessage(
               List<Map<String, String>> aDataTable ) throws Throwable {
      if ( aDataTable.get( 0 ).get( INVALID_SCENARIO_MODE )
            .equalsIgnoreCase( "No Vendor and No Order" ) ) {
         ...
         // Create Sample Message as a String that has NO Vendor Information NOR Purchase Order
         // Information
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               ...
               + "\r\n" + "<account>" + "\r\n" + "<code>" + aDataTable.get( 0 ).get( ACCOUNT_CODE )
               + "</code>" + "\r\n" + "</account>" + "\r\n" + "</invoice-line>" + "\r\n"
               + "</invoice>" + "\r\n" + "</create-purchase-invoice>" + "\r\n" + "</ns1:request>"
               + "\r\n" + "</soap:Body>" + "\r\n" + "</soap:Envelope>";
      } else if ( aDataTable.get( 0 ).get( INVALID_SCENARIO_MODE )
            .equalsIgnoreCase( "Invalid Order" ) ) {
         // Get the PO Number to be used within inbound Create Purchase Invoice message
         // It will be appended to the Invoice Number as to make the Invoice Number unique each time
         // the scenario is executed
         // It will also be used to fill in the PO Number field
         iPONumber = iInventoryDetailsPageDriver.clickTabDetails().getOrderNumber();
         // Create Sample Message as a String that has NO Vendor Information and INVALID Purchase
         // Order Information
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               + "http://schemas.xmlsoap.org/soap/envelope/" + "\"" + " xmlns:ns=" + "\""
               + "http://xml.mxi.com/xsd/integration/header/1.1" + "\"" + " xmlns:ns1=" + "\""
               ...
               + "\r\n" + "<account>" + "\r\n" + "<code>" + aDataTable.get( 0 ).get( ACCOUNT_CODE )
               + "</code>" + "\r\n" + "</account>" + "\r\n" + "<purchase-order>" + "\r\n"
               + "<po-number>" + "JS999999" + "</po-number>" + "\r\n" + "<po-line-number>"
               + aDataTable.get( 0 ).get( PO_LINE_NUMBER ) + "</po-line-number>" + "\r\n"
               + "</purchase-order>" + "\r\n" + "</invoice-line>" + "\r\n" + "</invoice>" + "\r\n"
               + "</create-purchase-invoice>" + "\r\n" + "</ns1:request>" + "\r\n" + "</soap:Body>"
               + "\r\n" + "</soap:Envelope>";
      } else if ( aDataTable.get( 0 ).get( INVALID_SCENARIO_MODE )
            .equalsIgnoreCase( "No Invoice Quantity Unit and No Order Info" ) ) {
         // Get the PO Number to be used within inbound Create Purchase Invoice message
         // It will be appended to the Invoice Number as to make the Invoice Number unique each time
         // the scenario is executed
         // It will also be used to fill in the PO Number field
         iPONumber = iInventoryDetailsPageDriver.clickTabDetails().getOrderNumber();
         // Create Sample Message as a String that has NO Invoice Quantity Unit and NO Order Info
         // (and No Vendor nor Line Description nor Part Information)
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               + "http://schemas.xmlsoap.org/soap/envelope/" + "\"" + " xmlns:ns=" + "\""
               + "http://xml.mxi.com/xsd/integration/header/1.1" + "\"" + " xmlns:ns1=" + "\""
               + "http://xml.mxi.com/xsd/integration/request/1.1" + "\"" + ">" + "\r\n"
         ...
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
            ...
            .equalsIgnoreCase( "No Account and No Order Info" ) ) {
         // Get the PO Number to be used within inbound Create Purchase Invoice message
         // It will be appended to the Invoice Number as to make the Invoice Number unique each time
         // the scenario is executed
         // It will also be used to fill in the PO Number field
         iPONumber = iInventoryDetailsPageDriver.clickTabDetails().getOrderNumber();
         // Create Sample Message as a String that has NO Account and NO Order Info
         // (and No Vendor nor Line Description nor Part Information nor Invoice Quantity Unit)
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               + "http://schemas.xmlsoap.org/soap/envelope/" + "\"" + " xmlns:ns=" + "\""
               + "http://xml.mxi.com/xsd/integration/header/1.1" + "\"" + " xmlns:ns1=" + "\""
               + "http://xml.mxi.com/xsd/integration/request/1.1" + "\"" + ">" + "\r\n"
            ...
            .equalsIgnoreCase( "Not Received, No Optional Fields" ) ) {
         // Get the PO Number to be used within inbound Create Purchase Invoice message
         // It will be appended to the Invoice Number as to make the Invoice Number unique each time
         // the scenario is executed
         // It will also be used to fill in the PO Number field
         ...
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               ...
               + "<po-line-number>" + aDataTable.get( 0 ).get( PO_LINE_NUMBER )
               + "</po-line-number>" + "\r\n" + "</purchase-order>" + "\r\n" + "</invoice-line>"
               + "\r\n" + "</invoice>" + "\r\n" + "</create-purchase-invoice>" + "\r\n"
               + "</ns1:request>" + "\r\n" + "</soap:Body>" + "\r\n" + "</soap:Envelope>";
      } else if ( aDataTable.get( 0 ).get( INVALID_SCENARIO_MODE )
            .equalsIgnoreCase( "Received, No Optional Fields" ) ) {
         // Get the PO Number to be used within inbound Create Purchase Invoice message
         // It will be appended to the Invoice Number as to make the Invoice Number unique each time
         // the scenario is executed
         // It will also be used to fill in the PO Number field
         iPONumber = iShipmentDetailsPageDriver.getOrderNumber();
         // Create Sample Message as a String that has NO Account and NO Order Info
         // (and No Vendor nor Line Description nor Part Information nor Invoice Quantity Unit)
         lSampleMessage = "<soap:Envelope xmlns:soap=" + "\""
               + "http://schemas.xmlsoap.org/soap/envelope/" + "\"" + " xmlns:ns=" + "\""
   ...
   public void anInvoiceIsNOTCreatedAndAnErrorMessageIsSent( List<Map<String, String>> aDataTable )
         throws Throwable {

      if ( ( aDataTable.get( 0 ).get( PART_TYPE ).equalsIgnoreCase( "Miscellaneous" ) )
            || ( aDataTable.get( 0 ).get( PART_TYPE )
                  .equalsIgnoreCase( "Tracked, Not Received" ) ) ) {
         // Get the PO Number to be used when searching for the Invoice from the Order Details Page
         iPONumber = iOrderDetailsDriver.getPONumber();
      } else if ( aDataTable.get( 0 ).get( PART_TYPE ).equalsIgnoreCase( "Tracked, Received" ) ) {
         // Get the PO Number to be used when searching for the Invoice from the Shipment Details
         // Page
         iPONumber = iShipmentDetailsPageDriver.getOrderNumber();
      } else {
         // Get the PO Number to be used when searching for the Invoice from the Inventory Details
         // Page
         iPONumber = iInventoryDetailsPageDriver.clickTabDetails().getOrderNumber();
      }
      ...
      // fail if there IS a Result Returned. That is, if the returned List has a size greater than
      // zero
      if ( lsizePOInvoiceResultRow > 0 ) {
         Assert.fail(
               "The Search returned one or more PO Invoices. The Invoice specified does exist in system, however should NOT exist in the system" );
      }
   }
}
