...
/**
 * This class tests the query com.mxi.mx.web.query.qarantine.QuarantineInfo.qrx
 */
@RunWith( BlockJUnit4ClassRunner.class )
public class QuarantineInfoTest {
    ...
    *
    * When the query QuarantineInfo.qrx is executed
    *
    * Then the quarantine info associated with the component is returned
    *
    */
   @Test
   public void testGetQuarantineInfoByComponent() {
      ...
      // ACT
      final QuerySet lQs = executeQueryByComponent( lComponent1.getDbId(), lComponent1.getId() );
   ...
   /**
    * Given quarantine events on specified quarantine
    *
    * When the query QuarantineInfo.qrx is executed
    *
    * Then the quarantine info associated with the quarantine is returned
    *
    */
   @Test
   public void testGetQuarantineInfoByQuarantine() {
      ...
      // ACT
      final QuerySet lQs = executeQueryByQuanrantine( lQuarQuar1.getPk().getQuarDbId(),
            lQuarQuar1.getPk().getQuarId() );
      ...
      final Calendar lQuarCalendar = Calendar.getInstance();
      lQuarCalendar.setTime( lQuarQuar.getDate() );
      long lActualQuarDateL = lQuarCalendar1.getTimeInMillis();
      ...
      assertClobQuarNoteCanBeConvertedToString( lQs );
   }


   /**
    * Assert that the retrieved quarantine note in Oracle CLOB data type can be converted to string
    * correctly without throwing exception
    *
    * @param aQuerySet
    */
   private void assertClobQuarNoteCanBeConvertedToString( QuerySet aQuerySet ) {
      aQuerySet.next();
      Object lQuarNoteClob = aQuerySet.getObject( "quar_note" );
      String lQuarNoteString = DataTypeUtils.toString( lQuarNoteClob );
      assertEquals( "Incorrect quarantine note", GUARANTINE_NOTE, lQuarNoteString );
   }
   ...
   private QuerySet executeQueryByComponent( int aInvNoDbId, int aInvNoId ) {
   ...
   private QuerySet executeQueryByQuanrantine( int aQuarDbId, int aQuarId ) {
      DataSetArgument lArgs = new DataSetArgument();
      lArgs.add( "aQuarDbId", aQuarDbId );
      lArgs.add( "aQuarId", aQuarId );
