...
import com.mxi.mx.core.worker.task.preconditions.CompleteTaskPrecondition;


/**
 * This class tests that the complete task precondition which ensures tasks that do not have removal
 * tasks are executed only after tasks which do contain removals is working as expected
 *
 */
@RunWith( BlockJUnit4ClassRunner.class )
public class EvaluateCompleteTaskPreConditionTest {
   ...
   /**
    *
    * Tests that when a work item for completing a task that contains a removal task is present, a
    * precondition failure occurs indicating that we must wait for all task bundles containing
    * removal tasks to first complete prior to working on other tasks which may contain install
    * tasks
    *
    * @throws Throwable
    */
   ...
   public void testPrecondition() throws Throwable {
      ...
      // Force in the special Removal Task Flag indicating that some task in this work item has a
      // removal
      lCompleteTask = new CompleteTask.CompleteTaskFromDataBuilder()
            .data( lCompleteTask.getData() + ";" + CompleteTask.REMOVAL_TASK )
            .key( lCompleteTask.getKey() ).build();

      generateWorkItem( lCompleteTask );

      CompleteTaskPrecondition lPrecondition =
            new CompleteTaskPrecondition( BATCH_COMPLETE_ID, lCompleteTask.getData() );
