...
/**
 * Tests for {@linkplain TaskDefnService#preventManualInitialization} that involve testing the
 * behavior of preventing manual initialization of task definitions
 */
public class PreventManualInitializationTest {
    ...
    * When the manual initialization is prevented
   ...
   public void itThrowsInvalidClassModeCodeExceptionWhenClassModeCodeNotReq() throws Exception {
      ...
      // When
      iTaskDefnService.preventManualInitialization( lJicDefn, null, null );
   ...
   public void itThrowsInvalidTaskDefinitionStatusExceptionWhenTaskDefinitionStatusObsolete()
      ...
      iExpectedException.expectMessage( lExpectedExceptionMessage );
      // When the manual initialization is prevented
      iTaskDefnService.preventManualInitialization( lReqDefn, null, null );
   ...
   @Test
   public void
         itThrowsActionAlreadyPerformedExceptionWhenTaskDefinitionIsAlreadyPreventManualInitialization()
               throws Exception {

      String lExpectedExceptionMessage =
            "[MXERR-33707] The action could not be completed because 'Prevent Manual Initialization' action is already performed for this task definition.";
      ...
      // When
      iTaskDefnService.preventManualInitialization( lReqDefn, null, null );
      fail( "ActionAlreadyPerformedException expected" );
    ...
    */
   @Test
   public void itSetsPreventManualInitialization() throws Exception {
      ...
      // When
      iTaskDefnService.preventManualInitialization( lReqDefn, lHr, null );
      ...
      // Then
      assertTrue( "Expected prevent manual initialization to be true",
    ...
    */
   @Test
   public void itRecordsSystemAndUserNoteWhenPreventManualInitializationIsSet() throws Exception {
      ...
      // When
      iTaskDefnService.preventManualInitialization( lReqDefn, lHr, USER_NOTE );
      ...
      if ( !lQs.next() )
         fail( "Unexpectedly, Prevent Manual Initialization action not logged" );

      String lExpectedSystemNote = i18n.get( "core.msg.PREVENT_MANUAL_INITIALIZATION" );
      // Then
      assertEquals( "Unexpected system note for prevent manual initialziation", lExpectedSystemNote,
            lQs.getString( TaskTaskLogTable.ColumnName.SYSTEM_NOTE.name() ) );
      assertEquals( "Unexpected user note for prevent manual initialziation", USER_NOTE,
