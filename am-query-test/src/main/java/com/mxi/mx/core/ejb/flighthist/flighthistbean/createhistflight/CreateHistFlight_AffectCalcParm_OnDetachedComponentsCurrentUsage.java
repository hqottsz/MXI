...
/**
 * Tests for {@linkplain FlightHistBean#createHistFlight} that involve updating detached components
 * calculated parameters within current usage
 */
public class CreateHistFlight_AffectCalcParm_OnDetachedComponentsCurrentUsage {
    ...
    * adjusted when a flight is created with an arrival time prior to when the component was
    * detached from the flight's aircraft.
    *
    ...
    * And the tracked component is tracking the same parameters as the engine.
    *
    * When create a historical flight before the component was detached.
    *
    * Then verify on the component current CYCLES is the sum of original current usage plus the
    * historical flight CYCLES usage and calc parm is recalculated based on the new CYCLES current
    * usage.
    */
   @Test
   public void itUpdatesUsageParmsAndCalcParmsOnDetachedComponents() throws Exception {
      ...
      final InventoryKey lEngine = Domain.createEngine( new DomainConfiguration<Engine>() {
         ...
         public void configure( Engine aBuilder ) {
            ...
            aBuilder.addUsage( MimDataType.getDataType( CALC_PARM_CD ), BigDecimal.ZERO );
      ...
      CollectedUsageParm[] lUsageParms = { generateFlightUsage( lAircraft, CYCLES, BigDecimal.ONE ),
            generateFlightUsage( lEngine, CYCLES, BigDecimal.ONE ) };

      iFlightHistBean.createHistFlight( new AircraftKey( lAircraft ), lHrKey,
            generateFlightInfoTO( FLIGHT_NAME, FLIGHT_DATE, FLIGHT_DATE ), lUsageParms,
            NO_MEASUREMENTS );
      ...
      for ( InvCurrUsage lInvCurrUsage : lInvCurrUsages ) {
         if ( lInvCurrUsage.getDataType().equals( CYCLES ) ) {
            assertEquals( new BigDecimal( lInvCurrUsage.getTsnQt() ), BigDecimal.ONE );

            assertEquals( new BigDecimal( lInvCurrUsage.getTsiQt() ), BigDecimal.ONE );

            assertEquals( new BigDecimal( lInvCurrUsage.getTsoQt() ), BigDecimal.ONE );
         }

         else {

            assertEquals( new BigDecimal( lInvCurrUsage.getTsnQt() ), new BigDecimal( 2 ) );

            assertEquals( new BigDecimal( lInvCurrUsage.getTsiQt() ), new BigDecimal( 2 ) );

            assertEquals( new BigDecimal( lInvCurrUsage.getTsoQt() ), new BigDecimal( 2 ) );
         }
      }
